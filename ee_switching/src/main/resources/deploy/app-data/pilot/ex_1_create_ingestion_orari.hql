...
drop table if exists  ${hiveconf:SWITCHING_EE_HIVE_DB_NAME}.ingestion_orari;

create table ${hiveconf:SWITCHING_EE_HIVE_DB_NAME}.ingestion_orari as
WITH quarti_ext as (
...
b.gruppomis , b.forfait , b.motivazione_stima , a.d_data_decorrenza, a.d_creazione,
...
(select * from
${hiveconf:AU_DB_NAME}.${hiveconf:SWITCHING_EE_HIVE_FLUSSO_MISURE_QUARTI} a
...
${hiveconf:AU_DB_NAME}.${hiveconf:SWITCHING_EE_HIVE_FLUSSO_MISURE_ESTENSIONE_QUARTI} b
INNER JOIN ${hiveconf:SWITCHING_EE_HIVE_DB_NAME}.bb
...
AND a.d_data_decorrenza=b.d_data_decorrenza AND a.d_creazione=b.d_creazione
)
...
SELECT a.pod14,
  ...
  , a.d_data_decorrenza, a.d_creazione
FROM
  (SELECT SUBSTR(b.podquarti,1,14) pod14,
    ...
    , b.d_data_decorrenza, b.d_creazione
  FROM quarti_ext b
  INNER JOIN ${hiveconf:SWITCHING_EE_HIVE_DB_NAME}.bb
  ON SUBSTR(b.podquarti,1,14)      =bb.pod14 AND b.d_data_decorrenza=bb.d_data_decorrenza AND b.d_creazione=bb.d_creazione
  WHERE
--  (tipodato_e            = 1
--  OR (tipodato_e             = 0
--  AND tipodato_s             = 0))
