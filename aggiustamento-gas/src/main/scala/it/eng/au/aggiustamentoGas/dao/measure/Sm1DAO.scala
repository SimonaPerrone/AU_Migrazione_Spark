...
class Sm1DAO extends MeasureDAO {
  override val parquetPath: String = Environment.getSm1ParquetPath
  ...
  override val filterFlow: Flow => Boolean = (f: Flow) => (f.ammissibilita.isEmpty && f.outcome == Some('1') && Set('E', 'S').contains(f.readType.getOrElse('-'))) ||
    (f.ammissibilita.isDefined && Set('E', 'S', 'A').contains(f.readType.getOrElse('-')))

  override val mapFunc: Row => Flow = (r: Row) => {
    val dateOldFlow = r.getAs[String](Sm1Schema.data_ril)
    val dateStdFlow = r.getAs[String](Sm1Schema.data_prest)
    val segnMisSost = r.getAs[String](Sm1Schema.segn_mis)
    val segnConv = r.getAs[String](Sm1Schema.segn_conv)
    val letTotPrel = r.getAs[String](Sm1Schema.let_tot_prel)
    val letTotConv = r.getAs[String](Sm1Schema.let_tot_conv)
    var esito = r.getAs[String](Sm1Schema.esito)
