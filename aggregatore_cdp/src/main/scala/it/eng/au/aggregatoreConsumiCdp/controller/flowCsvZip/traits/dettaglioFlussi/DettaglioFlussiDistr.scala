...
trait DettaglioFlussiDistr extends DettaglioFlussi {
  override val baseName: String = "CDP2"
  override val keyFields: List[String] = List(OutputCsvSchema.piva_distr.toString, OutputCsvSchema.piva_udd.toString)
  override val mainPiva: String = OutputCsvSchema.piva_distr.toString
  ...
  override val csvFields: List[String] = aggregatoColumns.values.toList.diff(List(OutputCsvSchema.piva_distr.toString, OutputCsvSchema.sessione.toString))

  override def fileSpecificFilterExpression: Column = col(OutputCsvSchema.piva_distr).isNotNull
