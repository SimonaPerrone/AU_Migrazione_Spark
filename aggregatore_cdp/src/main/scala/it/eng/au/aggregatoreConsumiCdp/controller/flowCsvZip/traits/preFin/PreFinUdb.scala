...
trait PreFinUdb extends PreFin {
  ...
  override def fileSpecificFilterExpression: Column = col(OutputCsvSchema.piva_udd).isNotNull && col(OutputCsvSchema.piva_udb).isNotNull
}
