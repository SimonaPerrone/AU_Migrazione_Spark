...
import it.au.misure.eng.model.{ReportEsitoFILEMessage, ReportMessage, Rule, RuleParameters, XMLMetadata}
...
object CheckAmmissibilitaFileRulesSMIS extends CheckAmmissibilitaRules[ReportEsitoFILEMessage] {
  ...
  val rulePIVARcuEmtCodContrDisp: Rule = Rule( //13
    ...
    message = ReportEsitoFILEMessage(
      codiceInamissibilita = COD904,
      descrizione = "La richiesta non è eseguibile (la PIVA_UTENTE non identifica un esercente la maggior tutela in RCU)",
      ammissibilita = FILE
    )
  )
  ...
  val ruleCodContrDispStruct: Rule = Rule( //14
    ...
    message = ReportEsitoFILEMessage(
      ...
      descrizione = ERROR_CONTRACT_CODE,
      ammissibilita = FILE
    )
  )
