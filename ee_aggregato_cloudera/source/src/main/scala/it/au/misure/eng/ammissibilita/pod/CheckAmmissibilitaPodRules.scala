...
object CheckAmmissibilitaPodRules extends CheckAmmissibilitaRules[ReportEsitoPODMessage] {
  ...
  val ruleDataMisura: Rule = Rule( // 16
    ...
    message = ReportEsitoPODMessage(
      ...
      descrizione = ERROR_COMPILATION_FIELD + "(tag Datamisura non valorizzato)",
  ...
  val ruleMeseAnno: Rule = Rule( //17
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag meseanno non valorizzato)",
      bloccante = BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleCodPrat: Rule = Rule( //18
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag CodPrat_SII non valorizzato)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleDataPrest1: Rule = Rule( //19
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag data prestazione non valorizzato)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleKaKrKp: Rule = Rule( //21
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag “ka”, “kr”, “kp” non valorizzati)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleMotivazioneStima: Rule = Rule( //22
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag Motivazione stima non valorizzato)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleTrattamento: Rule = Rule( //23
    ...
    condition = (datiPod: NodeSeq, fileXmlWIthMeta: XMLMetadata, _) => {
      Set("PNO2G", "VNO", "VNO2G").contains(fileXmlWIthMeta.codFlusso) &&
        (datiPod \\ Trattamento).text.equalsIgnoreCase("O")
    },
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag Trattamento non coerente con codice flusso)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleCodFlussoRaccolta: Rule = Rule( //24
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag Raccolta)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleCodFlussoTipoRettifica: Rule = Rule( //25
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(tag TipoRettifica)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
  ...
  val ruleCodFlussoPotMax: Rule = Rule( //26
    ...
    message = ReportEsitoPODMessage(
      codiceInamissibilita = COD010,
      descrizione = ERROR_COMPILATION_FIELD + "(campo “PotMax” non presente)",
      bloccante = NON_BLOCCANTE,
      ammissibilita = POD
    )
  )
