...
import it.au.misure.eng.schema.ammissibilita.AmmissibilitaFileSchema._
import it.au.misure.eng.schema.report_decompressione.ReportDecompressioneSchema
import it.au.misure.eng.utility.Constants.OK
...
object CheckAmmissibilitaFile extends CheckAmmissibilitaEE {
  def run(args: Array[String]): (RDD[XMLMetadata], DataFrame, DataFrame) = {
    ...
    if (params.isSmis) initializeSpark("Ammissibiità SMIS") else initializeSpark("Ammissibiità EE")
  ...
  def writeOnHive(rdd: RDD[ReportEsitoFILEMessage])(implicit sc: SparkContext, SQLContext: SQLContext): Unit = {
  ...
  def getRCUMaps(rcuAziendaPDF: DataFrame, rcuUddPDF: DataFrame, params: AmmissibilitaParameters)(implicit sc: SparkContext, SQLContext: SQLContext): (Map[String, (LocalDateTime, LocalDateTime)], Map[String, Set[String]]) = {
  ...
  def getAlreadyTransmittedFiles(rddMeta: RDD[XMLMetadata], params: AmmissibilitaParameters): RDD[(String, Boolean, XMLMetadata)] = {
    ...
    val reportDecompressione = hiveContext.table(PropertyUtility.getUnzipReportTable)
  ...
  def unionRcusUddP(rcuAziendaPDF: DataFrame, joinRcuUddAzienda: DataFrame): DataFrame = {
    ...
    joinRcuUddAzienda.select(RcuAziendaPSchema.t_piva, RcuUddPSchema.d_inizio, RcuUddPSchema.d_fine, RcuUddPSchema.t_codice_terna)
      .unionAll(rcusAziendaPDF).coalesce(joinRcuUddAzienda.rdd.getNumPartitions)
  }

  def getListPivaRcuDistr(rcuAziendaPDF: DataFrame)(implicit sc: SparkContext, SQLContext: SQLContext): List[String] = {
  ...
  def getListPivaRcuEmt(rcuAziendaPDF: DataFrame)(implicit sc: SparkContext, SQLContext: SQLContext): List[String] = {
