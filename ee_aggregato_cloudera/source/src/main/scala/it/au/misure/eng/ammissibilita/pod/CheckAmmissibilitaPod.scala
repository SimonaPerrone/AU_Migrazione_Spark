...
import it.au.misure.eng.ammissibilita.{CheckAmmissibilitaEE, CheckAmmissibilitaRules}
import it.au.misure.eng.args.AmmissibilitaParameters
import it.au.misure.eng.model.{ReportEsitoPODMessage, XMLMetadata}
import it.au.misure.eng.schema.GenericXmlSchema._
import it.au.misure.eng.schema.{RcuAziendaPSchema, RcuPodDistrPSchema, RcuPodPSchema, RcuPodUddPSchema, RcusPodDistrPSchema, RcusPodUddPSchema, RcuUddPSchema}
import it.au.misure.eng.schema.ammissibilita.AmmissibilitaPodSchema._
import it.au.misure.eng.utility._
...
object CheckAmmissibilitaPod extends CheckAmmissibilitaEE {
  def run(args: Array[String], tripleAdmissibleFilesRcuAziendaRcuUdd: (RDD[XMLMetadata], DataFrame, DataFrame)): Unit = {
    initializeSpark("Ammissibiità EE POD")
    ...
    val fileWithMessages =
      if (params.isSmis) {
        ...
        validateSMIS(admissibleFilesWithoutxsd.repartition(3000), resultid18, resultid19)
  ...
  def validateSMIS(rddXmlMetadata: RDD[XMLMetadata], unionPodDistrId18: DataFrame, unionPodDistrId19: DataFrame): RDD[(XMLMetadata, List[ReportEsitoPODMessage])] = {
    ...
    val rddWithIsPodCompetenceDistrUdd = rddMeta
      ...
      .map({ case (pod, ((dataMisura, meta, datiPod), desc)) => {
        ...
        (meta.copy(isPodCompetenceUdd = isPodCompetenceUdd), datiPod)
      }
      })
  ...
  def writeOnHive(rdd: RDD[ReportEsitoPODMessage])(implicit sc: SparkContext, SQLContext: SQLContext): Unit = {
    ...
    rdd.toDF
      ...
      .repartition(100)
      .write.partitionBy(anno, mese, giorno)
  ...
  def recoverRddXmlWithMetaSMIS(rcuAziendaPDF: DataFrame, rcuPod: DataFrame, rcusPod: DataFrame, rcuPodDistr: DataFrame, rcusPodDistr: DataFrame, rcuPodUdd: DataFrame, rcusPodUdd: DataFrame, rcuUdd: DataFrame): (DataFrame, DataFrame) = {
    ...
    val unionRcuPodAndRcusPod = rcuPod.unionAll(rcusPod).coalesce(rcuPod.rdd.getNumPartitions)
    //      .persist(StorageLevel.MEMORY_AND_DISK)
  ...
  def getPodCompetenceDistrOrUdd(rcuAziendaPDF: DataFrame, unionRcuPodAndRcusPod: DataFrame, rcuPodDistr: DataFrame, rcusPodDistr: DataFrame, rcuUddPDF: DataFrame, rcuPodUdd: DataFrame, rcusPodUdd: DataFrame): (DataFrame, DataFrame) = {
    ...
    val unionPodDistrId18 = rcuPodDistrJoinRcuAziendaPodPodId18.unionAll(rcusPodDistrJoinRcuAziendaPodPodId18).coalesce(rcusPodDistrJoinRcuAziendaPodPodId18.rdd.getNumPartitions)
    ...
    val unionPodDistrId19 = rcuPodDistrJoinRcuAziendaPodPodId19.unionAll(rcusPodDistrJoinRcuAziendaPodPodId19).coalesce(rcusPodDistrJoinRcuAziendaPodPodId19.rdd.getNumPartitions)
