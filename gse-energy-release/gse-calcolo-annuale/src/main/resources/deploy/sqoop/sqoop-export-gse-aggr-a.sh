...
sqoop export \
    ...
    --table 'GSE.GSE_AGGR_A' \
    --columns 'N_ID_GSE_RICHIESTA_ER_A,T_ANNO,T_MESE,T_COD_POD,N_CONSUMO_MENSILE,N_EXECUTION_ID,D_DATA_CREAZIONE' \
    --hcatalog-database '${gse.db}' \
    --hcatalog-table 'gse_aggr_a_export' \
    --input-null-string '\\N' \
    --input-null-non-string '\\N' \
    --staging-table 'GSE.GSE_AGGR_A_STG' \
    --clear-staging-table \
    --num-mappers 20
...
then
  echo "Sqoop export success, delete GSE_AGGR_A_EXPORT data..."
  hdfs dfs \
      -rm \
      -f \
      -skipTrash \
      "${hdfs.output.path}/gse_aggr_a_export/*"
else
    exit $errorMessage
fi