...
import it.eng.au.model.{ReportEsitoFILEMessage, ReportMessage, Rule, XMLMetadata}
...
object CheckAmmissibilitaFileRules extends CheckAmmissibilitaRules[ReportEsitoFILEMessage] {
  ...
  val ruleCodDpPIVA: Rule = Rule( //6
    ...
    message = ReportEsitoFILEMessage(
      codiceInamissibilita = COD904,
      descrizione = ERROR_FILE_STRUCTURE + "- COD_DP non coerente con PIVA RCU",
      ammissibilita = FILE
    )
  )
  ...
  val ruleStatoMisuratore: Rule = Rule( //9
    ...
    message = ReportEsitoFILEMessage(
      codiceInamissibilita = COD904,
      descrizione = ERROR_FILE_STRUCTURE + "- Stato misuratore",
      ammissibilita = FILE
    )
  )
  ...
  val ruleFlusso1XsdVal: Rule = Rule( //12
    ...
    condition = (fileContent: NodeSeq, fileXmlWIthMeta: XMLMetadata, _) => {
      val flusso1ListExtended = flusso1List ::: flusso1ListRule12
      if (flusso1ListExtended.contains(fileXmlWIthMeta.flusso.toLowerCase())) {
    ...
    message = ReportEsitoFILEMessage(
      codiceInamissibilita = COD904,
      descrizione = ERROR_FILE_STRUCTURE + "- Struttura xml Flusso 1",
      ammissibilita = FILE
    )
  )
  ...
  val ruleFlusso2XsdVal: Rule = Rule( //13
    ...
    condition = (fileContent: NodeSeq, fileXmlWIthMeta: XMLMetadata, _) => {
      val flusso2ListExtended = flusso2List ::: flusso2ListRule13
      if (flusso2ListExtended.contains(fileXmlWIthMeta.flusso.toLowerCase())) {
    ...
    message = ReportEsitoFILEMessage(
      codiceInamissibilita = COD904,
      descrizione = ERROR_FILE_STRUCTURE + "- Struttura xml Flusso 2",
      ammissibilita = FILE
    )
  )
