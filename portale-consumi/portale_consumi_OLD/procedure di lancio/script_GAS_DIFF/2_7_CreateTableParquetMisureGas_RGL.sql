...
where CAST(date_format(d_caricamento,'yyyyMMdd') as int) >=  ${hiveconf:last_dt_elab_rgl} and
      cast((date_format(data_racc,'yyyyMM')) as int) >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int) ;
