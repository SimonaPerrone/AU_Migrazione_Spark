...
create table misuregas.consumi_result_mm  Stored As Parquet as
	...
	and annomese >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int)  ;
...
from (select * from misuregas.misure_storic where tipo_flusso='TAL' and annomese >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int) )  as PRT_CMG_TAL
...
select  PRT_CMG_TAV.cod_pdr,
    ...
    codice_fornitura,
    tipo_flusso as Flusso
from (select * from misuregas.misure_storic where tipo_flusso='TAV' and annomese >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int)  ) as PRT_CMG_TAV
...
drop table misuregas.letture_union;
    create table misuregas.letture_union Stored As Parquet as
		...
		from misuregas.misure_storic where tipo_flusso in('TGL','RGL') and annomese >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int)  ; 
