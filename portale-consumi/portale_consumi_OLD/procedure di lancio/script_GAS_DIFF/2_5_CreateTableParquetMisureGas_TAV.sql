...
where CAST(date_format(d_caricamento,'yyyyMMdd') as int) >=  ${hiveconf:last_dt_elab_tav} and
      cast(annomese as int) >=  cast(date_format(date_sub(current_date(),${env:limit_gg_gas}),'yyyyMM') as int);
