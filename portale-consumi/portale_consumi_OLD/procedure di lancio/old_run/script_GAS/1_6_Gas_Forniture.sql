...
SELECT distinct
   ...
   case 
   when rcugas_fornitura.inizio_num < cast(date_format(date_sub(current_date,396),'yyyyMMdd') as int)
        then cast(date_format(date_sub(current_date,396),'yyyyMMdd') as int)
...
FROM (
   ...
   cast(case when rcugas_fornitura.d_data_inizio is null then date_format(date_sub(current_date,396),'yyyyMMdd') else date_format(rcugas_fornitura.d_data_inizio,'yyyyMMdd') end  as int) inizio_num,
 ...
 (cast(date_format(rcugas_fornitura.d_data_fine,'yyyyMMdd') as int) >= cast(date_format(date_sub(current_date,396),'yyyyMMdd') as int)))
