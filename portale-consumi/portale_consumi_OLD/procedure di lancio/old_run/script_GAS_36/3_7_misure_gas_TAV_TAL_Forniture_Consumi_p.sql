drop table misuregas.consumi_result_mm;
  create table misuregas.consumi_result_mm  Stored As Parquet as
    select *,'' as Motivazione,'0' as priority from misuregas.letture_tml UNION ALL select *,'1' as priority from misuregas.letture_rml;
...
create table misuregas.consumi_result_mm2  Stored As Parquet as
    select distinct
            ...
            lu.tipo_lettura,
            lu.codice_fornitura,
            lu.Flusso,'AF' as Tipo from misuregas.consumi_result_mm as lu
inner join (
   ...
   data_racc,dt_caricamento,priority
    ...
    group by T1.cod_pdr,T1.annomese_riferimento
     ...
     (
      ...
      PRT_VTG6.flusso
      FROM misuregas.letture_vtg PRT_VTG6 where PRT_VTG6.cod_pdr_rmv is null
     ) as volture
union all select *, 'V' as Tipo from
(
select  PRT_CMG_RMV.cod_pdr,PRT_CMG_RMV.annomese_riferimento,
        ...
        PRT_CMG_RMV.Flusso
    from misuregas.letture_rmv PRT_CMG_RMV
) as rmv;
...
where
    ...
    and cast(date_format(data_lettura,'yyyyMMdd') as int) >=data_inizio_fornitura_num and
...
create table misuregas.union_autoletture Stored As Parquet as
  select * from misuregas.letture_tav UNION ALL select * from misuregas.letture_tal;
