...
CAST(CASE WHEN nvl(d_inizio_validita,'')='' THEN concat( year(date_sub(current_date,396)),lpad(month(date_sub(current_date,396)),2,0),'01') ELSE CAST(CONCAT(SUBSTR(d_inizio_validita,1,4),SUBSTR(d_inizio_validita,6,2),SUBSTR(d_inizio_validita,9,2)) AS INT) END AS BIGINT)d_inizio_validita,
