...
CAST(CASE WHEN d_inizio='' THEN concat( year(date_sub(current_date,125)),lpad(month(date_sub(current_date,125)),2,0),'01') ELSE CAST(CONCAT(SUBSTR(d_inizio,1,4),SUBSTR(d_inizio,6,2),SUBSTR(d_inizio,9,2)) AS INT) END AS BIGINT)inizio,
...
CAST(concat( year(date_sub(current_date,125)),lpad(month(date_sub(current_date,125)),2,0),'01') as BIGINT) d_inizio_start
from 
(
 select n_id_fornitura,
...
where  fine >= cast(concat(year(date_sub(current_date,125)),lpad(month(date_sub(current_date,125)),2,0),'01') as bigint)  and (inizio+2) < fine
) AS TTX
