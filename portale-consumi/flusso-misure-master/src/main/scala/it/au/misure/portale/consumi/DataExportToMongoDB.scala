...
object DataExportToMongoDB extends LoggingSupport {
  ...
  val connectionStringMongo= prop.getProperty("spark.app.mongodb.connstr.produzione")
  val connectionStringMongoCollaudo=prop.getProperty("spark.app.mongodb.connstr.collaudo")
...
*/
  ...
  var numeroMesi = prop.getProperty("spark.app.mongodb.delay")
  var mesiTracciaturaSpid = prop.getProperty("spark.app.mongodb.mesiTracciaturaSpid")
  ...
  def main(args: Array[String]) {
    ...
    try {
      if (tipoExport == "F") {
        /*
        * mev21_059 svecchiamento tracciatura spid
        */
       // SvecchiamentoTracciaturaSpid(dbMongo,collezioneMongoTracciaturaSpid)
...
append("data_lettura", "20200515")
  ...
  def ExportMisureElettrico(dbMongo :String,collectionMongo:String): Unit ={
    ...
    {
      cal.add(Calendar.DAY_OF_YEAR, -this.calcolaDelay()) // Mev21_056
      val anno: String = Integer.toString(cal.get(Calendar.YEAR))
      val mese: String = "0" + Integer.toString(cal.get(Calendar.MONTH) + 1) takeRight 2
  ...
  //MEV21_056
  def calcolaDelay (): Int = {
    var mesi = 3
    try{
      mesi = this.numeroMesi.toInt
    }catch {
  ...
  def  SvecchiamentoTracciaturaSpid (dbMongo:String,collectionMongo:String): Unit = {
    import org.mongodb.scala._
    println(s"inizio procedura di svecchiamento" )
    var mesi = 6
    try{
      mesi = this.mesiTracciaturaSpid.toInt
    }catch {
      case e: Exception => { mesi = 6}
    }
    var sogliaTracciaturaSpid = mesi * 30

    val timeZone = prop.getProperty("spark.app.time_zone")
    val cal = Calendar.getInstance(TimeZone.getTimeZone(timeZone))
    cal.add(Calendar.DAY_OF_YEAR, -sogliaTracciaturaSpid)
    val anno: String = Integer.toString(cal.get(Calendar.YEAR))
    val mese: String = "0" + Integer.toString(cal.get(Calendar.MONTH) + 1) takeRight 2
    val giorno: String = Integer.toString(cal.get(Calendar.DATE))
    val LimiteTraccaituraSpid = anno.toString + (("0" + mese.toString) takeRight 2)+giorno
    println(s"calocalto numero di mesi da tenere per svecchiamento =: ${LimiteTraccaituraSpid}")

    var connString = ""
    if (mongocollaudo) {
      connString = connectionStringMongoCollaudo
    } else {
      connString = s"${connectionStringMongo}/?replicaSet=rset&authSource=${dbMongo}"
    }
    println(s"stringa di connessione: ${connString}")
    try {
      val mongoClient = org.mongodb.scala.MongoClient(connString)
      val database: org.mongodb.scala.MongoDatabase = mongoClient.getDatabase(dbMongo)
      val coll = database.getCollection(collectionMongo)
      coll.deleteMany(
        Document("_id" -> Document("$lt" -> LimiteTraccaituraSpid))
      )

    }catch {
      case e: Exception => {println(s"sono andato in errore ")}
    }
  }
