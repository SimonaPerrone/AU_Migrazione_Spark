...
import it.au.misure.ingestione.InjectionNew_NoCheckAmm.prop
        ...
        try {
				    ...
				    val noBom = if(rdd1._2._2.startsWith(UTF8_BOM)){
					    ...
					    val xml = XML.loadString(noBom)
							val datiPod = (xml \\ "FlussoMisure" \\ "DatiPod")
							...
							val dpRes = dpIt.flatMap { dp =>
    							...
    							val mese = meseAnno(0)
									...
									//passaggio ora solare/legale (attività che esegue solo a ottobre)
                ...
                val tipo_file = (xml \\ "FlussoMisure" \ "@CodFlusso").text
	...
	val commandLine:CommandLine = commonsCliUtils.parseArgsList(args, commandLineOptions.getOptions)
			...
			sqlCtx.setConf("hive.exec.dynamic.partition.mode","nonstrict")
			  ...
			  try{
						...
						val quartiPDO = sc.getConf.get("spark.flusso.misure.quarti")
					...
					rddUDDTmp.cache()
					  ...
					  val lists = rddUDDTmp.mapPartitions{partition => 
    					...
    					log.info(s"*** WholeTextFiles OK")
							...
							log.info("*** utilizzo metodo  validation " )
