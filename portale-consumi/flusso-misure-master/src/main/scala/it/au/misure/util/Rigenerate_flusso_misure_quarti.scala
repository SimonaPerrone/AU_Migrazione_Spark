...
object Rigenerate_flusso_misure_quarti
  ...
  var tbl_misurequarti_dest=s"${_dbDest}.flusso_misure_quarti"
  ...
  def main(args: Array[String]) {
    ...
    val nameApp = "Rigenerazione flusso_misure_quarti"
    ...
    tbl_misurequarti_dest=if(current_isilon==isilon_new_path) s"${_dbDest}.flusso_misure_quarti_new" else tbl_misurequarti_dest
    ...
    log.info(s"*** tabella quarti sorgente : ${tbl_misurequarti}" )
    ...
    val dati_schema =hiveCtx.sql(s"show create table ${tbl_misurequarti_dest}").collect()
    val l =dati_schema.length-1
    var location_quarti_dest=""
    var location_quarti_ext_dest=if(current_isilon==isilon_new_path) s"/user/hive/warehouse/au.db/misure_ee_au/flusso_misure_estensione_quarti_new" else "/user/silvia/au/misure_ee_au/flusso_misure_estensione_quarti_new"

    breakable {
      for (i <- l to 0 by -1) {
        val str = dati_schema(i).getString(0)
        if (str.contains(current_isilon)) {
          location_quarti_dest = str.replace("'", "").replace(current_isilon, "").trim
          break
        }
      }
    }
    log.info(s"*** Location tabella ${tbl_misurequarti_dest} : ${location_quarti_dest}")
    log.info(s"*** Location tabella ${_dbDest}.flusso_misure_estensione_quarti_new : ${location_quarti_ext_dest}")

    val location_ext_quarti_src=if(current_isilon==isilon_new_path) s"/user/hive/warehouse/au.db/misure_ee_au/flusso_misure_estensione_quarti" else "/user/silvia/au/misure_ee_au/flusso_misure_estensione_quarti"
    val location_quarti_src=if(current_isilon==isilon_new_path) s"/user/hive/warehouse/au.db/misure_ee_au/flusso_misure_quarti" else "/user/silvia/au/misure_ee_au/flusso_misure_quarti_bck_ok20190306"

    log.info(s"*** Location tabella quarti src : ${location_quarti_src}")
    log.info(s"*** Location tabella estensione quarti src  : ${location_ext_quarti_src}")
    ...
    for (an <- 2011 to anno.toInt ) {
     for(ms <- 1 to 12) {
       ...
       if(res > 0) {
         if (ms % 1 == 0) {
           mesi += s"${ms}"
           log.info(s"*** Avvio lettura e scrittura anno ${an} e mesi ${mesi} da ${tbl_misurequarti} a ${tbl_misurequarti_dest}")


           val pt=new Path(s"${location_quarti_src}/annoquarti=${an}/mesequarti=${mesi}/")
           if(hdfs.exists(pt)) {
             ...
             log.info(s"*** Num coalesce files tabella quarti : ${numCoalesceFiles}")

             val query =
               s"""
           SELECT distinct codcontrdispquarti,coducquarti,podquarti,pivautentequarti,pivadistributorequarti,areaquarti,
           tipodato_e,tipodato_s,tensione,trattamento_o,potcontrimpl,potdisp,cifreatt,cifrerea,
           CASE (split(nomefile,"_")[3]) WHEN 'RFO' then '' else raccolta end raccolta,
           validato,potmax,perdita,nomefile,annomesegiornodir,dataelaborazione,time_stamp,
           CASE WHEN (split(nomefile,"_")[3])='RFO' AND nvl(raccolta,'') <> '' then (split(raccolta,"_")[0]) else '' end motivazione,
           CASE WHEN (split(nomefile,"_")[3])='RFO' AND nvl(raccolta,'') <> '' then (split(raccolta,"_")[1]) else '' end tipo_rettifica,
           giornoquarti,(split(nomefile,"_")[3])tipo_flusso,
           CASE (split(nomefile,"_")[3]) WHEN 'PDO' then null when 'RFO' then null else
           CONCAT(annoquarti,mesequarti,nvl(cifrerea,''),dataelaborazione,podquarti,nomefile) end key_estensione_quarti,
           ...
           er86 ,er87 ,er88 ,er89 ,er90 ,er91 ,er92 ,er93 ,er94 ,er95 ,er96 ,er97 ,er98 ,er99 ,er100 ,annoquarti,mesequarti
           FROM ${tbl_misurequarti} where annoquarti=${an} and mesequarti IN(${mesi})
         """.stripMargin
           ...
           */
             ...
             dtTemp
               ...
               .save(location_quarti_dest)

             hiveCtx.sql(s"MSCK REPAIR TABLE ${tbl_misurequarti_dest}")

             val pt2 = new Path(s"${location_ext_quarti_src}/annoquarti=${an}/mesequarti=${mesi}/")
             if(hdfs.exists(pt2)) {
               val sizeDir2 = hdfs.getContentSummary(pt2).getSpaceConsumed()
               var numCoalesceFiles2 = (sizeDir2 / (blocksize * 1.0)).toInt
               if(numCoalesceFiles2==0)
                 numCoalesceFiles2=1
               else
                 numCoalesceFiles2 = numCoalesceFiles + 3

               log.info(s"*** Num coalesce files tabella estensione quarti : ${numCoalesceFiles2}")

               log.info("*** Avvio riscrittura tabella au.flusso_misure_estensioni_quarti")
               val query2 =
                 s"""
             SELECT distinct codcontrdispquarti ,podquarti ,pivautentequarti ,pivadistributorequarti ,areaquarti ,nomefile ,annomesegiornodir ,dataelaborazione ,time_stamp ,
             tipo_flusso ,tipodato_a ,ka ,kr ,kp ,data_misura ,tipo_rettifica ,data_rilevazione ,motivazione ,data_prest ,codprat_sii ,gruppomis ,forfait ,
             motivazione_stima ,data_inizio_periodo ,eaf1 ,eaf2 ,eaf3 ,eaf4 ,eaf5 ,eaf6 ,erf1 ,erf2 ,erf3 ,erf4 ,erf5 ,erf6 ,potf1 ,potf2 ,potf3 ,potf4 ,potf5 ,potf6 ,
             eam ,erm ,potm ,progr_podsez ,CONCAT(annoquarti,mesequarti,progr_podsez,dataelaborazione,podquarti,nomefile) key_estensione_quarti,annoquarti ,mesequarti
             FROM au.flusso_misure_estensione_quarti where annoquarti=${an} and mesequarti IN(${mesi})
           """.stripMargin

               val dtTemp2 =  hiveCtx.sql(query2).repartition(numCoalesceFiles2) 
               ...
               dtTemp2
                 ...
                 .mode(SaveMode.Append)
                 .partitionBy("annoquarti", "mesequarti")
                 .save(location_quarti_ext_dest)

               hiveCtx.sql(s"MSCK REPAIR TABLE au.flusso_misure_estensione_quarti_new")
             }

             log.info(s"*** Lettura e scrittura anno ${an} e mesi ${mesi} da ${tbl_misurequarti} a ${tbl_misurequarti_dest} completato")
