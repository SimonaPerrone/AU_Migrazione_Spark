...
object InjectionNew_NoCheckAmm extends LoggingSupport {
	...
	//mev21_066 tabelle ingestione enrgia reattiv acapacitiva/induttiva
	val tabFlussoQurtiErcEri = prop.getProperty("spark.app.ingestione.flussoQuartiErcEri")
	val tabFlussoNoAgrrErcEri = prop.getProperty("spark.app.ingestione.flussoNoAgrrErcEri")
	val tabFlussoSmisErcEri = prop.getProperty("spark.app.ingestione.flussoSmisErcEri")
	...
	val tbl_misurequarti:String =prop.getProperty("spark.app.flussoquarti_table")
	/**
	...
	def getRowsEaEr(elxml: scala.xml.NodeSeq,commonFields : List[Any] , mese :Int,Kr_Ka : Double,isEa: Boolean):Seq[(List[Any])] = {
	...
	def letturaEValidazioni_vecchioFormato(rdd1: (Boolean,(String,String),String,Boolean), dataelaborazione:java.sql.Timestamp , xml :scala.xml.Elem ): List[(Row)] = {
        ...
        */
        ...
        val tipo_rettifica = if(cod_flusso=="RFO")(dp \ "TipoRettifica").text else ""
        val motivazione =if(cod_flusso=="RFO") (dp \ "Motivazione").text else ""
        ...
        //val raccolta_tmp = if(cod_flusso=="RFO") motivazione+"_"+tipo_rettifica else raccolta
				...
				val commonFields = List(codContrDisp, coduc, pod, pivautente, pivadistributore, area, anno.toInt, mese.toInt,  //dataTime,
					...
					perditatens,nomefileT,annoMeseGiornoDir,dataelaborazione,timeStamp)
        ...
        if(sezCurva.length==0) {
          ...
          val row_non_oraria=(commonFields ++ List(null) ++ eaValues ++ erValues ++ List())
          Seq(Row.fromSeq(row_non_oraria))

        }else {
          val ea = (sezCurva \ "Ea")
          val itRes = getRowsEaEr(ea, commonFields, mese.toInt, 1, true)
	...
	//mev21_066 metodi preEstenti commentati inizio
...
//
//				val trattamento_o = (sezDatiPdp \ "Trattamento").text
//				val tensioneVista = (sezDatiPdp \ "Tensione").text.toDouble
//				val tensione = tensioneVista / 1000.0
//
//				// <Tensione>132000</Tensione> / 1000 = V
//				val perditatens = if (tensione == 0.0){
...
//					//fine campi nuovi
//
//					val keyrow= cod_flusso + "_" + pivadistributore + "_"+pivautente +"_" +codContrDisp +"_" + pod + "_" + mese_N +"_"+ anno_N + "_" + trattamento_o
//
//
...
//							potmax, perditatens, nomefileT, annoMeseGiornoDir, dataelaborazione, timeStamp
//						)
//
//						val ea = (sezMisura \ "Ea")
//						val er = (sezMisura \ "Er")
//
//						//per i PDO e RFO 1G non bisogna moltiplicare i valori della curva per ka e kr
//						val kax=if(cod_flusso=="PDO" || cod_flusso=="RFO")1 else ka
//						val krx=if(cod_flusso=="PDO" || cod_flusso=="RFO")1 else kr
//
//						val itRes = getRowsEaEr(ea, commonFields, mese_N, kax, true)
//
//						val irRes = getRowsEaEr(er, List(), mese_N, krx, false)
//
//
//						for ((a, r) <- (itRes zip irRes)) yield Row.fromSeq(a ++ r)
...
//							DataInizioPeriodo, eaf1, eaf2, eaf3, eaf4, eaf5, eaf6, erf1, erf2, erf3, erf4, erf5, erf6, potf1, potf2, potf3, potf4, potf5, potf6, EaM, ErM, PotM,tmp_index))
//
//						newTableFields
//					}
//					else {
...
//							validato, potmax, perditatens, nomefileT, annoMeseGiornoDir, dataelaborazione, timeStamp
//						)
//
//						val eaValues =List.fill(100)(null)
//						val erValues =List.fill(100)(null)
...
//		}
//	}
	//mev21_066 metodi preEstenti commentati fine
        ...
        val xml = XML.loadString(noBom)*/
			...
			val dpRes = dpIt.flatMap { dp =>
				...
				if( sezMisura_tmp.length > 0 || sezCurva.length >0 ){
					...
					// Campio Mev 21_066
					val ErcM = Try((sezMisura \ "ErcM").text.replace(",", ".").toDouble).getOrElse(null)
					val EriM = Try((sezMisura \ "EriM").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf1 = Try((sezMisura \ "ErcF1").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf2 = Try((sezMisura \ "ErcF2").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf3 = Try((sezMisura \ "ErcF3").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf4 = Try((sezMisura \ "ErcF4").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf5 = Try((sezMisura \ "ErcF5").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf6 = Try((sezMisura \ "ErcF6").text.replace(",", ".").toDouble).getOrElse(null)

					val erif1 = Try((sezMisura \ "EriF1").text.replace(",", ".").toDouble).getOrElse(null)
					val erif2 = Try((sezMisura \ "EriF2").text.replace(",", ".").toDouble).getOrElse(null)
					val erif3 = Try((sezMisura \ "EriF3").text.replace(",", ".").toDouble).getOrElse(null)
					val erif4 = Try((sezMisura \ "EriF4").text.replace(",", ".").toDouble).getOrElse(null)
					val erif5 = Try((sezMisura \ "EriF5").text.replace(",", ".").toDouble).getOrElse(null)
					val erif6 = Try((sezMisura \ "EriF6").text.replace(",", ".").toDouble).getOrElse(null)
					// fine campi mev 21_066
            ...
            val data_inst_mispot=null*/
					...
					//mev 21_066
					val curva2 = if(cod_flusso.trim().contains("PDO") || cod_flusso.trim().contains("RFO")) {
						val commonFields2 = List(codContrDisp, coduc, pod, pivautente, pivadistributore, area, anno_N, mese_N, //dataTime,
							cifreatt, cifrerea, nomefileT, annoMeseGiornoDir, dataelaborazione, timeStamp
						)

						val erc = (sezMisura \ "Erc")
						val eri = (sezMisura \ "Eri")

						//per i PDO e RFO 1G non bisogna moltiplicare i valori della curva per ka e kr
						val kax = if (cod_flusso == "PDO" || cod_flusso == "RFO") 1 else ka
						val krx = if (cod_flusso == "PDO" || cod_flusso == "RFO") 1 else kr

						val ercRes = getRowsEaEr(erc, commonFields2, mese_N, kax, true)
						val eriRes = getRowsEaEr(eri, List(ercf1,ercf2,ercf3,ercf4,ercf5,ercf6,erif1,erif2,erif3,erif4,erif5,erif6,ErcM,EriM), mese_N, krx, false)

						for ((a, r) <- (ercRes zip eriRes)) yield Row.fromSeq(a ++ r)
					}else //tracciato per flussi no PDO ed RFO (schema schemaflusso_noaggr)
					{
						val isnew_flusso=1
            ...
            val data_inst_mispot=null*/
						...
						val commonFields = List(anno_N,mese_N,cod_flusso,pivadistributore,codContrDisp,area,
							isnew_flusso, coduc, pod, pivautente,data_misura,data_inizio,data_voltura, cifreatt,
							cifrerea,cifrepot,data_rilevazione,data_prest,cod_prat_att,codprat_sii,nomefileT,
							annoMeseGiornoDir,dataelaborazione,timeStamp
						)

						val erc = (sezMisura \ "Erc")
						val eri = (sezMisura \ "Eri")
						val itRes = getRowsEaEr(erc, commonFields, mese_N, ka,true)
						val irRes = getRowsEaEr(eri, List(ercf1,ercf2,ercf3,ercf4,ercf5,ercf6,erif1,erif2,erif3,erif4,erif5,erif6,ErcM,EriM), mese_N, kr,false)
						if(itRes != Nil && irRes !=Nil) {
							for ((a, r) <- (itRes zip irRes)) yield Row.fromSeq(a ++ r)
						}else {

							val lista = List(ercf1,ercf2,ercf3,ercf4,ercf5,ercf6,erif1,erif2,erif3,erif4,erif5,erif6,ErcM,EriM)
							var flag = false
							for(l<-lista){
								flag =flag || l!=null
							}
							...
							if (flag == true){
								...
								val eaValues =List.fill(100)(null)
								val erValues =List.fill(100)(null)
								val row_non_oraria=(commonFields ++ List(null) ++ eaValues ++ erValues ++ lista)
								Seq(Row.fromSeq(row_non_oraria))
							}else{
					...
					curva ++ new_data ++ curva2 //mev21_066
          ...
          val potdisp =null*/
					...
					// Campio Mev 21_066
					val ErcM = Try((sezConsumo \ "ErcM").text.replace(",", ".").toDouble).getOrElse(null)
					val EriM = Try((sezConsumo \ "EriM").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf1 = Try((sezConsumo \ "ErcF1").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf2 = Try((sezConsumo \ "ErcF2").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf3 = Try((sezConsumo \ "ErcF3").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf4 = Try((sezConsumo \ "ErcF4").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf5 = Try((sezConsumo \ "ErcF5").text.replace(",", ".").toDouble).getOrElse(null)
					val ercf6 = Try((sezConsumo \ "ErcF6").text.replace(",", ".").toDouble).getOrElse(null)

					val erif1 = Try((sezConsumo \ "EriF1").text.replace(",", ".").toDouble).getOrElse(null)
					val erif2 = Try((sezConsumo \ "EriF2").text.replace(",", ".").toDouble).getOrElse(null)
					val erif3 = Try((sezConsumo \ "EriF3").text.replace(",", ".").toDouble).getOrElse(null)
					val erif4 = Try((sezConsumo \ "EriF4").text.replace(",", ".").toDouble).getOrElse(null)
					val erif5 = Try((sezConsumo \ "EriF5").text.replace(",", ".").toDouble).getOrElse(null)
					val erif6 = Try((sezConsumo \ "EriF6").text.replace(",", ".").toDouble).getOrElse(null)
					// fine campi mev 21_066
            ...
            val data_inst_mispot=null*/
					...
					//inizio mev21_066
					val curva_a_consumo2 = if(cod_flusso.trim().contains("PDO") || cod_flusso.trim().contains("RFO")) {
						//(schema schemaQuarti)
						...
						val row_non_oraria=(commonFields ++ List(ercf1,ercf2,ercf3,ercf4,ercf5,ercf6,erif1,erif2,erif3,erif4,erif5,erif6,ErcM,EriM) ++ eaValues ++ erValues ++ List())
						Seq(Row.fromSeq(row_non_oraria))
            ...
            val data_inst_mispot=null*/
					...
					curva_a_consumo ++ new_data ++ curva_a_consumo2 //mev21_066
	...
	def letturaEValidazioni_FlussoSMIS(rdd1: (Boolean,(String,String),String,Boolean), dataelaborazione:java.sql.Timestamp,xml:scala.xml.Elem ): List[(Row)] = {
		...
		try {
			...
			val dpRes = dpIt.flatMap { dp =>
				...
				val PotM = Try((sezDatiSmontaggio \ "PotM").text.replace(",", ".").toDouble).getOrElse(null)
				//mev21_066
				val ErcM = Try((sezDatiSmontaggio \ "ErcM").text.replace(",", ".").toDouble).getOrElse(null)
				val EriM = Try((sezDatiSmontaggio \ "EriM").text.replace(",", ".").toDouble).getOrElse(null)

				val ercf1 = Try((sezDatiSmontaggio \ "ErcF1").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf2 = Try((sezDatiSmontaggio \ "ErcF2").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf3 = Try((sezDatiSmontaggio \ "ErcF3").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf4 = Try((sezDatiSmontaggio \ "ErcF4").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf5 = Try((sezDatiSmontaggio \ "ErcF5").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf6 = Try((sezDatiSmontaggio \ "ErcF6").text.replace(",", ".").toDouble).getOrElse(null)

				val erif1 = Try((sezDatiSmontaggio \ "EriF1").text.replace(",", ".").toDouble).getOrElse(null)
				val erif2 = Try((sezDatiSmontaggio \ "EriF2").text.replace(",", ".").toDouble).getOrElse(null)
				val erif3 = Try((sezDatiSmontaggio \ "EriF3").text.replace(",", ".").toDouble).getOrElse(null)
				val erif4 = Try((sezDatiSmontaggio \ "EriF4").text.replace(",", ".").toDouble).getOrElse(null)
				val erif5 = Try((sezDatiSmontaggio \ "EriF5").text.replace(",", ".").toDouble).getOrElse(null)
				val erif6 = Try((sezDatiSmontaggio \ "EriF6").text.replace(",", ".").toDouble).getOrElse(null)
				//fine mev
				//fine campi smontaggio

				// SEZIONE DatiPod -> Smontaggio
				val fieldsSmontaggio=if(sezDatiSmontaggio.length >0){

					List(List(tipo_misuratore_sm,data_misura_sm,tipo_dato,
						eaf1,eaf2,eaf3,eaf4,eaf5,eaf6,
						erf1,erf2,erf3,erf4,erf5,erf6,
						potf1,potf2,potf3,potf4,potf5,potf6,
						EaM,ErM,PotM))
				...
				//mev21_066
				val fieldsSmontaggioErcEri=if(sezDatiSmontaggio.length >0){

					List(List(
						ercf1,ercf2,ercf3,ercf4,ercf5,ercf6,
						erif1,erif2,erif3,erif4,erif5,erif6,
						ErcM,EriM))
				...
				val PotM_mn = Try((sezDatiMontaggio \ "PotM").text.replace(",", ".").toDouble).getOrElse(null)
				//montaggio mev21_066
				val ErcM_mn = Try((sezDatiMontaggio \ "ErcM").text.replace(",", ".").toDouble).getOrElse(null)
				val EriM_mn = Try((sezDatiMontaggio \ "EriM").text.replace(",", ".").toDouble).getOrElse(null)

				val ercf1_mn = Try((sezDatiMontaggio \ "ErcF1").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf2_mn = Try((sezDatiMontaggio \ "ErcF2").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf3_mn = Try((sezDatiMontaggio \ "ErcF3").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf4_mn = Try((sezDatiMontaggio \ "ErcF4").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf5_mn = Try((sezDatiMontaggio \ "ErcF5").text.replace(",", ".").toDouble).getOrElse(null)
				val ercf6_mn = Try((sezDatiMontaggio \ "ErcF6").text.replace(",", ".").toDouble).getOrElse(null)

				val erif1_mn = Try((sezDatiMontaggio \ "EriF1").text.replace(",", ".").toDouble).getOrElse(null)
				val erif2_mn = Try((sezDatiMontaggio \ "EriF2").text.replace(",", ".").toDouble).getOrElse(null)
				val erif3_mn = Try((sezDatiMontaggio \ "EriF3").text.replace(",", ".").toDouble).getOrElse(null)
				val erif4_mn = Try((sezDatiMontaggio \ "EriF4").text.replace(",", ".").toDouble).getOrElse(null)
				val erif5_mn = Try((sezDatiMontaggio \ "EriF5").text.replace(",", ".").toDouble).getOrElse(null)
				val erif6_mn = Try((sezDatiMontaggio \ "EriF6").text.replace(",", ".").toDouble).getOrElse(null)
				//fine mev
				//fine campi montaggio

				val fieldsMontaggio=if(sezDatiMontaggio.length >0){

					List(List(tipo_misuratore_mn,data_misura_mn,data_messa_regime_mn,
						tensione_vista,perditatens_mn,ka_mn,kr_mn,kp_mn,
						matr_att_mn,matr_rea_mn,matr_pot_mn,cifre_att_mn,cifre_rea_mn,cifre_pot_mn,
						eaf1_mn,eaf2_mn,eaf3_mn,eaf4_mn,eaf5_mn,eaf6_mn,
						erf1_mn,erf2_mn,erf3_mn,erf4_mn,erf5_mn,erf6_mn,
				...
				//mev21_066
				val fieldsMontaggioErcEri=if(sezDatiMontaggio.length >0){

					List(List(
						ercf1_mn,ercf2_mn,ercf3_mn,ercf4_mn,ercf5_mn,ercf6_mn,
						erif1_mn,erif2_mn,erif3_mn,erif4_mn,erif5_mn,erif6_mn,
						ErcM_mn,EriM_mn))
				...
				var flag_montaggio = false
				var flag_smontaggio = false
				for (f <- fieldsMontaggioErcEri){
					for(i<-f){
						flag_montaggio= flag_montaggio || i!=null
					}
				}

				for (f <- fieldsSmontaggioErcEri){
					for(i<-f) {
						flag_smontaggio = flag_smontaggio || i != null
					}
				}
				...
				//log.info("SMIS KEYROW : " + keyrow)
				val fields=List(codContrDisp,pod,pivautente,pivadistributore,anno,mese,nomefileT,annoMeseGiornoDir,dataelaborazione,timeStamp,motivazione)
				val fields2=List(codContrDisp,pod,pivautente,pivadistributore,anno,mese,nomefileT,annoMeseGiornoDir,dataelaborazione,timeStamp)
				var field_seq = for ((a, r) <- (fieldsSmontaggio zip fieldsMontaggio)) yield Row.fromSeq(fields ++ a++ r) //mev21_066
				var field_seqErcEri =if(flag_smontaggio == true || flag_montaggio == true){
					for ((a, r) <- (fieldsSmontaggioErcEri zip fieldsMontaggioErcEri)) yield Row.fromSeq( fields2 ++ a++ r) //mev21_066
				} else{
					Nil
				}
				//var field_seq= Seq(Row.fromSeq(fields ++ fieldsSmontaggio ++ fieldsMontaggio))
				field_seq ++ field_seqErcEri //mev21_066
				//field_seq
			}
	...
	def letturaEValidazioni(rdd1: (Boolean,(String,String),String,Boolean), dataelaborazione:java.sql.Timestamp ): List[(Row)] = {
		...
		val cod_flusso = (sezflusso \ "@CodFlusso").text
    val isnewtrack=isNuovoFlusso(xml,cod_flusso,true)//rdd1._4

		val ret =if(cod_flusso.trim().contains("SMIS") ){
			println("SMIS fllusso")
			letturaEValidazioni_FlussoSMIS(rdd1, dataelaborazione,xml)
		}else if(isnewtrack){
			println("nuovo fllusso")
	...
	/**
		...
		*/
	def isNuovoFlusso(xmldata: scala.xml.Elem ,codflusso:String,force1G:Boolean=false) : Boolean = {
 ...
 */
	def tryToValidation(rdd_input: (String,String),tipo_flusso:String) : (Boolean,(String,String),String,Boolean) = {
		log.info("mev21_066 asono entrato tryToValidation" )
		if(verbose=="true")
		...
		log.info("Mev21_066 sono dentro il tryToValidation")
		log.info("Mev21_066 noBomm: " +noBom )
		if(verbose=="true")
      ...
      val isNewFlusso=isNuovoFlusso(xml,cod_flusso)
      ...
      val rdd = validationXSD(rdd_input,tipo_flusso,cod_flusso,isNewFlusso)
			...
			if (rdd._1) {
				try {
					...
					/*val dpRes2 = if ((datiPod \ "MeseAnno").length > 0) {
          ...
          //CONTROLLO SULLA MISURA FUTURA AL MOMENTO DISATTIVATO
					//val misura_futura=if(cod_flusso.toUpperCase =="SMIS" || cod_flusso.toUpperCase =="SNS" || cod_flusso.toUpperCase =="SOS") false else { if(annomese_cur < dataNf.toInt ) true else false}
          val misura_futura=false

					/*val misura_under_60mesi=if(cod_flusso1G=="RFO" || cod_flusso1G=="PDO")
					{
          ...
          val AM_NOTOK=checkMisure._1
					...
					// mev 21-030
					// controllo nuova variabile per abilitare o disattivare i controlli seguenti
					if (isCheckAmmiOn.equals("true")) {
						isValid2 = if (PIvaDistributoreNF != PIvaDistributoreTX) {
						...
						} else if (AM_NOTOK) { //else if (dpRes2.length != 0) {
							//mev 21-030
							...
							if (cod_flusso1G == "RFO") (false, "RFO_UNDER60MESI") else (false, "PDO_UNDER60MESI")
						} else {
							(true, "OK")
						}
					}
  ...
  def validationXSD(rdd: (String,String) ,tipo_flusso:String,cod_flusso:String,isNewFlusso:Boolean) : (Boolean,(String,String),String,Boolean) =
  {
    ...
    try{
      ...
      //val isNewFlusso=isNuovoFlusso(xmldata)
        ...
        case e: Exception =>
          ...
          if(err.contains("PIVAType")){
            ...
            if(!isNewFlusso && !(xsdProp_old == null) && (cod_flusso.contains("PDO") || cod_flusso.contains("RFO")))
              ...
              return (false, (rdd._1, e.getMessage().replace("'","")),"001",isNewFlusso)
          }
      }
      //Row(f._3, f._2._1, f._2._2, dataelaborazione, (argsObjMaster.anno + argsObjMaster.mese).toInt))
      ...
      case e: Exception =>
        ...
        if(!isNewFlusso && !(xsdProp_old == null) && (cod_flusso.contains("PDO") || cod_flusso.contains("RFO")))
          ...
          return (false, (rdd._1, e.getMessage().replace("'","") + " - err1 " ),"001",isNewFlusso)
    }
  ...
  def validationXSD_old(rdd: (String,String) ,tipo_flusso:String,cod_flusso:String,isNewFlusso:Boolean) : (Boolean,(String,String),String,Boolean) =
  {
    ...
    try{
      ...
      case e: Exception =>
        if(verbose=="true")
          e.printStackTrace()
        (false, (rdd._1, e.getMessage().replace("'","") + " - err1 " ),"001",isNewFlusso)
    }
	...
	def main(args: Array[String]) {
		...
		val nameApp= if(argsObjMaster.appName.contains("Decompressione ed ingestione")){
			log.info("***** Fase di ingestione *****")
		  argsObjMaster.appName +" - Fase di ingestione "
		}else{
    ...
    log.info("***** xsd.properties_vecchio presente : " + (!(xsdProp_old==null)))
    ...
    log.info("*** rootDir: " + rootDir)
		...
		try {
			...
			val quartiPDO_RFO = sc.getConf.get("spark.flusso.misure.quarti")
			val flusso_EXT = sc.getConf.get("spark.flusso.misure.ext")
			val flusso_NoAggr = sc.getConf.get("spark.flusso.misure.noaggr")
			val flusso_SMIS = sc.getConf.get("spark.flusso.misure.smis")

			val report = sc.getConf.get("spark.flusso.misure.report")
			val slash = sc.getConf.get("spark.flusso.misure.slash")

			println("---------------------------Tabelle-----------------------------")
			println(report)
			println(quartiPDO_RFO)
			println(flusso_EXT)
			println(flusso_NoAggr)
			println(flusso_SMIS)
			println(tabFlussoQurtiErcEri)
			println(tabFlussoNoAgrrErcEri)
			println(tabFlussoSmisErcEri)
			println("---------------------------fine----------------------------------")
      ...
      */
			val threadReports = if( (!(rdd2.partitions.isEmpty)) &&  (testlevel=="1" || testlevel=="0" || testlevel==null)) {
				...
				val rdd2_report = rdd2.map(f => {
					if (f._1) {
           // numf_validi.value = numf_validi.value + 1
            Row("000", f._2._1, "OK", dataelaborazione, (argsObjMaster.anno + argsObjMaster.mese).toInt)
          }
					else{
           // numf_invalidi.value=numf_invalidi.value + 1
						Row(f._3, f._2._1, f._2._2, dataelaborazione, (argsObjMaster.anno + argsObjMaster.mese).toInt)
          }
				})
            ...
            val dfQS3 = sqlCtx.createDataFrame(rdd2_report, schemaReport)
			...
			log.info("***** Sto per iniziare lettura e validazioni")
		...
		//	rddUDDTmp.unpersist()
			...
			//mev21_066
			var flusso_quarti_erc_eri =  false
			var flusso_noaggr_Erc_Eri = false
			var flusso_smis_erc_eri = false
			...
			if (cc_data  && (tipo_flusso==TypeDataToElab.Pdo.toString || tipo_flusso==TypeDataToElab.Rfo.toString) && (testlevel=="1" || testlevel=="0" || testlevel==null)) {
				...
				val vflusso_quarti = validData.filter(f => f.length == schemaQuarti.length)


				val thread1 = new Thread {
					override def run {

						val vflusso_ext = validData.filter(f => f.length == schemaflussoExt.length)
            ...
            vflusso_ext.foreachPartition(f=> if(f.length > 0)numPartPiene.add(1))
						...
						if ((numPartPiene.value>0) && (testlevel == "1" || testlevel == "0" || testlevel == null)) {
							...
							val dfQS11 = sqlCtx.createDataFrame(vflusso_ext, schemaflussoExt)
				...
				val thread2 = new Thread {
					override def run {
						...
						val vflusso_quartiErcEri = validData.filter(f => f.length == schemaQuartiErcEri.length)

						val numPartPiene: org.apache.spark.Accumulator[Int] = sqlCtx.sparkContext.accumulator(0, "Partitioni_piene")
						vflusso_quartiErcEri.foreachPartition(f=> if(f.length > 0)numPartPiene.add(1))

						if ((numPartPiene.value>0) && (testlevel == "1" || testlevel == "0" || testlevel == null)) {


							val dfQS12 = sqlCtx.createDataFrame(vflusso_quartiErcEri, schemaQuartiErcEri)
							//bilanciamento partizioni di scrittura
							...
							log.info("***** creazione DataFrame schema quarti Erc Eri OK")
							...
							if (testlevel == "0" || testlevel == null) {
								...
								dfQS12
									...
									.partitionBy("annoquarti", "mesequarti", "pivadistributorequarti", "codcontrdispquarti", "areaquarti")
									.save(tabFlussoQurtiErcEri)
								flusso_quarti_erc_eri = true
          ...
          if(!vflusso_quarti.partitions.isEmpty) {
            val dfQS1 = sqlCtx.createDataFrame(vflusso_quarti, schemaQuarti)
            ...
            log.info("***** insert misure quarti OK")
				...
				if(flusso_quarti_ext && flusso_quarti_erc_eri)//mev21_066
          ...
          val vflusso_noaggr = validData.filter(f => f.length == schemaflusso_noaggr.length)
					...
					if ((testlevel == "0" || testlevel == null)) {
						...
						val thread3 = new Thread {
							override def run {
								...
								val vflusso_noaggr_erc_eri = validData.filter(f => f.length == schemaflusso_noaggrErcEri.length)

								val numPartPiene: org.apache.spark.Accumulator[Int] = sqlCtx.sparkContext.accumulator(0, "Partitioni_piene")
								vflusso_noaggr_erc_eri.foreachPartition(f=> if(f.length > 0)numPartPiene.add(1))

								if ((numPartPiene.value>0) && (testlevel == "1" || testlevel == "0" || testlevel == null)) {


									val dfQS22 = sqlCtx.createDataFrame(vflusso_noaggr_erc_eri, schemaflusso_noaggrErcEri)
									//bilanciamento partizioni di scrittura
                  ...
                  val dfQS11x=dftmp11.drop("codpod")*/
									...
									log.info("***** creazione DataFrame schema noaggr erc eri OK")
									...
									if (testlevel == "0" || testlevel == null) {
										...
										//	.partitionBy("annoquarti", "mesequarti", "pivadistributorequarti", "codcontrdispquarti", "areaquarti")
											...
											.save(tabFlussoNoAgrrErcEri)//tabella nonagr ErcEri
										flusso_noaggr_Erc_Eri = true
            ...
            if (!vflusso_noaggr.partitions.isEmpty) {
              ...
              //dfQS2x
                ...
                //.partitionBy("anno", "mese", "pivadistributore", "codcontrdisp", "tipo_flusso", "area")
                .partitionBy("anno", "mese", "tipo_flusso")
                .save(flusso_NoAggr)
            ...
            log.info("***** insert flusso dati da non aggregare OK : " + flusso_noaggr +" "+ flusso_noaggr_Erc_Eri)//mev21_066
            ...
            val vflusso_smis = validData.filter(f => f.length == schemaflussoSMIS.length)
						...
						val thread4 = new Thread {
							override def run {
								...
								val vflusso_smis_erc_eri = validData.filter(f => f.length == schemaflussoSMISErcEri.length)

								val numPartPiene: org.apache.spark.Accumulator[Int] = sqlCtx.sparkContext.accumulator(0, "Partitioni_piene")
								vflusso_smis_erc_eri.foreachPartition(f=> if(f.length > 0)numPartPiene.add(1))

								if ((numPartPiene.value>0) && (testlevel == "1" || testlevel == "0" || testlevel == null)) {


									val dfQS23 = sqlCtx.createDataFrame(vflusso_smis_erc_eri, schemaflussoSMISErcEri)
									//bilanciamento partizioni di scrittura
                  ...
                  val dfQS11x=dftmp11.drop("codpod")*/
									...
									log.info("***** creazione DataFrame schema  smis erc eri  OK")
									...
									if (testlevel == "0" || testlevel == null) {
										...
										dfQS23
											...
											.save(tabFlussoSmisErcEri)//tabella smis ErcEri
										flusso_smis_erc_eri = true
            ...
            if (!vflusso_smis.partitions.isEmpty) {
             ...
             /* val dftmp21 =dfQS21.select(col("*"), substring(col("pod"), 6, 2).as("codpod")).
              ...
              is_flusso_smis = true
						...
						if(flusso_smis_erc_eri) log.info("***** insert dati per flusso SMIS OK")
      ...
      */
        ...
        log.info("***** Avvio aggiornamento partizioni ")
				...
				if (flusso_quarti_ext) {
					...
					sqlCtx.sql("MSCK REPAIR TABLE " + db + ".flusso_misure_estensione_quarti")
				}
				...
				//mev21_066
				if(flusso_quarti_erc_eri)
					sqlCtx.sql("MSCK REPAIR TABLE " + db + ".flusso_misure_quarti_erc_eri")

				if(flusso_noaggr_Erc_Eri)
					sqlCtx.sql("MSCK REPAIR TABLE " + db + ".flusso_noagrr_erc_eri")

				if(flusso_smis_erc_eri)
					sqlCtx.sql("MSCK REPAIR TABLE " + db + ".flusso_smis_erc_eri")
