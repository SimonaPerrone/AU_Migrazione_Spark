...
with open(pathConfig, 'r') as conf_file:
...
def open_connection_mongo():
    client = MongoClient(config["uri"], readPreference='primaryPreferred')
    client.get_database('Forniture', read_preference=ReadPreference.PRIMARY_PREFERRED)
    return client

def checkCollection(mongodb,collection_base,collection_detail):
    if(collection_detail==""):
       collection_detail="nocollection"
    
    collection = mongodb[collection_detail]
    if(collection.count() == 0):
       client = connect_impala(HOST, PORT, USERNAME, PASSWORD)
       query="select max(timeid) timeid from misure.collection_writted where collectionbase='"+collection_base+"'"
       df = df_from_db(query, client)
       last_timeid=df['timeid'][0]
       if(last_timeid == None):
          last_timeid="0"
       query="select collectiondetail from misure.collection_writted where collectionbase='"+collection_base+"' and timeid = '"+last_timeid+"'"
       df = df_from_db(query, client)
       coll_detail_daimpala=""
       if('collectiondetail' in df and df['collectiondetail'].count() > 0):
          coll_detail_daimpala = df['collectiondetail'][0]
       else: 
          coll_detail_daimpala=collection_detail
          
       #print(coll_detail_daimpala)
       if(coll_detail_daimpala == None):
          coll_detail_daimpala=collection_detail
       collection = mongodb[coll_detail_daimpala]
       if(collection.count() > 0):
          print("Collection da utilizzare per la collection_base : " + collection_base + " = " + coll_detail_daimpala)
          return coll_detail_daimpala
       else:
          print("Non riesco a trovare la collection da utilizzare per la collection : " + collection_base)
          return collection_detail
    else:
        return collection_detail


def getCollectionMongo(collectionbase):
    clientmongo = open_connection_mongo()
    db = clientmongo[config["database"]]
    collections = []
    if("Collection"+collectionbase not in config):
        print("Non è stato possibile trovare una chiave di dizionario Collection"+collectionbase+ " nella configurazione preimpostata")  
        return collections
    collconfig_detail=config["Collection"+collectionbase]
    index=-1
    
    for col in collconfig_detail:
        index=index+1
        for kk in col:
           coll_detail=checkCollection(db,kk,col[kk])
           config["Collection"+collectionbase][index][kk]=coll_detail
           collections.append(coll_detail)
...
convert_fields_elettrico = [
    ...
    "potenza_max_erogata","potf1","potf2","potf3","potfm"
]
...
def AddAuditFornitureElettriche(codice_fiscale, tipo, spid_code, esito, parametri=None):
    return #scrittura al momento disabilitata
    global config
    ...
    collection = db[getCollectionMongo("AuditElettrico")]
...
def AddAuditFornitureGas(codice_fiscale, tipo, spid_code, esito, parametri=None):
    ...
    collection = db[getCollectionMongo("AuditGas")]
    update_or_insert(codice_fiscale, collection, new_audit)
...
def get_consumi_quartorari():
    ...
    # codice sopra commentato il 2022/03/18 

    if  codice_pod is None:
        info = "Codice pod:{} non valorizzato".format( codice_pod)
    ...
    app.logger.info("Received - GetConsumiQuartorari args:{}, "
                    "codice_pod:{}".format(args, codice_pod))
...
def _getFornitureElettriche(codice_fiscale, spid_code):
    ...
    collection = db[getCollectionMongo("FornitureElettriche")]
    query = {"codice_fiscale": codice_fiscale}
...
def GetAuditFornitureElettriche(codice_fiscale, spid_code):
    ...
    #collection = db[config["CollectionAuditElettrico"]]
    # collection = db[getCollectionMongo("AuditElettrico")]
    collection = db[config["CollectionAuditElettricoLog"]]
    query = {"codice_fiscale": codice_fiscale}
    projection = {"audit": 1}
    doc_ret = {}
    cf_exists = False
...
def GetMisureFornitureElettriche():
    ...
    for col in getCollectionMongo("MisureElettriche"):
...
def _getFornitureGas(codice_fiscale, spid_code):
    ...
    collection = db[getCollectionMongo("FornitureGas")]
    query = {"codice_fiscale": codice_fiscale}
...
def GetAuditFornitureGas(codice_fiscale, spid_code):
    ...
    #collection = db[config["CollectionAuditGas"]]
    #collection = db[getCollectionMongo("AuditGas")]
    collection = db[config["CollectionAuditGasLog"]]
    query = {"codice_fiscale": codice_fiscale}
    projection = {"audit": 1}
    doc_ret = {}
    cf_exists = False
...
def GetMisureFornitureGas():
    ...
    for col in getCollectionMongo("MisureGas"):
...
def login():
    ...
    codice_fiscale = request.args.get('codice_fiscale')
    spid_code = request.args.get('spid_code')
    # ottenimento parametri nuovi dalla request, company_fiscal_number e iva_code
    # company_fiscal_number
    cfn = request.args.get('company_fiscal_number')
    # iva_code
    piva = request.args.get('iva_code')
    #trim dei due parametri 
    if cfn is not None:
        cfn=cfn.strip()
    if piva is not None:
        piva=piva.strip()
    # chiamata al metodo per l'audit di login
    AddAuditLogin(codice_fiscale, 'login', spid_code,cfn, piva)
    return jsonify({})

#nuovo metodo per gestire la registrazione dell'audit dei login
def AddAuditLogin(codice_fiscale, tipo, spid_code, cfn=None,  piva=None):
    ...
    info = {}
    # i due parametri CFN e PIVA viaggiano a coppia, controllo solamnete il primo CFN
    if (piva is None ):
        piva= ""
    if (cfn is not None ) and  (len(cfn)>0):
        info = 'CF: ' + codice_fiscale + ', CFN: ' + cfn + ', PIVA: ' + piva
    else:
        info = 'CF: ' + codice_fiscale 
    new_audit = {'tipo': tipo, 'data': my_date.strftime('%Y-%m-%dT%H:%M:%S.%f%z'), 'spid_code': spid_code, 'info': info}
    # client = MongoClient(config["uri"])
    client = open_connection_mongo()
    db = client[config["database"]]
...
@app.route('/api/AddTracciaturaSPID',methods = ['POST'])
@crossdomain(origin='*')
def TracciaturaAccessiSPID():
    richiesta=  {
        "request_id": "",
        "idp_entity_id":"",
        "timestamp_phase":"",
    ...
    try:
        ...
        if(request_data.keys()!= richiesta.keys()):
            response = make_response(
                jsonify(
                    {"esito":"KO, Status 400","description_error": "Errore nella Request, oggetto JSON non conforme","payload":request_data}
                ),
                400,
            )
            return response
        data_request = request_data["timestamp_phase"]
        data_id = data_request[:10].replace("/","")
        ora_auth = data_request[11:13]
        time_request = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
        request_data.update({"Time_authentication":ora_auth})
        request_data.update({"Time_request":time_request})
        mappa={"_id":data_id,
               "autenticazione":[request_data]}
        client=open_connection_mongo()
        db = client[config["database"]]
        col =db[config["TracciatureSPID"]]
        query ={"_id":data_id}
        if(col.count(query)>0):
            col.update({"_id":data_id},{"$push":{"autenticazione":request_data}})
        else:
            col.insert_one(mappa)
        response = make_response(
                jsonify(
                    {"esito":"OK, Status 201","description_error": "","payload":request_data}
                ),
                201,
            )
        response.headers["Content-Type"] = "application/json"
        ...
        response = make_response(
                jsonify(
                    {"esito":"KO, Status 400","description_error": str(sys.exc_info()[1]),"payload":request_data}
                ),
                400,
            )
        response.headers["Content-Type"] = "application/json"
