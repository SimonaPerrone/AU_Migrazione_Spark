...
CAST(CASE WHEN d_inizio='' THEN concat( year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0),'01') ELSE CAST(CONCAT(SUBSTR(d_inizio,1,4),SUBSTR(d_inizio,6,2),SUBSTR(d_inizio,9,2)) AS INT) END AS BIGINT)inizio,
...
CAST(concat( year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0),'01') as BIGINT) d_inizio_start
from 
(
 select n_id_fornitura,
 ...
 inner join rcu.rcu_pod_stato_p stato_pods on stato_pods.n_id_pod = pods.n_id_pod
 WHERE T_STATO_ATTIVAZIONE ='A' OR NVL(T_STATO_ATTIVAZIONE,'')='' and isNumeric(n_id_fornitura) = true
 UNION ALL
 select n_id_fornitura,
 nvl(d_inizio_titolarita,'') as d_inizio,
 nvl(d_fine_titolarita,'') as d_fine,
...
where  fine >= cast(concat(year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0),'01') as bigint)  and (inizio+2) < fine
) AS TTX
...
SELECT     DISTINCT 
            ...
            year(date_add(cast(rcu_pod_tecn_out.d_oper_misurator_att as timestamp),395)) anno_start_misure_orarie,
            month(date_add(cast(rcu_pod_tecn_out.d_oper_misurator_att as timestamp),395)) mese_start_misure_orarie
        FROM (
            SELECT
...
CAST(CASE WHEN d_inizio_validita='' THEN concat( year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0),'01') ELSE CAST(CONCAT(SUBSTR(d_inizio_validita,1,4),SUBSTR(d_inizio_validita,6,2),SUBSTR(d_inizio_validita,9,2)) AS INT) END AS BIGINT)d_inizio_validita,
...
SELECT DISTINCT
    nvl(rcu_clientefinale.t_cf,'')t_cf,
...
INSERT INTO misure.prt_tmo_mn_f PARTITION(annomese) 
                     ...
                     where  CAST(SUBSTR(regexp_replace(datamisura,'-',''),1,6) AS INT) >=cast(concat(year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0)) as INT)
...
INSERT INTO misure.prt_tmo_mv_f PARTITION(annomese) 
                     ...
                     where  CAST(SUBSTR(regexp_replace(datavoltura,'-',''),1,6) AS INT) >=cast(concat(year(date_sub(current_date,365)),lpad(month(date_sub(current_date,365)),2,0)) as INT) 
