...
--  from(select 1 idrow ,current_date()EndDate , date_sub(current_date,125)StartDate)  t
--lateral view 
...
tipo_lettura
from CMG_GAS.prt_cmg_tgl_p_v2 TGL
where tipo_lettura ='E' AND 
cast((date_format(from_unixtime(unix_timestamp(data_comp , 'dd/MM/yyyy')),'yyyyMM')) as int) >=  cast(date_format(date_sub(current_date(),125),'yyyyMM') as int) ;
