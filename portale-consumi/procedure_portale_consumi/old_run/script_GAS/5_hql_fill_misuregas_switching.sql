...
CREATE TEMPORARY TABLE forniture_gas_iniziali_senza_misure
as
...
ON CONCAT(FO.KK_KEY,FO.annomese_iniziof)=CONCAT(M_MSAF.cod_pdr,M_MSAF.n_id_fornitura,M_MSAF.competenza_consumi_af)
LEFT OUTER JOIN misuregas.misure_gas_portale_mensili M_MS
ON CONCAT(FO.KK_KEY,FO.annomese_iniziof)=CONCAT(M_MS.cod_pdr,M_MS.n_id_fornitura,M_MS.competenza_consumi_mmmm)
...
DROP TABLE forniture_gas_iniziali_senza_misure;

--MISURE ANTE SWITCHING 

CREATE TEMPORARY TABLE fornituregas_switching
as
