...
INSERT INTO misure.prt_tmo_mv_f PARTITION(annomese) 
                     ...
                     where  CAST(SUBSTR(regexp_replace(datavoltura,'-',''),1,6) AS INT) >=cast(concat(year(date_sub(current_date,${env:limit_gg})),lpad(month(date_sub(current_date,${env:limit_gg})),2,0)) as INT) 
