...
do_import () {
                ...
                && ( \
                        ...
                        ; sqoop import --connect  jdbc:oracle:thin:@scancl01-01.siiau.local:1521/SIIP --username pag_rcugas --password Psiiprdpag_rcugas5142f --table ${tabella} -m 30 --null-string "\\\\N" --null-non-string "\\\\N" --fields-terminated-by '\b' --hive-delims-replacement ' ' --target-dir /user/hive/warehouse/acquirente_unico/sqoop_portcons/n${tabella} --direct \
...
tabelle_schemi=( \
        ...
        rcus.rcus_udd "N_ID_SCHEDA STRING,N_ID_UDD STRING,T_CODICE_TERNA STRING,D_AGGIORNAMENTO STRING,D_ARCHIVIAZIONE STRING,N_ID_TRACCIA STRING,N_ID_S_PREC STRING,N_ID_S_SUCC STRING,B_VALIDO STRING,D_INIZIO STRING,D_FINE STRING,N_ID_AZIENDA_RIF STRING,T_TIPO STRING" \
        ...
        switch_gas.prt_vtg "N_ID_VTG string, N_ID_PRATICA string, T_STATO string, T_CODICE_PDR string, T_CODICE_REMI string, D_DATA_RICHIESTA string, D_DATA_DEC_RICHIESTA string, D_DATA_DEC string, T_TITOLO string, N_ID_NVG1 string, B_AMMISSIBILE string, T_COD_CAUSALE string, T_MOTIVO string, N_ID_UTENTE string, T_PIVA_UTENTE string, T_RUOLO_UTENTE string, T_CP_UTENTE string, N_ID_DISTR string, T_CP_DISTR string, N_ID_UDD_U string, N_ID_CC_U string, N_ID_UDD_RCU_E string, N_ID_CC_E string, T_RUOLO_CC_E string, T_PIVA_CC string, N_ID_CLIENTE_FINALE string, N_ID_FORNITURA string, N_ID_DISPACCIAMENTO string, N_ID_COD_CAUSALE_DISTR string, T_TIPOLOGIA_VOLTURA string, T_TIPO_FORNITURA string, T_CF_COD_FISC string, T_CF_PIVA string, B_CF_COD_FISC_ESTERO string, B_CF_PERSONA_FISICA string, T_ESITO string, T_COD_ESITO string, T_DETT_ESITO string, N_MAX_ORE_CONG string, D_DATA_STATO string, N_VERSIONE_ISTANZA string, T_PIVA_UDB string, D_DATA_DEC_RET string, T_PRESENZA_DS string" \
        ...
        rcugas.rcugas_udb "N_ID_UDB STRING,N_ID_AZIENDA STRING,T_TIPO_UDB STRING,D_DATA_INIZIO STRING,D_DATA_FINE STRING,T_NOTE STRING,D_AGGIORNAMENTO STRING,N_ID_TRACCIA STRING,N_ID_S_PREC STRING,D_DATA_RIF STRING" \
        rcugas.rcugas_pdr_stato "N_ID_PDR_STATO STRING,N_ID_PDR STRING,T_COD_STATO_PDR STRING,D_DATA_INIZIO STRING,D_DATA_FINE STRING,T_COD_CAUSALE_NO_DISAT STRING,D_AGGIORNAMENTO STRING,N_ID_TRACCIA STRING,N_ID_S_PREC STRING,B_PROCESSO_IN_CORSO STRING,T_COD_CAUSALE_NO_RIATT STRING,TIPO_DATA_INIZIO STRING,TIPO_DATA_FINE STRING,T_NOTE STRING,D_DATA_RIF STRING" \
        RCUGAS.RCUGAS_UDD "N_ID_UDD STRING,N_ID_AZIENDA STRING,D_DATA_INIZIO STRING,D_DATA_FINE STRING,T_NOTE STRING,D_AGGIORNAMENTO STRING,N_ID_TRACCIA STRING,N_ID_S_PREC STRING,T_COD_UDD STRING,D_DATA_RIF STRING" \
        rcugas.rcugas_var_prel_annuo "N_ID_VAR_PREL_ANNUO string, N_ID_PDR string, T_ANNO string, N_PRELIEVO_ANNUO string, D_DATA_INIZIO string, D_DATA_FINE string, T_NOTE string, D_AGGIORNAMENTO string, N_ID_TRACCIA string, N_ID_S_PREC string, D_DATA_RIF string" \
        rcugas.rcugas_var_trattamento "N_ID_VAR_TRATTAMENTO string, N_ID_PDR string, T_ANNO string, T_TRATTAMENTO_SETTLEMENT string, D_DATA_INIZIO string, D_DATA_FINE string, T_NOTE string, D_AGGIORNAMENTO string, N_ID_TRACCIA string, N_ID_S_PREC string, D_DATA_RIF string" \
        rcugas.rcugas_var_profilo "N_ID_VAR_PROFILO string, N_ID_PDR string, T_ANNO string, T_COD_PROFILO string, T_COD_CAT_USO string, T_COD_CLASSE_PRELIEVO string, D_DATA_INIZIO string, D_DATA_FINE string, T_NOTE string, D_AGGIORNAMENTO string, N_ID_TRACCIA string, N_ID_S_PREC string, D_DATA_RIF string, T_PRESENZA_DS string" \
)
...
# MEV22-006: Dismissione import da tabella RCUGAS.RCUGAS_CONNESSIONI_DISTR in Oracle
...
# MEV22-006: gestione di viste speciali: viste che hanno un nome diverso in Oracle dal nome della tabella Hive in cui verranno importate
# in questo caso ciascuna "riga" dell'array sotto (vistespeciali_schemi) e' composta da tre elementi:
# NOME_TABELLA_ORACLE_DA_IMPORTARE NOME_TABELLA_IN_HIVE_SENZA_SUFFISSO_P SCHEMA
# Esempio: passando rcugas.rcugas_connessioni_distr2 rcugas.rcugas_connessioni_distr "SCHEMA XXX"
# verra' importata la tabella oracle rcugas.rcugas_connessioni_distr2 nella tabella Hive rcugas.rcugas_connessioni_distr_p.
# Si e' reso necessario questo per gestire il passaggio dalla tabella rcugas.rcugas_connessioni_distr alla vista rcugas.rcugas_connessioni_distr2
 
# Inizio Mod MEV22-006
vistespeciali_schemi=( \
        rcugas.rcugas_connessioni_distr2 rcugas.rcugas_connessioni_distr "T_CODICE_PDR string, N_ID_PDR string, N_ID_REMI string, D_DATA_INIZIO_CONN string, D_DATA_FINE_CONN string, T_REMI string, N_ID_DISTR string, D_DATA_INIZIO_GESTECN string, D_DATA_FINE_GESTECN string, T_REMI_RCU string, ID_REGIONE_CLIMATICA string, T_PIVA_DISTR string" \
)


for ((i = 0; i < ${#vistespeciali_schemi[@]}; i++))
do
        if [ $((i%3)) -eq 0 ]
        then
		        vista_original=${vistespeciali_schemi[$i]}
                vista=${vistespeciali_schemi[$i+1]}
                schema=${vistespeciali_schemi[$i+2]}

                echo ""
                echo "$(date) Scarico vista ${vista_original^^} in corso..."

                echo "$(date) Scarico vista ${vista_original^^} in corso..." &>> "$F_LOG" \
                && ( \
                        hdfs dfs -rm -R -f -skipTrash /user/hive/warehouse/acquirente_unico/sqoop_portcons/n${vista}/ \
                        ; sqoop import --connect jdbc:oracle:thin:@scancl01-01.siiau.local:1521/SIIP --username pag_rcugas --password Psiiprdpag_rcugas5142f --table ${vista_original^^} -m 1 --null-string "\\\\N" --null-non-string "\\\\N" --fields-terminated-by '\b' --hive-delims-replacement ' ' --target-dir /user/hive/warehouse/acquirente_unico/sqoop_portcons/n${vista} --direct \
                ) &> esito_sqoop_${vista}.log \
                && ( \
                        ([ $(cat esito_sqoop_${vista}.log | grep "100%" | wc -l) -ge 1 ] && echo "$(date) Importazione dati vista ${vista_original^^} completata con successo." > esito_sqoop_${vista}.log ) \
                        || echo "$(date) ERRORE nell'importazione dei dati vista ${vista_original} : " &>> esito_sqoop_${vista}.log \
                ) \
                && ( \
                        rm -f ${vista}.hql \
                        && echo "DROP TABLE IF EXISTS ${vista}_new;" >> ${vista}.hql \
...
# Costruzione tabella RCUGAS.RCUGAS_PDR_DATIPRELIEVO
/mnt/isilonshare1/Software/EE/portale_consumi/sqoop/go_create_table_rcugas_datiprelievo.sh "$F_LOG"
# Fine costruzione tabella RCUGAS.RCUGAS_PDR_DATIPRELIEVO
