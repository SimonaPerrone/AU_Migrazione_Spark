...
LOG_FILE="$BASEPATH""/schedulazioni/log_crontab/log_ALTRIFLUSSI1G_$CURTMS"".txt"
...
if [ -n "$error" ]
then
 echo "Decompressione ALTRI FLUSSI 1G non andata a buon fine" &>> "$LOG_FILE"
 exit 1
fi
...
#"$BASEPATH"/last_release/bin/flusso-misure.sh -i -g -O &>> "$LOG_FILE"
