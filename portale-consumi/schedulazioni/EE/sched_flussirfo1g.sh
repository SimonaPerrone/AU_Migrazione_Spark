...
LOG_FILE="$BASEPATH""/schedulazioni/log_crontab/log_RFO1G_$CURTMS"".txt"
...
sudo ./flusso-misure.sh -D -g -T --anno "$ANNO" --mese "$MESE" --giorno "$GIORNO" &> "$LOG_FILE"
...
if [ -n "$error" ]
then
 echo "Decompressione FLUSSI RFO 1G non andata a buon fine" &>> "$LOG_FILE"
 exit 1
fi
