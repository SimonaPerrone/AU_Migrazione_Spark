...
import it.au.misure.ingestionMisureGasUnico.model.schema.IGMRSchema.annomese
...
import it.au.misure.ingestionMisureGasUnico.model.schema.ammissibilita.AmmissibilitaPDRSchema.{cartella_cloud, nome_file, codice_inamissibilita}
...
import it.au.misure.ingestionMisureGasUnico.validate.CheckAmmissibilitaPDRRulesIGMR
...
object IGMRFlow extends Flow {
  override val schema: SchemaEnum = IGMRSchema
  val exportSchema: SchemaEnum = IGMRExportSchema
  override val hiveTableName: String = "prt_cmg_igmr_p"
  val hiveExportTableName: String = "prt_cmg_igmr_export"
  val igmgHiveTableName: String = "prt_cmg_igmg_p"
  override val flowType: String = "IGMR"

  override def flowName: String = "IGMR"

  override val flowDataPath: String = s"$rootPath/$flowType/$flowName"
  override val partitioningColumns: List[String] = List(
    annomese
  )
  override val ammissPath: String = PropertyUtility.getAmmissibilitaIgmrPath
  ...
  def parseXml(gasMetadata: GasMetadata): List[Row] = {
    ...
    (xml \ DatiPdR).toList.map(pdrNode => {
      ...
      val motRettLett = (pdrNode \ mot_ret_lett).text
      ...
      val telegestione = FileUtility.extractNodeOrNull(pdrNode \ Post_int \ IGMRXMLSchema.telegestione)
  ...
  /*(implicit sc: SparkContext, sqlContext: SQLContext)*/ : (RDD[(GasXmlMetadata, List[ReportEsitoPDRMessage])], RDD[GasXmlMetadata]) = {
    val checkAmm = Environment.getSpark.sparkContext.broadcast(new CheckAmmissibilitaPDRRulesIGMR)
    ...
    val rcuGasPdr = Environment.getSpark.sqlContext.table(PropertyUtility.getRcugasPdrTable)
    val rcuGasPdrStato = Environment.getSpark.sqlContext.table(PropertyUtility.getRcugasPdrStatoTable)
  ...
  override def parse(inputRdd: RDD[GasXmlMetadata]): DataFrame = {
    ...
    Environment.getSpark.sqlContext.createDataFrame(dfRdd, IGMRParsedSchema.createSparkSchema())
  }
  ...
  def writeExport(df: DataFrame): Unit = {
    val exportDf = df
      .where(col(IGMRExportSchema.ammissibilita) =!= BLOCCANTE)
  ...
  def getPdrExtraMetadata(rddPdrMeta: RDD[(Node, GasXmlMetadata)], prtIgmg: DataFrame, rcuGasPdr: DataFrame, rcuGasPdrStato: DataFrame)
  /*(implicit sc: SparkContext, sqlContext: SQLContext)*/ : RDD[(Node, GasXmlMetadata)] = {
    ...
    val rcuGasPdrStatoRdd = rcuGasPdrStato
    ...
    val gasXmlMapped: RDD[((String, String), (Node, GasXmlMetadata))] = pdrNodeNotValidInRcu
      ...
      .map { case (node, metadata) =>
        val codPdr = (node \ cod_PdR).text
        val dataMisura = (node \ data_misura).text
        ((codPdr, dataMisura), (node, metadata))
      }

    //4.2 update igmgMatch value for igmr files
    val joinedRdd = gasXmlMapped
  ...
  override def writeAmmissibilitaReportsHive(xmlWithMessages: RDD[(GasXmlMetadata, List[ReportEsitoPDRMessage])], unzipTimestamp: String) /*(implicit sc: SparkContext, sqlContext: SQLContext)*/ : Unit = {
