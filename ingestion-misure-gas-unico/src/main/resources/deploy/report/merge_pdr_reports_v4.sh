...
#               "${destination_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt"
            sort -u ${base_folder}/${REPORT_FILE_IGMG_NAME}*.txt > "${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt" # si potrebbe togliere il -u
          else
            sort -u ${destination_folder}/${REPORT_FILE_IGMG_NAME}_*.txt ${base_folder}/${REPORT_FILE_IGMG_NAME}*.txt \
                    > "${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt"
          fi

          split -C 5MB --numeric-suffixes --additional-suffix=.txt ${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt ${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}_
#          rm "${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt"
          local tmp_reports_file_igmg_chunks=$(find "${tmp_folder}" -type f -name "${REPORT_FILE_IGMG_NAME}_${CURTMS}_*.txt" | wc -l)
          if [[ tmp_reports_file_igmg_chunks -eq 1 ]]; then
            cp -f "${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}_00.txt" "${destination_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}.txt"
          else
            cp -f ${tmp_folder}/${REPORT_FILE_IGMG_NAME}_${CURTMS}_*.txt "${destination_folder}"
          fi
      fi
...
#               "${destination_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt"
            sort -u ${base_folder}/${REPORT_FILE_STANDARD_NAME}*.txt > "${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt" # si potrebbe togliere il -u
          else
            sort -u ${destination_folder}/${REPORT_FILE_STANDARD_NAME}_*.txt ${base_folder}/${REPORT_FILE_STANDARD_NAME}*.txt \
                    > "${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt"
          fi

          split -C 5MB --numeric-suffixes --additional-suffix=.txt ${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt ${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}_
#          rm "${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt"
          local tmp_reports_file_std_chunks=$(find "${tmp_folder}" -type f -name "${REPORT_FILE_STANDARD_NAME}_${CURTMS}_*.txt" | wc -l)
          if [[ tmp_reports_file_std_chunks -eq 1 ]]; then
            cp -f "${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}_00.txt" "${destination_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}.txt"
          else
            cp -f ${tmp_folder}/${REPORT_FILE_STANDARD_NAME}_${CURTMS}_*.txt "${destination_folder}"
          fi
      fi
    fi
...
#              echo "${CSV_HEADER_PDR}" > "${destination_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt" # riaggiungo header tolto da sed
              sort -u "$tmp_folder/TMP_aggregazione_IGMG.txt" > "${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt" # si potrebbe togliere il -u
          else
              sort -u ${destination_folder}/${REPORT_PDR_IGMG_NAME}_*.txt "${tmp_folder}/TMP_aggregazione_IGMG.txt" \
                    > "${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt"
          fi

          split -C 5MB --numeric-suffixes --additional-suffix=.txt ${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt ${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}_
#          rm "${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt"
          local tmp_reports_pdr_igmg_chunks=$(find "${tmp_folder}" -type f -name "${REPORT_PDR_IGMG_NAME}_${CURTMS}_*.txt" | wc -l)
          if [[ tmp_reports_pdr_igmg_chunks -eq 1 ]]; then
            cp -f "${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}_00.txt" "${destination_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}.txt"
          else
            cp -f ${tmp_folder}/${REPORT_PDR_IGMG_NAME}_${CURTMS}_*.txt "${destination_folder}"
          fi
      fi
...
#              echo "${CSV_HEADER_PDR}" > "${destination_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt"  # riaggiungo header tolto da sed
              sort -u "$tmp_folder/TMP_Aggregazione_STD.txt" > "${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt" # si potrebbe togliere il -u
          else
              sort -u ${destination_folder}/${REPORT_PDR_STANDARD_NAME}_*.txt "${tmp_folder}/TMP_Aggregazione_STD.txt" \
                    > "${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt"
          fi

          split -C 5MB --numeric-suffixes --additional-suffix=.txt ${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt ${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}_
#          rm "${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt"
          local tmp_reports_pdr_std_chunks=$(find "${tmp_folder}" -type f -name "${REPORT_PDR_STANDARD_NAME}_${CURTMS}_*.txt" | wc -l)
          if [[ tmp_reports_pdr_std_chunks -eq 1 ]]; then
            cp -f "${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}_00.txt" "${destination_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}.txt"
          else
            cp -f ${tmp_folder}/${REPORT_PDR_STANDARD_NAME}_${CURTMS}_*.txt "${destination_folder}"
          fi
      fi
    fi
}
