...
echo "Processo lanciato sul giorno: $GIORNO-$MESE-$ANNO"
echo "Processo lanciato sul giorno: $GIORNO-$MESE-$ANNO" &>> ${SCHEDULATION_LOG_FILE}
echo "*************************************************" &>> ${SCHEDULATION_LOG_FILE}


/mnt/isilonshare1/Software/schedulazioni/GAS/sched_flussitds_gas.sh ${ANNO} ${MESE}
/mnt/isilonshare1/Software/schedulazioni/GAS/sched_flussitfc_gas.sh ${ANNO} ${MESE}
/mnt/isilonshare1/Software/schedulazioni/GAS/sched_flussiVPG_gas.sh ${ANNO} ${MESE}


# Cleaning useful resources
...
OLD_DEPLOY_PATH=/mnt/isilonshare1/Software/schedulazioni/GAS

# Starting Decompression process

echo "Unzip Flussi GAS in corso..."
echo "Unzip Flussi GAS in corso..." &>> ${SCHEDULATION_LOG_FILE}
"$STD_DEPLOY_PATH"/spark-submit-gas-unzip.sh -d "${ANNO}-${MESE}-${GIORNO}"
unzip_result=$?
...
then
  ...
  "$STD_DEPLOY_PATH"/spark-submit-gas-ingestion.sh -f IGMR
  result=$?
  echo "IGMR -> exit code: $result" &>> ${SCHEDULATION_LOG_FILE}
  if [ $result -eq 0 ]
  then
    echo "Running IGMR Export Process..."
    EXPORT_IGMR_LOG_FILE="${logs.root.path}/log-export-igmr-$(date +%Y-%m-%d_%H:%M:%S).log"
    "$STD_DEPLOY_PATH"/sqoop/igmr_export.sh &>> ${EXPORT_IGMR_LOG_FILE}
    result=$?
    echo "IGMR Export Process -> exit code: $result" &>> ${SCHEDULATION_LOG_FILE}
  else
    echo "IGMR Export Process not run since the job exited with a non-zero code." &>> ${SCHEDULATION_LOG_FILE}
  fi
  ...
  echo "Procedura di acquisizione GAS STANDARD, IGMG e IGMR con produzione reports ammissibilità terminata: $(date)" &>> ${SCHEDULATION_LOG_FILE}


  #****************************************************************************************


  # Starting Ingestion process (Flussi Vecchio Tracciato)
#
#  echo "Ingestion Flussi GAS (vecchio tracciato):"
#  echo "Ingestion Flussi GAS (vecchio tracciato):" &>> ${SCHEDULATION_LOG_FILE}
#
#  OLD_FLOWS_1=(
#  "DEF"
#  "FUI"
...
#  )
#
#  for flow in "${OLD_FLOWS_1[@]}"; do
#      "$OLD_DEPLOY_PATH"/sched_flussi_gen.sh $flow ${ANNO} ${MESE} ${GIORNO}
#       result=$?
#       echo "$flow -> exit code: $result" &>> ${SCHEDULATION_LOG_FILE}
#  done
#
#  for flow in "${OLD_FLOWS_2[@]}"; do
#      "$OLD_DEPLOY_PATH"/sched_flussi_gen_2.sh $flow ${ANNO} ${MESE} ${GIORNO}
#       result=$?
#       echo "$flow -> exit code: $result" &>> ${SCHEDULATION_LOG_FILE}
#  done
#
#
#  "$OLD_DEPLOY_PATH"/sched_flussi_gen_im.sh IM1 ${ANNO} ${MESE} ${GIORNO}
#  result=$?
#  echo "IM1 -> exit code: $result" &>> ${SCHEDULATION_LOG_FILE}
#
#  dd=`date`
#  echo "Procedura di acquisizione GAS vecchio tracciato terminata: $dd" &>> ${SCHEDULATION_LOG_FILE}
#
else
...
#report_file + reports errori
#"$OLD_DEPLOY_PATH"/sched_reports.sh ${ANNO} ${MESE} ${GIORNO} "Y"
