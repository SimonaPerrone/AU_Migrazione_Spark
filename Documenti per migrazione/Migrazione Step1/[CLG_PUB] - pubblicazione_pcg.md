# STEP 1: MIGRAZIONE ARCHITETTURALE pubblicazione_pcg
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-pubblicazione_pcg-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-pubblicazione_pcg-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_pubblicazione_pcg` | `/mnt/hdfs-gateway/pubblicazione_pcg/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/pubblicazione_pcg_INJ_STD/...` | `/mnt/hdfs-gateway/pubblicazione_pcg/temp` | `config.properties` |
...
### A1. `spark-submit-pubblicazione_pcg-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.pubblicazione_pcg.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-pubblicazione_pcg-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.pubblicazione_pcg.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/pubblicazione_pcg/input
# PRIMA: /mnt/isilonshare_pubblicazione_pcg

# Temp paths
temp.path=/mnt/hdfs-gateway/pubblicazione_pcg/temp
# PRIMA: /mnt/isilonshare1/pubblicazione_pcg_INJ_STD/isilonshare_pubblicazione_pcg

temp.path.old=/mnt/hdfs-gateway/pubblicazione_pcg/temp_old
# PRIMA: /mnt/isilonshare1/pubblicazione_pcg_INJ/isilonshare_pubblicazione_pcg

# XSD e config
xsd.path=/mnt/hdfs-gateway/pubblicazione_pcg/xsd
# PRIMA: /mnt/isilonshare1/XSD_pubblicazione_pcg_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/pubblicazione_pcg/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/pubblicazione_pcg/deploy
# PRIMA: /mnt/isilonshare1/Software/pubblicazione_pcg/STANDARD
...
rootPath=/mnt/hdfs-gateway/pubblicazione_pcg/ingestion
# PRIMA: /mnt/isilonshare1/pubblicazione_pcg_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/pubblicazione_pcg/temp
# PRIMA: /mnt/isilonshare1/pubblicazione_pcg_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/pubblicazione_pcg/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/pubblicazione_pcg/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-pubblicazione_pcg-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-pubblicazione_pcg-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-pubblicazione_pcg-unzip.sh con kinit
  ☐ Modifica spark-submit-pubblicazione_pcg-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-pubblicazione_pcg-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-pubblicazione_pcg-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/pubblicazione_pcg/*
...
Directory ownership: /mnt/hdfs-gateway/pubblicazione_pcg/* → deve essere writable da airflow
