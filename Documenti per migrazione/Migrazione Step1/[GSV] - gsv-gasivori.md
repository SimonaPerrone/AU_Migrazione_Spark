# STEP 1: MIGRAZIONE ARCHITETTURALE gsv-gasivori
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-gsv-gasivori-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-gsv-gasivori-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_gsv-gasivori` | `/mnt/hdfs-gateway/gsv-gasivori/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/gsv-gasivori_INJ_STD/...` | `/mnt/hdfs-gateway/gsv-gasivori/temp` | `config.properties` |
...
### A1. `spark-submit-gsv-gasivori-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.gsv-gasivori.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-gsv-gasivori-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.gsv-gasivori.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/gsv-gasivori/input
# PRIMA: /mnt/isilonshare_gsv-gasivori

# Temp paths
temp.path=/mnt/hdfs-gateway/gsv-gasivori/temp
# PRIMA: /mnt/isilonshare1/gsv-gasivori_INJ_STD/isilonshare_gsv-gasivori

temp.path.old=/mnt/hdfs-gateway/gsv-gasivori/temp_old
# PRIMA: /mnt/isilonshare1/gsv-gasivori_INJ/isilonshare_gsv-gasivori

# XSD e config
xsd.path=/mnt/hdfs-gateway/gsv-gasivori/xsd
# PRIMA: /mnt/isilonshare1/XSD_gsv-gasivori_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/gsv-gasivori/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/gsv-gasivori/deploy
# PRIMA: /mnt/isilonshare1/Software/gsv-gasivori/STANDARD
...
rootPath=/mnt/hdfs-gateway/gsv-gasivori/ingestion
# PRIMA: /mnt/isilonshare1/gsv-gasivori_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/gsv-gasivori/temp
# PRIMA: /mnt/isilonshare1/gsv-gasivori_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/gsv-gasivori/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/gsv-gasivori/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-gsv-gasivori-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-gsv-gasivori-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-gsv-gasivori-unzip.sh con kinit
  ☐ Modifica spark-submit-gsv-gasivori-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-gsv-gasivori-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-gsv-gasivori-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/gsv-gasivori/*
...
Directory ownership: /mnt/hdfs-gateway/gsv-gasivori/* → deve essere writable da airflow
