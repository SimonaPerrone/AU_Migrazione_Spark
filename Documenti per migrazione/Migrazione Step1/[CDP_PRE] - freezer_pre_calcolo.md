# STEP 1: MIGRAZIONE ARCHITETTURALE freezer_pre_calcolo
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-freezer_pre_calcolo-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-freezer_pre_calcolo-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_freezer_pre_calcolo` | `/mnt/hdfs-gateway/freezer_pre_calcolo/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/freezer_pre_calcolo_INJ_STD/...` | `/mnt/hdfs-gateway/freezer_pre_calcolo/temp` | `config.properties` |
...
### A1. `spark-submit-freezer_pre_calcolo-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.freezer_pre_calcolo.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-freezer_pre_calcolo-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.freezer_pre_calcolo.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/freezer_pre_calcolo/input
# PRIMA: /mnt/isilonshare_freezer_pre_calcolo

# Temp paths
temp.path=/mnt/hdfs-gateway/freezer_pre_calcolo/temp
# PRIMA: /mnt/isilonshare1/freezer_pre_calcolo_INJ_STD/isilonshare_freezer_pre_calcolo

temp.path.old=/mnt/hdfs-gateway/freezer_pre_calcolo/temp_old
# PRIMA: /mnt/isilonshare1/freezer_pre_calcolo_INJ/isilonshare_freezer_pre_calcolo

# XSD e config
xsd.path=/mnt/hdfs-gateway/freezer_pre_calcolo/xsd
# PRIMA: /mnt/isilonshare1/XSD_freezer_pre_calcolo_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/freezer_pre_calcolo/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/freezer_pre_calcolo/deploy
# PRIMA: /mnt/isilonshare1/Software/freezer_pre_calcolo/STANDARD
...
rootPath=/mnt/hdfs-gateway/freezer_pre_calcolo/ingestion
# PRIMA: /mnt/isilonshare1/freezer_pre_calcolo_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/freezer_pre_calcolo/temp
# PRIMA: /mnt/isilonshare1/freezer_pre_calcolo_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/freezer_pre_calcolo/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/freezer_pre_calcolo/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-freezer_pre_calcolo-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-freezer_pre_calcolo-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-freezer_pre_calcolo-unzip.sh con kinit
  ☐ Modifica spark-submit-freezer_pre_calcolo-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-freezer_pre_calcolo-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-freezer_pre_calcolo-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/freezer_pre_calcolo/*
...
Directory ownership: /mnt/hdfs-gateway/freezer_pre_calcolo/* → deve essere writable da airflow
