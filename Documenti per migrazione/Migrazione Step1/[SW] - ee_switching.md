# STEP 1: MIGRAZIONE ARCHITETTURALE ee_switching
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-ee_switching-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-ee_switching-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_ee_switching` | `/mnt/hdfs-gateway/ee_switching/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/ee_switching_INJ_STD/...` | `/mnt/hdfs-gateway/ee_switching/temp` | `config.properties` |
...
### A1. `spark-submit-ee_switching-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.ee_switching.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-ee_switching-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.ee_switching.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/ee_switching/input
# PRIMA: /mnt/isilonshare_ee_switching

# Temp paths
temp.path=/mnt/hdfs-gateway/ee_switching/temp
# PRIMA: /mnt/isilonshare1/ee_switching_INJ_STD/isilonshare_ee_switching

temp.path.old=/mnt/hdfs-gateway/ee_switching/temp_old
# PRIMA: /mnt/isilonshare1/ee_switching_INJ/isilonshare_ee_switching

# XSD e config
xsd.path=/mnt/hdfs-gateway/ee_switching/xsd
# PRIMA: /mnt/isilonshare1/XSD_ee_switching_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/ee_switching/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/ee_switching/deploy
# PRIMA: /mnt/isilonshare1/Software/ee_switching/STANDARD
...
rootPath=/mnt/hdfs-gateway/ee_switching/ingestion
# PRIMA: /mnt/isilonshare1/ee_switching_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/ee_switching/temp
# PRIMA: /mnt/isilonshare1/ee_switching_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/ee_switching/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/ee_switching/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-ee_switching-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-ee_switching-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-ee_switching-unzip.sh con kinit
  ☐ Modifica spark-submit-ee_switching-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-ee_switching-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-ee_switching-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/ee_switching/*
...
Directory ownership: /mnt/hdfs-gateway/ee_switching/* → deve essere writable da airflow
