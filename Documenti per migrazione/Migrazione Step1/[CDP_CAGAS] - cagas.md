# STEP 1: MIGRAZIONE ARCHITETTURALE cagas
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-cagas-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-cagas-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_cagas` | `/mnt/hdfs-gateway/cagas/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/cagas_INJ_STD/...` | `/mnt/hdfs-gateway/cagas/temp` | `config.properties` |
...
### A1. `spark-submit-cagas-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.cagas.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-cagas-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.cagas.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/cagas/input
# PRIMA: /mnt/isilonshare_cagas

# Temp paths
temp.path=/mnt/hdfs-gateway/cagas/temp
# PRIMA: /mnt/isilonshare1/cagas_INJ_STD/isilonshare_cagas

temp.path.old=/mnt/hdfs-gateway/cagas/temp_old
# PRIMA: /mnt/isilonshare1/cagas_INJ/isilonshare_cagas

# XSD e config
xsd.path=/mnt/hdfs-gateway/cagas/xsd
# PRIMA: /mnt/isilonshare1/XSD_cagas_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/cagas/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/cagas/deploy
# PRIMA: /mnt/isilonshare1/Software/cagas/STANDARD
...
rootPath=/mnt/hdfs-gateway/cagas/ingestion
# PRIMA: /mnt/isilonshare1/cagas_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/cagas/temp
# PRIMA: /mnt/isilonshare1/cagas_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/cagas/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/cagas/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-cagas-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-cagas-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-cagas-unzip.sh con kinit
  ☐ Modifica spark-submit-cagas-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-cagas-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-cagas-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/cagas/*
...
Directory ownership: /mnt/hdfs-gateway/cagas/* → deve essere writable da airflow
