# STEP 1: MIGRAZIONE ARCHITETTURALE sbg-sessione-bilanciamento-gas
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-sbg-sessione-bilanciamento-gas-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-sbg-sessione-bilanciamento-gas-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_sbg-sessione-bilanciamento-gas` | `/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/sbg-sessione-bilanciamento-gas_INJ_STD/...` | `/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/temp` | `config.properties` |
...
### A1. `spark-submit-sbg-sessione-bilanciamento-gas-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.sbg-sessione-bilanciamento-gas.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-sbg-sessione-bilanciamento-gas-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.sbg-sessione-bilanciamento-gas.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/input
# PRIMA: /mnt/isilonshare_sbg-sessione-bilanciamento-gas

# Temp paths
temp.path=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/temp
# PRIMA: /mnt/isilonshare1/sbg-sessione-bilanciamento-gas_INJ_STD/isilonshare_sbg-sessione-bilanciamento-gas

temp.path.old=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/temp_old
# PRIMA: /mnt/isilonshare1/sbg-sessione-bilanciamento-gas_INJ/isilonshare_sbg-sessione-bilanciamento-gas

# XSD e config
xsd.path=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/xsd
# PRIMA: /mnt/isilonshare1/XSD_sbg-sessione-bilanciamento-gas_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/deploy
# PRIMA: /mnt/isilonshare1/Software/sbg-sessione-bilanciamento-gas/STANDARD
...
rootPath=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/ingestion
# PRIMA: /mnt/isilonshare1/sbg-sessione-bilanciamento-gas_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/temp
# PRIMA: /mnt/isilonshare1/sbg-sessione-bilanciamento-gas_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-sbg-sessione-bilanciamento-gas-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-sbg-sessione-bilanciamento-gas-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-sbg-sessione-bilanciamento-gas-unzip.sh con kinit
  ☐ Modifica spark-submit-sbg-sessione-bilanciamento-gas-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-sbg-sessione-bilanciamento-gas-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-sbg-sessione-bilanciamento-gas-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/*
...
Directory ownership: /mnt/hdfs-gateway/sbg-sessione-bilanciamento-gas/* → deve essere writable da airflow
