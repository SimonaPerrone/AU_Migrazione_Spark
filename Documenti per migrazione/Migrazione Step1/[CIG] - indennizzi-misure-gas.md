# STEP 1: MIGRAZIONE ARCHITETTURALE indennizzi-misure-gas
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-indennizzi-misure-gas-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-indennizzi-misure-gas-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_indennizzi-misure-gas` | `/mnt/hdfs-gateway/indennizzi-misure-gas/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/indennizzi-misure-gas_INJ_STD/...` | `/mnt/hdfs-gateway/indennizzi-misure-gas/temp` | `config.properties` |
...
### A1. `spark-submit-indennizzi-misure-gas-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.indennizzi-misure-gas.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-indennizzi-misure-gas-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.indennizzi-misure-gas.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/indennizzi-misure-gas/input
# PRIMA: /mnt/isilonshare_indennizzi-misure-gas

# Temp paths
temp.path=/mnt/hdfs-gateway/indennizzi-misure-gas/temp
# PRIMA: /mnt/isilonshare1/indennizzi-misure-gas_INJ_STD/isilonshare_indennizzi-misure-gas

temp.path.old=/mnt/hdfs-gateway/indennizzi-misure-gas/temp_old
# PRIMA: /mnt/isilonshare1/indennizzi-misure-gas_INJ/isilonshare_indennizzi-misure-gas

# XSD e config
xsd.path=/mnt/hdfs-gateway/indennizzi-misure-gas/xsd
# PRIMA: /mnt/isilonshare1/XSD_indennizzi-misure-gas_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/indennizzi-misure-gas/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/indennizzi-misure-gas/deploy
# PRIMA: /mnt/isilonshare1/Software/indennizzi-misure-gas/STANDARD
...
rootPath=/mnt/hdfs-gateway/indennizzi-misure-gas/ingestion
# PRIMA: /mnt/isilonshare1/indennizzi-misure-gas_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/indennizzi-misure-gas/temp
# PRIMA: /mnt/isilonshare1/indennizzi-misure-gas_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/indennizzi-misure-gas/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/indennizzi-misure-gas/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-indennizzi-misure-gas-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-indennizzi-misure-gas-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-indennizzi-misure-gas-unzip.sh con kinit
  ☐ Modifica spark-submit-indennizzi-misure-gas-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-indennizzi-misure-gas-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-indennizzi-misure-gas-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/indennizzi-misure-gas/*
...
Directory ownership: /mnt/hdfs-gateway/indennizzi-misure-gas/* → deve essere writable da airflow
