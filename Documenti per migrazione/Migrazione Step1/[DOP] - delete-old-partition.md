# STEP 1: MIGRAZIONE ARCHITETTURALE delete-old-partition
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-delete-old-partition-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-delete-old-partition-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_delete-old-partition` | `/mnt/hdfs-gateway/delete-old-partition/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/delete-old-partition_INJ_STD/...` | `/mnt/hdfs-gateway/delete-old-partition/temp` | `config.properties` |
...
### A1. `spark-submit-delete-old-partition-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.delete-old-partition.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-delete-old-partition-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.delete-old-partition.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/delete-old-partition/input
# PRIMA: /mnt/isilonshare_delete-old-partition

# Temp paths
temp.path=/mnt/hdfs-gateway/delete-old-partition/temp
# PRIMA: /mnt/isilonshare1/delete-old-partition_INJ_STD/isilonshare_delete-old-partition

temp.path.old=/mnt/hdfs-gateway/delete-old-partition/temp_old
# PRIMA: /mnt/isilonshare1/delete-old-partition_INJ/isilonshare_delete-old-partition

# XSD e config
xsd.path=/mnt/hdfs-gateway/delete-old-partition/xsd
# PRIMA: /mnt/isilonshare1/XSD_delete-old-partition_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/delete-old-partition/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/delete-old-partition/deploy
# PRIMA: /mnt/isilonshare1/Software/delete-old-partition/STANDARD
...
rootPath=/mnt/hdfs-gateway/delete-old-partition/ingestion
# PRIMA: /mnt/isilonshare1/delete-old-partition_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/delete-old-partition/temp
# PRIMA: /mnt/isilonshare1/delete-old-partition_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/delete-old-partition/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/delete-old-partition/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-delete-old-partition-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-delete-old-partition-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-delete-old-partition-unzip.sh con kinit
  ☐ Modifica spark-submit-delete-old-partition-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-delete-old-partition-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-delete-old-partition-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/delete-old-partition/*
...
Directory ownership: /mnt/hdfs-gateway/delete-old-partition/* → deve essere writable da airflow
