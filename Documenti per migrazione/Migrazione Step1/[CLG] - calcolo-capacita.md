# STEP 1: MIGRAZIONE ARCHITETTURALE calcolo-capacita
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-calcolo-capacita-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-calcolo-capacita-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_calcolo-capacita` | `/mnt/hdfs-gateway/calcolo-capacita/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/calcolo-capacita_INJ_STD/...` | `/mnt/hdfs-gateway/calcolo-capacita/temp` | `config.properties` |
...
### A1. `spark-submit-calcolo-capacita-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.calcolo-capacita.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-calcolo-capacita-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.calcolo-capacita.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/calcolo-capacita/input
# PRIMA: /mnt/isilonshare_calcolo-capacita

# Temp paths
temp.path=/mnt/hdfs-gateway/calcolo-capacita/temp
# PRIMA: /mnt/isilonshare1/calcolo-capacita_INJ_STD/isilonshare_calcolo-capacita

temp.path.old=/mnt/hdfs-gateway/calcolo-capacita/temp_old
# PRIMA: /mnt/isilonshare1/calcolo-capacita_INJ/isilonshare_calcolo-capacita

# XSD e config
xsd.path=/mnt/hdfs-gateway/calcolo-capacita/xsd
# PRIMA: /mnt/isilonshare1/XSD_calcolo-capacita_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/calcolo-capacita/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/calcolo-capacita/deploy
# PRIMA: /mnt/isilonshare1/Software/calcolo-capacita/STANDARD
...
rootPath=/mnt/hdfs-gateway/calcolo-capacita/ingestion
# PRIMA: /mnt/isilonshare1/calcolo-capacita_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/calcolo-capacita/temp
# PRIMA: /mnt/isilonshare1/calcolo-capacita_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/calcolo-capacita/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/calcolo-capacita/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-calcolo-capacita-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-calcolo-capacita-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-calcolo-capacita-unzip.sh con kinit
  ☐ Modifica spark-submit-calcolo-capacita-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-calcolo-capacita-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-calcolo-capacita-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/calcolo-capacita/*
...
Directory ownership: /mnt/hdfs-gateway/calcolo-capacita/* → deve essere writable da airflow
