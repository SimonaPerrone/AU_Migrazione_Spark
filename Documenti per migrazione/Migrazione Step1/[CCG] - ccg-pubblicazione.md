# STEP 1: MIGRAZIONE ARCHITETTURALE ccg-pubblicazione
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-ccg-pubblicazione-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-ccg-pubblicazione-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_ccg-pubblicazione` | `/mnt/hdfs-gateway/ccg-pubblicazione/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/ccg-pubblicazione_INJ_STD/...` | `/mnt/hdfs-gateway/ccg-pubblicazione/temp` | `config.properties` |
...
### A1. `spark-submit-ccg-pubblicazione-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.ccg-pubblicazione.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-ccg-pubblicazione-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.ccg-pubblicazione.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/ccg-pubblicazione/input
# PRIMA: /mnt/isilonshare_ccg-pubblicazione

# Temp paths
temp.path=/mnt/hdfs-gateway/ccg-pubblicazione/temp
# PRIMA: /mnt/isilonshare1/ccg-pubblicazione_INJ_STD/isilonshare_ccg-pubblicazione

temp.path.old=/mnt/hdfs-gateway/ccg-pubblicazione/temp_old
# PRIMA: /mnt/isilonshare1/ccg-pubblicazione_INJ/isilonshare_ccg-pubblicazione

# XSD e config
xsd.path=/mnt/hdfs-gateway/ccg-pubblicazione/xsd
# PRIMA: /mnt/isilonshare1/XSD_ccg-pubblicazione_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/ccg-pubblicazione/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/ccg-pubblicazione/deploy
# PRIMA: /mnt/isilonshare1/Software/ccg-pubblicazione/STANDARD
...
rootPath=/mnt/hdfs-gateway/ccg-pubblicazione/ingestion
# PRIMA: /mnt/isilonshare1/ccg-pubblicazione_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/ccg-pubblicazione/temp
# PRIMA: /mnt/isilonshare1/ccg-pubblicazione_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/ccg-pubblicazione/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/ccg-pubblicazione/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-ccg-pubblicazione-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-ccg-pubblicazione-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-ccg-pubblicazione-unzip.sh con kinit
  ☐ Modifica spark-submit-ccg-pubblicazione-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-ccg-pubblicazione-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-ccg-pubblicazione-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/ccg-pubblicazione/*
...
Directory ownership: /mnt/hdfs-gateway/ccg-pubblicazione/* → deve essere writable da airflow
