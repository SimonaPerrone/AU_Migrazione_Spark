# STEP 1: MIGRAZIONE ARCHITETTURALE ee_aggregato_cloudera
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-ee_aggregato_cloudera-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-ee_aggregato_cloudera-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_ee_aggregato_cloudera` | `/mnt/hdfs-gateway/ee_aggregato_cloudera/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/ee_aggregato_cloudera_INJ_STD/...` | `/mnt/hdfs-gateway/ee_aggregato_cloudera/temp` | `config.properties` |
...
### A1. `spark-submit-ee_aggregato_cloudera-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.ee_aggregato_cloudera.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-ee_aggregato_cloudera-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.ee_aggregato_cloudera.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/ee_aggregato_cloudera/input
# PRIMA: /mnt/isilonshare_ee_aggregato_cloudera

# Temp paths
temp.path=/mnt/hdfs-gateway/ee_aggregato_cloudera/temp
# PRIMA: /mnt/isilonshare1/ee_aggregato_cloudera_INJ_STD/isilonshare_ee_aggregato_cloudera

temp.path.old=/mnt/hdfs-gateway/ee_aggregato_cloudera/temp_old
# PRIMA: /mnt/isilonshare1/ee_aggregato_cloudera_INJ/isilonshare_ee_aggregato_cloudera

# XSD e config
xsd.path=/mnt/hdfs-gateway/ee_aggregato_cloudera/xsd
# PRIMA: /mnt/isilonshare1/XSD_ee_aggregato_cloudera_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/ee_aggregato_cloudera/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/ee_aggregato_cloudera/deploy
# PRIMA: /mnt/isilonshare1/Software/ee_aggregato_cloudera/STANDARD
...
rootPath=/mnt/hdfs-gateway/ee_aggregato_cloudera/ingestion
# PRIMA: /mnt/isilonshare1/ee_aggregato_cloudera_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/ee_aggregato_cloudera/temp
# PRIMA: /mnt/isilonshare1/ee_aggregato_cloudera_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/ee_aggregato_cloudera/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/ee_aggregato_cloudera/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-ee_aggregato_cloudera-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-ee_aggregato_cloudera-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-ee_aggregato_cloudera-unzip.sh con kinit
  ☐ Modifica spark-submit-ee_aggregato_cloudera-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-ee_aggregato_cloudera-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-ee_aggregato_cloudera-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/ee_aggregato_cloudera/*
...
Directory ownership: /mnt/hdfs-gateway/ee_aggregato_cloudera/* → deve essere writable da airflow
