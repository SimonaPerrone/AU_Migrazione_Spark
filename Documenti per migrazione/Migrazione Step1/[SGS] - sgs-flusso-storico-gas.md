# STEP 1: MIGRAZIONE ARCHITETTURALE sgs-flusso-storico-gas
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-sgs-flusso-storico-gas-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-sgs-flusso-storico-gas-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_sgs-flusso-storico-gas` | `/mnt/hdfs-gateway/sgs-flusso-storico-gas/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/sgs-flusso-storico-gas_INJ_STD/...` | `/mnt/hdfs-gateway/sgs-flusso-storico-gas/temp` | `config.properties` |
...
### A1. `spark-submit-sgs-flusso-storico-gas-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.sgs-flusso-storico-gas.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-sgs-flusso-storico-gas-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.sgs-flusso-storico-gas.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/sgs-flusso-storico-gas/input
# PRIMA: /mnt/isilonshare_sgs-flusso-storico-gas

# Temp paths
temp.path=/mnt/hdfs-gateway/sgs-flusso-storico-gas/temp
# PRIMA: /mnt/isilonshare1/sgs-flusso-storico-gas_INJ_STD/isilonshare_sgs-flusso-storico-gas

temp.path.old=/mnt/hdfs-gateway/sgs-flusso-storico-gas/temp_old
# PRIMA: /mnt/isilonshare1/sgs-flusso-storico-gas_INJ/isilonshare_sgs-flusso-storico-gas

# XSD e config
xsd.path=/mnt/hdfs-gateway/sgs-flusso-storico-gas/xsd
# PRIMA: /mnt/isilonshare1/XSD_sgs-flusso-storico-gas_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/sgs-flusso-storico-gas/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/sgs-flusso-storico-gas/deploy
# PRIMA: /mnt/isilonshare1/Software/sgs-flusso-storico-gas/STANDARD
...
rootPath=/mnt/hdfs-gateway/sgs-flusso-storico-gas/ingestion
# PRIMA: /mnt/isilonshare1/sgs-flusso-storico-gas_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/sgs-flusso-storico-gas/temp
# PRIMA: /mnt/isilonshare1/sgs-flusso-storico-gas_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/sgs-flusso-storico-gas/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/sgs-flusso-storico-gas/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-sgs-flusso-storico-gas-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-sgs-flusso-storico-gas-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-sgs-flusso-storico-gas-unzip.sh con kinit
  ☐ Modifica spark-submit-sgs-flusso-storico-gas-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-sgs-flusso-storico-gas-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-sgs-flusso-storico-gas-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/sgs-flusso-storico-gas/*
...
Directory ownership: /mnt/hdfs-gateway/sgs-flusso-storico-gas/* → deve essere writable da airflow
