# STEP 1: MIGRAZIONE ARCHITETTURALE scambio-dati-gasivori
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-scambio-dati-gasivori-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-scambio-dati-gasivori-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_scambio-dati-gasivori` | `/mnt/hdfs-gateway/scambio-dati-gasivori/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/scambio-dati-gasivori_INJ_STD/...` | `/mnt/hdfs-gateway/scambio-dati-gasivori/temp` | `config.properties` |
...
### A1. `spark-submit-scambio-dati-gasivori-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.scambio-dati-gasivori.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-scambio-dati-gasivori-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.scambio-dati-gasivori.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/scambio-dati-gasivori/input
# PRIMA: /mnt/isilonshare_scambio-dati-gasivori

# Temp paths
temp.path=/mnt/hdfs-gateway/scambio-dati-gasivori/temp
# PRIMA: /mnt/isilonshare1/scambio-dati-gasivori_INJ_STD/isilonshare_scambio-dati-gasivori

temp.path.old=/mnt/hdfs-gateway/scambio-dati-gasivori/temp_old
# PRIMA: /mnt/isilonshare1/scambio-dati-gasivori_INJ/isilonshare_scambio-dati-gasivori

# XSD e config
xsd.path=/mnt/hdfs-gateway/scambio-dati-gasivori/xsd
# PRIMA: /mnt/isilonshare1/XSD_scambio-dati-gasivori_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/scambio-dati-gasivori/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/scambio-dati-gasivori/deploy
# PRIMA: /mnt/isilonshare1/Software/scambio-dati-gasivori/STANDARD
...
rootPath=/mnt/hdfs-gateway/scambio-dati-gasivori/ingestion
# PRIMA: /mnt/isilonshare1/scambio-dati-gasivori_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/scambio-dati-gasivori/temp
# PRIMA: /mnt/isilonshare1/scambio-dati-gasivori_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/scambio-dati-gasivori/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/scambio-dati-gasivori/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-scambio-dati-gasivori-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-scambio-dati-gasivori-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-scambio-dati-gasivori-unzip.sh con kinit
  ☐ Modifica spark-submit-scambio-dati-gasivori-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-scambio-dati-gasivori-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-scambio-dati-gasivori-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/scambio-dati-gasivori/*
...
Directory ownership: /mnt/hdfs-gateway/scambio-dati-gasivori/* → deve essere writable da airflow
