# STEP 1: MIGRAZIONE ARCHITETTURALE aggregatore_cdp
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-aggregatore_cdp-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-aggregatore_cdp-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_aggregatore_cdp` | `/mnt/hdfs-gateway/aggregatore_cdp/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/aggregatore_cdp_INJ_STD/...` | `/mnt/hdfs-gateway/aggregatore_cdp/temp` | `config.properties` |
...
### A1. `spark-submit-aggregatore_cdp-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.aggregatore_cdp.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-aggregatore_cdp-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.aggregatore_cdp.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/aggregatore_cdp/input
# PRIMA: /mnt/isilonshare_aggregatore_cdp

# Temp paths
temp.path=/mnt/hdfs-gateway/aggregatore_cdp/temp
# PRIMA: /mnt/isilonshare1/aggregatore_cdp_INJ_STD/isilonshare_aggregatore_cdp

temp.path.old=/mnt/hdfs-gateway/aggregatore_cdp/temp_old
# PRIMA: /mnt/isilonshare1/aggregatore_cdp_INJ/isilonshare_aggregatore_cdp

# XSD e config
xsd.path=/mnt/hdfs-gateway/aggregatore_cdp/xsd
# PRIMA: /mnt/isilonshare1/XSD_aggregatore_cdp_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/aggregatore_cdp/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/aggregatore_cdp/deploy
# PRIMA: /mnt/isilonshare1/Software/aggregatore_cdp/STANDARD
...
rootPath=/mnt/hdfs-gateway/aggregatore_cdp/ingestion
# PRIMA: /mnt/isilonshare1/aggregatore_cdp_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/aggregatore_cdp/temp
# PRIMA: /mnt/isilonshare1/aggregatore_cdp_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/aggregatore_cdp/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/aggregatore_cdp/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-aggregatore_cdp-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-aggregatore_cdp-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-aggregatore_cdp-unzip.sh con kinit
  ☐ Modifica spark-submit-aggregatore_cdp-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-aggregatore_cdp-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-aggregatore_cdp-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/aggregatore_cdp/*
...
Directory ownership: /mnt/hdfs-gateway/aggregatore_cdp/* → deve essere writable da airflow
