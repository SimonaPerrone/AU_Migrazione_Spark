# STEP 1: MIGRAZIONE ARCHITETTURALE pubblicazione_cce
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-pubblicazione_cce-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-pubblicazione_cce-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_pubblicazione_cce` | `/mnt/hdfs-gateway/pubblicazione_cce/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/pubblicazione_cce_INJ_STD/...` | `/mnt/hdfs-gateway/pubblicazione_cce/temp` | `config.properties` |
...
### A1. `spark-submit-pubblicazione_cce-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.pubblicazione_cce.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-pubblicazione_cce-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.pubblicazione_cce.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/pubblicazione_cce/input
# PRIMA: /mnt/isilonshare_pubblicazione_cce

# Temp paths
temp.path=/mnt/hdfs-gateway/pubblicazione_cce/temp
# PRIMA: /mnt/isilonshare1/pubblicazione_cce_INJ_STD/isilonshare_pubblicazione_cce

temp.path.old=/mnt/hdfs-gateway/pubblicazione_cce/temp_old
# PRIMA: /mnt/isilonshare1/pubblicazione_cce_INJ/isilonshare_pubblicazione_cce

# XSD e config
xsd.path=/mnt/hdfs-gateway/pubblicazione_cce/xsd
# PRIMA: /mnt/isilonshare1/XSD_pubblicazione_cce_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/pubblicazione_cce/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/pubblicazione_cce/deploy
# PRIMA: /mnt/isilonshare1/Software/pubblicazione_cce/STANDARD
...
rootPath=/mnt/hdfs-gateway/pubblicazione_cce/ingestion
# PRIMA: /mnt/isilonshare1/pubblicazione_cce_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/pubblicazione_cce/temp
# PRIMA: /mnt/isilonshare1/pubblicazione_cce_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/pubblicazione_cce/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/pubblicazione_cce/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-pubblicazione_cce-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-pubblicazione_cce-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-pubblicazione_cce-unzip.sh con kinit
  ☐ Modifica spark-submit-pubblicazione_cce-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-pubblicazione_cce-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-pubblicazione_cce-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/pubblicazione_cce/*
...
Directory ownership: /mnt/hdfs-gateway/pubblicazione_cce/* → deve essere writable da airflow
