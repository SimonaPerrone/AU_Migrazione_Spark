# STEP 1: MIGRAZIONE ARCHITETTURALE gas_trasmissionemisure_cloudera
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-gas_trasmissionemisure_cloudera-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-gas_trasmissionemisure_cloudera-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_gas_trasmissionemisure_cloudera` | `/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/gas_trasmissionemisure_cloudera_INJ_STD/...` | `/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/temp` | `config.properties` |
...
### A1. `spark-submit-gas_trasmissionemisure_cloudera-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.gas_trasmissionemisure_cloudera.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-gas_trasmissionemisure_cloudera-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.gas_trasmissionemisure_cloudera.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/input
# PRIMA: /mnt/isilonshare_gas_trasmissionemisure_cloudera

# Temp paths
temp.path=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/temp
# PRIMA: /mnt/isilonshare1/gas_trasmissionemisure_cloudera_INJ_STD/isilonshare_gas_trasmissionemisure_cloudera

temp.path.old=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/temp_old
# PRIMA: /mnt/isilonshare1/gas_trasmissionemisure_cloudera_INJ/isilonshare_gas_trasmissionemisure_cloudera

# XSD e config
xsd.path=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/xsd
# PRIMA: /mnt/isilonshare1/XSD_gas_trasmissionemisure_cloudera_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/deploy
# PRIMA: /mnt/isilonshare1/Software/gas_trasmissionemisure_cloudera/STANDARD
...
rootPath=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/ingestion
# PRIMA: /mnt/isilonshare1/gas_trasmissionemisure_cloudera_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/temp
# PRIMA: /mnt/isilonshare1/gas_trasmissionemisure_cloudera_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-gas_trasmissionemisure_cloudera-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-gas_trasmissionemisure_cloudera-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-gas_trasmissionemisure_cloudera-unzip.sh con kinit
  ☐ Modifica spark-submit-gas_trasmissionemisure_cloudera-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-gas_trasmissionemisure_cloudera-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-gas_trasmissionemisure_cloudera-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/*
...
Directory ownership: /mnt/hdfs-gateway/gas_trasmissionemisure_cloudera/* → deve essere writable da airflow
