# STEP 1: MIGRAZIONE ARCHITETTURALE aggregatore-consumi-agg
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-aggregatore-consumi-agg-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-aggregatore-consumi-agg-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_aggregatore-consumi-agg` | `/mnt/hdfs-gateway/aggregatore-consumi-agg/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/aggregatore-consumi-agg_INJ_STD/...` | `/mnt/hdfs-gateway/aggregatore-consumi-agg/temp` | `config.properties` |
...
### A1. `spark-submit-aggregatore-consumi-agg-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.aggregatore-consumi-agg.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-aggregatore-consumi-agg-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.aggregatore-consumi-agg.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/aggregatore-consumi-agg/input
# PRIMA: /mnt/isilonshare_aggregatore-consumi-agg

# Temp paths
temp.path=/mnt/hdfs-gateway/aggregatore-consumi-agg/temp
# PRIMA: /mnt/isilonshare1/aggregatore-consumi-agg_INJ_STD/isilonshare_aggregatore-consumi-agg

temp.path.old=/mnt/hdfs-gateway/aggregatore-consumi-agg/temp_old
# PRIMA: /mnt/isilonshare1/aggregatore-consumi-agg_INJ/isilonshare_aggregatore-consumi-agg

# XSD e config
xsd.path=/mnt/hdfs-gateway/aggregatore-consumi-agg/xsd
# PRIMA: /mnt/isilonshare1/XSD_aggregatore-consumi-agg_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/aggregatore-consumi-agg/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/aggregatore-consumi-agg/deploy
# PRIMA: /mnt/isilonshare1/Software/aggregatore-consumi-agg/STANDARD
...
rootPath=/mnt/hdfs-gateway/aggregatore-consumi-agg/ingestion
# PRIMA: /mnt/isilonshare1/aggregatore-consumi-agg_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/aggregatore-consumi-agg/temp
# PRIMA: /mnt/isilonshare1/aggregatore-consumi-agg_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/aggregatore-consumi-agg/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/aggregatore-consumi-agg/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-aggregatore-consumi-agg-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-aggregatore-consumi-agg-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-aggregatore-consumi-agg-unzip.sh con kinit
  ☐ Modifica spark-submit-aggregatore-consumi-agg-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-aggregatore-consumi-agg-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-aggregatore-consumi-agg-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/aggregatore-consumi-agg/*
...
Directory ownership: /mnt/hdfs-gateway/aggregatore-consumi-agg/* → deve essere writable da airflow
