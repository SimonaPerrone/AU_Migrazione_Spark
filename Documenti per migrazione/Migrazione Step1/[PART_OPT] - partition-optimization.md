# STEP 1: MIGRAZIONE ARCHITETTURALE partition-optimization
...
|---|---|---|---|
| **UnzipDriver** | YARN + NFS | YARN + HDFS Gateway + Kerberos | `spark-submit-partition-optimization-unzip.sh` |
| **FlowDriver** | YARN + NFS | CDE + HDFS + Kerberos | `spark-submit-partition-optimization-ingestion.sh` |
| **Autenticazione** | — | `kinit` (Kerberos) | Aggiungi agli script |
| **Path Input** | `/mnt/isilonshare_partition-optimization` | `/mnt/hdfs-gateway/partition-optimization/input` | `config.properties` |
| **Path Temp** | `/mnt/isilonshare1/partition-optimization_INJ_STD/...` | `/mnt/hdfs-gateway/partition-optimization/temp` | `config.properties` |
...
### A1. `spark-submit-partition-optimization-unzip.sh` - YARN Decompression
...
spark2-submit \
    --class it.au.partition-optimization.driver.UnzipDriver \
    --master yarn \
    ... (resto invariato)
A2. spark-submit-partition-optimization-ingestion.sh - CDE Calcolo
bash
...
spark2-submit --master yarn \
    --class it.au.partition-optimization.driver.FlowDriver \
    ... (resto invariato)
...
# Input paths
unzip.input.path=/mnt/hdfs-gateway/partition-optimization/input
# PRIMA: /mnt/isilonshare_partition-optimization

# Temp paths
temp.path=/mnt/hdfs-gateway/partition-optimization/temp
# PRIMA: /mnt/isilonshare1/partition-optimization_INJ_STD/isilonshare_partition-optimization

temp.path.old=/mnt/hdfs-gateway/partition-optimization/temp_old
# PRIMA: /mnt/isilonshare1/partition-optimization_INJ/isilonshare_partition-optimization

# XSD e config
xsd.path=/mnt/hdfs-gateway/partition-optimization/xsd
# PRIMA: /mnt/isilonshare1/XSD_partition-optimization_STANDARD

# Validation/reports
ammissibilita.standard.path=/mnt/hdfs-gateway/partition-optimization/ammissibilita
# PRIMA: /mnt/isilonshare1/TMG_SYNC_TMP1

# Deploy
deploy.path=/mnt/hdfs-gateway/partition-optimization/deploy
# PRIMA: /mnt/isilonshare1/Software/partition-optimization/STANDARD
...
rootPath=/mnt/hdfs-gateway/partition-optimization/ingestion
# PRIMA: /mnt/isilonshare1/partition-optimization_INJ_STD/...

tempRootPath=/mnt/hdfs-gateway/partition-optimization/temp
# PRIMA: /mnt/isilonshare1/partition-optimization_INJ_STD/...
...
Gateway Access	ls /mnt/hdfs-gateway/partition-optimization/input/	Directory listing OK
Write Access	touch /mnt/hdfs-gateway/partition-optimization/temp/test.txt && rm test.txt	File created/deleted
UnzipDriver	bash spark-submit-partition-optimization-unzip.sh --input=sample.zip	Job SUCCEEDED
FlowDriver	bash spark-submit-partition-optimization-ingestion.sh --input=unzipped/	Job SUCCEEDED
...
CODE
  ☐ Modifica spark-submit-partition-optimization-unzip.sh con kinit
  ☐ Modifica spark-submit-partition-optimization-ingestion.sh con kinit
...
2	Script Unzip	Add kinit + test	spark-submit-partition-optimization-unzip.sh	☐
3	Script Flow	Add kinit/CDE + test	spark-submit-partition-optimization-ingestion.sh	☐
...
I. NOTE IMPORTANTI
Path mapping: ogni occorrenza di /mnt/isilonshare* → /mnt/hdfs-gateway/partition-optimization/*
...
Directory ownership: /mnt/hdfs-gateway/partition-optimization/* → deve essere writable da airflow
