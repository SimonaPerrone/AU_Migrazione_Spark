...
class AggBackInTimeFlow extends Flow {
  @transient private lazy val logger = Logger.getLogger(getClass.getName)
  val spark: SparkSession = Environment.getSpark

  import spark.implicits._

  override val flowName: String = "AGG Back In Time"
  ...
  //parametri da proprietà
  val aggExecutionId: Long = Environment.getProperty("job.params.executionid_agg").toLong
  val dataSogliaMid: LocalDate = LocalDate.parse(Environment.getProperty("job.params.data_back_in_time"), DateTimeFormatter.ofPattern("dd/MM/yyyy"))
  val annomeseDa: String = Environment.getProperty("job.params.annomese_da")
  val annomeseA: String = Environment.getProperty("job.params.annomese_a")
  ...
  /** *
   * Simile al processo AGG ma aggiunge la possibilita' di filtrare i dati elaborati per periodo compresso tra
   * annomeseDa - annomeseA e di partire da un calcolo della tabella MID precedente all'ultima versione e uguale a quella
   * presente in dataMid
   */
  override def run(): Unit = {
    ...
    logger.warn(s"dataBackInTime: $dataSogliaMid")
    logger.warn(s"annomeseDa: $annomeseDa")
    logger.warn(s"annomeseA: $annomeseA")
    ...
    val annomeseAnomaliTotaliList = infoEsecuzione("annomeseAnomali").asInstanceOf[mutable.WrappedArray[String]].toList
    ...
    logger.warn(s"annomeseAnomaliTotaliList: ${annomeseAnomaliTotaliList.mkString(", ")}")

    //filtrare gli annomesi per i valori annomeseDa e annomeseA
    logger.warn("Filtro anno mesi estratti con valori nell'intervallo: annomeseDa e annomeseA")
    val annomeseAnomaliList = annomeseAnomaliTotaliList.filter(annomese => annomese >= annomeseDa && annomese <= annomeseA)
    if (annomeseAnomaliList.isEmpty) {
      throw new IllegalArgumentException(s"Nessuna intersezione di mesi tra quelli calcolati da $executionId e quelli applicati ai filtri annomeseDa, annomeseA")
    }
    ...
    val esclusioniDs = if (fileEsclusioniAbilitato) {
      ...
      val tmpEsclusioni = fileEsclusioni.read().where(col(InclusioniSchema.annomese).isin(annomeseAnomaliList: _*))
    ...
    // Calcolo dati PDR presenti in tabella MID nell'ultima versione con valore executionId entro [[dataSogliaMid]]
    val midContatoriPrecedenti = leggiAnomaliMidPrecedenti(midContatoriDs, annomeseAnomaliList, dataSogliaMid)
    // Rimuovi inclusioni ed esclusioni
    val midContatoriPrecedenteFiltrati = CalcoloMidFunzioni.rimuoviInclusioniEdEsclusioni(
      midContatoriPrecedenti, inclusioniDs, esclusioniDs
    ).cache()
    ...
    val midContatoriFinale = CalcoloMidFunzioni.finalizzaMidContatori(
      ...
      tipoCalcolo = CostantiMid.TIPO_CALCOLO_STRAORDINARIO,
  ...
  /** *
   * Leggi anomali su precedente calcolo MID per lista annomese e data soglia come limite massimo
   */
  def leggiAnomaliMidPrecedenti(mid: Dataset[MidContatoriModel], annomeseLista: List[String], dataSoglia: LocalDate): Dataset[MidContatoriModel] = {
    val rowNumberCol = "_tmp_rn"
    /*
    per la data soglia si utilizza l'executionId: quindi per includere i calcoli effettuati nel giorno di soglia bisogna
    prendere il giorno successivo a mezzanotte e calcolarne l'epoch in millis (epochSoglia).
    Poi filtrare per executionId < epochSoglia (epoch strettamente minore della mezzanotte del giorno successivo)
     */
    val dataSogliaGiornoSuccessivo = LocalDateTime.of(dataSoglia.plusDays(1), LocalDateTime.MIN.toLocalTime)
    val epochSoglia = dataSogliaGiornoSuccessivo.atZone(ZoneId.systemDefault()).toInstant.toEpochMilli
    mid
      .where(col(MidContatoriSchema.annomese).isin(annomeseLista: _*))
      .where(col(MidContatoriSchema.executionid_tracciatura) < epochSoglia)
