...
class AggBitFlowCollaudoTest extends EnvironmentSparkTest {
  ...
  /** *
   * Collaudo flusso AGG Back In Time
   */
  def testCollaudo(): Unit = {
    ...
    val executionIdPrecedente = LocalDateTime.parse("2023-12-31 12:00:00", dateTimeFormatter).atZone(ZoneId.systemDefault()).toInstant.toEpochMilli
    val trattamento = "Y"
    val dataCalcolo = LocalDate.parse("2023-12-31", DateTimeFormatter.ofPattern("yyyy-MM-dd"))
    val dataTracciatura = Date.valueOf(dataCalcolo)
    val annomese = "202401"
    val classeMisuratore = "G4"
    val dataSogliaMid = LocalDate.parse("2023-12-31")
    // dati presenti in righe MID da ignorare nel calcolo
    val executionIdDaIgnorare = LocalDateTime.parse("2024-01-01 12:00:00", dateTimeFormatter).atZone(ZoneId.systemDefault()).toInstant.toEpochMilli
    val dataTracciaturaDaIgnorare = Date.valueOf(LocalDate.parse("2024-01-01", DateTimeFormatter.ofPattern("yyyy-MM-dd")))
    ...
    val midContatoriDao = new MidContatoriDaoMock(Seq(
      ...
      // ultimo calcolo da ignorare
      MidContatoriModel(pdr = "agg5", contatore = 2, stato = CostantiMid.STATO_VALIDO, treatment = trattamento, data_tracciatura = dataTracciaturaDaIgnorare, processo_tracciatura = processoTracciatura, sessione_tracciatura = session, causale_tracciatura = CostantiMid.CAUSALE_ESCLUSI, tipo_calcolo = CostantiMid.TIPO_CALCOLO_ORDINARIO, executionid_daily_consumption = executionIdPrecedente, executionid_tracciatura_prev = null, annomese = annomese, executionid_tracciatura = executionIdDaIgnorare),
      MidContatoriModel(pdr = "agg6", contatore = 2, stato = CostantiMid.STATO_VALIDO, treatment = trattamento, data_tracciatura = dataTracciaturaDaIgnorare, processo_tracciatura = processoTracciatura, sessione_tracciatura = session, causale_tracciatura = CostantiMid.CAUSALE_INCOERENTI, tipo_calcolo = CostantiMid.TIPO_CALCOLO_ORDINARIO, executionid_daily_consumption = executionIdPrecedente, executionid_tracciatura_prev = null, annomese = annomese, executionid_tracciatura = executionIdDaIgnorare),
      MidContatoriModel(pdr = "agg7", contatore = 2, stato = CostantiMid.STATO_VALIDO, treatment = trattamento, data_tracciatura = dataTracciaturaDaIgnorare, processo_tracciatura = processoTracciatura, sessione_tracciatura = session, causale_tracciatura = null, tipo_calcolo = CostantiMid.TIPO_CALCOLO_ORDINARIO, executionid_daily_consumption = executionIdPrecedente, executionid_tracciatura_prev = null, annomese = annomese, executionid_tracciatura = executionIdDaIgnorare)
    ).toDS)
