...
CREATE TEMPORARY MACRO getStatoPod(d_oper_misurator_att bigint)
    CASE
        
	WHEN d_oper_misurator_att > cast(concat( year(current_date),lpad(month(current_date),2,0),lpad(day(current_date),2,0)) as bigint) then 'IN FUNZIONE'
	

	WHEN DAY(concat(SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',SUBSTRING( cast(d_oper_misurator_att as String),5, 2),'-',SUBSTRING( cast(d_oper_misurator_att as String),7, 2)) ) < 16 AND CURRENT_DATE > 	concat(SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',SUBSTRING( cast(d_oper_misurator_att as String),5, 2),'-',SUBSTRING( cast(d_oper_misurator_att as String),7, 2))  THEN
            CASE
                WHEN CURRENT_DATE < cast(add_months(concat(
                    SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',
                    cast(SUBSTRING( cast(d_oper_misurator_att as String),5, 2)as int),'-',
                    '01') ,2)as DATE)  THEN 'ATTIVO'
                ELSE 'COMPLETAMENTE CONFIGURABILE'
            END
        WHEN DAY(concat(SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',SUBSTRING( cast(d_oper_misurator_att as String),5, 2),'-',SUBSTRING( cast(d_oper_misurator_att as String),7, 2)) ) > 15 AND CURRENT_DATE > 			concat(SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',SUBSTRING( cast(d_oper_misurator_att as String),5, 2),'-',SUBSTRING( cast(d_oper_misurator_att as String),7, 2))  THEN
            CASE
                WHEN CURRENT_DATE < cast(add_months(concat(
                    SUBSTRING( cast(d_oper_misurator_att as String),1, 4),'-',
                    cast(SUBSTRING( cast(d_oper_misurator_att as String),5, 2) as int),'-',
                    '01'),3) as DATE) THEN 'ATTIVO'
                ELSE 'COMPLETAMENTE CONFIGURABILE'
            END
        ELSE ''
    END;
...
SELECT DISTINCT
        ...
        fornitura.t_piva,
        fornitura.t_rag_soc,
        fornitura.t_servizio_tutela_sii
    ...
    LEFT JOIN (select CAST(n_id_azienda AS STRING)n_id_azienda,t_rag_soc,t_piva from rcu.rcu_azienda_p where isNumeric(n_id_azienda)=true ) AS rcu_azienda
    ON rcu_azienda.n_id_azienda = NVL(fornitura.n_id_fornitore,'');
    
    
    
drop table mongodbs.forniture_elettriche_1;
create TABLE mongodbs.forniture_elettriche_1 stored as parquet as    
SELECT DISTINCT
    nvl(pod.n_id_pod,'') n_id_pod,
    nvl(pod.n_id_cliente,'')n_id_cliente,
    ...
    CASE WHEN pod.tipo_misuratore='G' then (getStatoPod(d_oper_misurator_att)) ELSE '' END AS stato_misuratore_2g,
    ...
    nvl(pod.t_servizio_tutela_sii,'')t_servizio_tutela_sii
...
drop table mongodbs.forniture_elettriche_2;
create TABLE mongodbs.forniture_elettriche_2 stored as parquet as    
SELECT aa.*, case WHEN tabF.t_cf IS NULL then "N" else "Y" end as isVulnerabile from mongodbs.forniture_elettriche_1 AA
LEFT JOIN 
(SELECT p.t_cf, tabB.* from
(SELECT tabPod.t_codice_pod, v1.*  from tde.tde_vuln_p v1
INNER JOIN
(SELECT substring(b.t_codice_pod,1,14) t_codice_pod, a.* from tde.tde_vuln_p a
INNER JOIN rcu.rcu_pod_p b on a.n_id_pod=b.n_id_pod) tabPod
on  v1.n_id_cliente= tabPod.n_id_cliente) tabB
INNER JOIN rcu.rcu_clientefinale_p p
on tabB.n_id_cliente=p.n_id_cliente) tabF
on (AA.t_cf=tabF.t_cf and AA.codice_pod=tabF.t_codice_pod);


drop table mongodbs.forniture_elettriche;
create TABLE mongodbs.forniture_elettriche stored as parquet as
select A.*, nvl(b.t_codice_offerta,'') codice_offerta from mongodbs.forniture_elettriche_2 A
left join (select n_id_fornitura, max(d_aggiornamento), t_codice_offerta from rcu.rcu_codice_offerta_p where d_data_fine is null group by n_id_fornitura,t_codice_offerta ) B 
on A.codice_fornitura=B.n_id_fornitura;
