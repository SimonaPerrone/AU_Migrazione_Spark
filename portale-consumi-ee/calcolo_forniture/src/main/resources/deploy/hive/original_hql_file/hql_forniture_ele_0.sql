...
CASE WHEN inizio < d_inizio_start THEN d_inizio_start ELSE inizio end inizio,fine ,d_inizio_str,d_fine_str,codice_pod,attivo,n_id_pod,n_id_fornitore,t_tipo_mercato,n_id_cliente,n_id_indirizzo,n_id_ind_forn,t_servizio_tutela_sii
 FROM (
...
min(inizio) over ( partition by codice_pod,fine)min_inizio,d_inizio_str,d_fine_str,d_inizio_start,t_servizio_tutela_sii
from (
select n_id_fornitura,
CAST(CASE WHEN d_inizio='' THEN concat( year(date_sub(current_date,1126)),lpad(month(date_sub(current_date,1126)),2,0),'01') ELSE CAST(CONCAT(SUBSTR(d_inizio,1,4),SUBSTR(d_inizio,6,2),SUBSTR(d_inizio,9,2)) AS INT) END AS BIGINT)inizio,
...
CAST(concat( year(date_sub(current_date,1126)),lpad(month(date_sub(current_date,1126)),2,0),'01') as BIGINT) d_inizio_start,t_servizio_tutela_sii
from
(
 select n_id_fornitura,
 ...
 forn.n_id_pod,nvl(forn.n_id_fornitore,'')n_id_fornitore,forn.n_id_cliente,forn.t_tipo_mercato,n_id_indirizzo,n_id_ind_forn,nvl(forn.t_servizio_tutela_sii,'') as t_servizio_tutela_sii
 from
 ...
 forn.n_id_pod,nvl(forn.n_id_fornitore,'')n_id_fornitore,forn.n_id_cliente,forn.t_tipo_mercato,n_id_indirizzo,n_id_ind_forn, "" as t_servizio_tutela_sii
 from
...
where  fine >= cast(concat(year(date_sub(current_date,1126)),lpad(month(date_sub(current_date,1126)),2,0),'01') as bigint)  and (inizio+2) < fine
) AS TTX
