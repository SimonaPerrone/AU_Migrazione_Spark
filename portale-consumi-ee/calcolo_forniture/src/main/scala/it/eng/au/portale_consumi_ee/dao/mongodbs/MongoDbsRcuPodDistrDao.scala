package it.eng.au.portale_consumi_ee.dao.mongodbs

import it.eng.au.portale_consumi_ee.common.dao.hive.HiveDao
import it.eng.au.portale_consumi_ee.common.schema.SchemaEnum
import it.eng.au.portale_consumi_ee.common.utility.environment.Environment
import it.eng.au.portale_consumi_ee.model.mongodbs.{ RcuPodDistrModel}
import it.eng.au.portale_consumi_ee.schema.mongodbs.{ RcuPodDistrSchema}

case class MongoDbsRcuPodDistrDao() extends HiveDao[RcuPodDistrModel]{

  override val tableName: String = Environment.getProperty("hive.table.mongodbs.rcu_pod_distr")
  override val schema: SchemaEnum = RcuPodDistrSchema

}
