...
cd /home/silvia/au_test_20190905/bin
...
if [ "$TEST" = true ]; then
  ...
  echo "${BASEPATH}/script/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO}"
  echo "${BASEPATH}/script/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO}" >>  ${LOG_FILE}
  ${BASEPATH}/script/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO} &>> ${LOG_FILE}
fi
