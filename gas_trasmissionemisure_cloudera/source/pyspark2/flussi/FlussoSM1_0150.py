...
class FlussoSM1_0150:
    def __init__(self):
        self.XSD_SCHEMAS_DIR = "file:///mnt/isilonshare1/XSD_GAS/"
        self.PATH_HDFS = "/user/hive/warehouse/au.db/misure_gas_au/cmg_gas/prt_cmg_sm1_0150_p"
        self.CMD_REFRESH = "MSCK REPAIR TABLE cmg_gas.prt_cmg_sm1_0150_p"

    def getCodFlusso(self):
        return "SM1_0150"

    def get_xsd_file(self, flusso):
        FILE_XSD = os.path.join(self.XSD_SCHEMAS_DIR, 'SM/SM1_0150.xsd')
        return FILE_XSD
    ...
    def write(self, rdd, sc, sqlCtx, test_mode_enabled):
        ...
        schema = StructType([
            ...
            StructField("data_ril", StringType(), True),
            StructField("num_ten", StringType(), True),
	...
	annomese_udf = udf(
            lambda x: "EE" if x is None else "{}{}".format(str(x.split("/")[7]), str(x.split("/")[8][:2]))
        )
        ...
        df = df.withColumn("annomese", annomese_udf(col('local_file')))
	...
	print("Refresh table: {}".format(cmd_refresh))
    ...
    def getItems(self, filename, content_xml):
        ...
        cod_prat_distr, cod_prat_utente, piva_utente, piva_distr, esito, \
        data_disattivazione, segn_mis, segn_conv, matr_mis, cod_pdr, \
        note, cod_servizio, cod_flusso = self.reset()
        ...
        cod_causale = None if content_xml.find("cod_causale") is None else content_xml.find("cod_causale").text

        # DatiTecnici

        cod_pdr = None if content_xml.find("DatiTecnici/cod_pdr") is None else content_xml.find("DatiTecnici/cod_pdr").text
        matr_mis = None if content_xml.find("DatiTecnici/matr_mis") is None else content_xml.find("DatiTecnici/matr_mis").text
        matr_conv = None if content_xml.find("DatiTecnici/matr_conv") is None else content_xml.find("DatiTecnici/matr_conv").text
        segn_mis = None if content_xml.find("DatiTecnici/segn_mis") is None else content_xml.find("DatiTecnici/segn_mis").text
        segn_conv = None if content_xml.find("DatiTecnici/segn_conv") is None else content_xml.find("DatiTecnici/segn_conv").text
        coeff_corr = None if content_xml.find("DatiTecnici/coeff_corr") is None else content_xml.find("DatiTecnici/coeff_corr").text
        tipo_lettura = None if content_xml.find("DatiTecnici/tipo_lettura") is None else content_xml.find("DatiTecnici/tipo_lettura").text
        data_ril = None if content_xml.find("DatiTecnici/data_ril") is None else content_xml.find("DatiTecnici/data_ril").text
        num_ten = None if content_xml.find("DatiTecnici/num_ten") is None else content_xml.find("DatiTecnici/num_ten").text
        ...
        result.append(
            (
                cod_prat_distr, cod_prat_utente, piva_utente, piva_distr,
                esito, cod_causale,
                cod_pdr, matr_mis, matr_conv, segn_mis, segn_conv, coeff_corr, tipo_lettura, data_ril, num_ten,
                cod_servizio, cod_flusso, filename
            )
        )
    ...
    def reset(self):
        ...
        return cod_prat_distr, cod_prat_utente, piva_utente, piva_distr, esito, \
               data_disattivazione, segn_mis, segn_conv, matr_mis, cod_pdr, \
               note, cod_servizio, cod_flusso
