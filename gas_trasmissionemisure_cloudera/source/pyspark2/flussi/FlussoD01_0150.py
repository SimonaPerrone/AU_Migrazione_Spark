...
class FlussoD01_0150:
    def __init__(self):
        self.XSD_SCHEMAS_DIR = "file:///mnt/isilonshare1/XSD_GAS/"
        self.PATH_HDFS = "/user/hive/warehouse/au.db/misure_gas_au/cmg_gas/prt_cmg_d01_0150_p"
        self.CMD_REFRESH = "MSCK REPAIR TABLE cmg_gas.prt_cmg_D01_0150_p"

    def getCodFlusso(self):
        return "D01_0150"

    def get_xsd_file(self, flusso):
        FILE_XSD = os.path.join(self.XSD_SCHEMAS_DIR, 'D/D01_0150.xsd')
        return FILE_XSD
    ...
    def write(self, rdd, sc, sqlCtx, test_mode_enabled):
        ...
        schema = StructType([
            ...
            StructField("data_disattivazione", StringType(), True),
        ...
        df = df.withColumn("annomese", annomese_udf(col('data_disattivazione')))
        ...
        # df = df.withColumn("annomese", annomese_udf(col('data_comp')))
    ...
    def getItems(self, filename, content_xml):
        ...
        data_disattivazione = None if content_xml.find("DatiTecnici/data_disattivazione") is None else content_xml.find(
            "DatiTecnici/data_disattivazione").text
        ...
        result.append(
            (
                cod_prat_distr, cod_prat_utente, piva_utente, piva_distr, esito,
                data_disattivazione, segn_mis, segn_conv, matr_mis, cod_pdr,
                note, cod_servizio, cod_flusso, filename
            )
        )
