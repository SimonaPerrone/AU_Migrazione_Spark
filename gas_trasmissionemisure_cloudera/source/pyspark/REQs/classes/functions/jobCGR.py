...
class JobCGR:
    def __init__(self, conf):
        ...
        self.conf = conf
        self.number_obligatory = conf["CGR"] ["NUMBER_RECORDS_OBLICATORY"] 
        self.pattern = conf["CGR"]["PATTERN_VALID_FILE"]
        self.dirDest = conf["CGR"]["WORKDIR"]
        self.dirOutput =  conf["CGR"]["OUTPUT"]
        self.has_header = conf["CGR"]["HEADER_CSV"]
        self.time_stamp =  str(now.year).zfill(4) + "" + str(now.month).zfill(2) + "" + str(now.day).zfill(2)
        self.nameTableHive = conf["CGR"]["TABLEHIVE"]
        self.cmdTableRefresh = conf["CGR"]["cmdTableRefresh"]
        self.table = conf["CGR"]["table"]
    ...
    def valida_record(self, obj):
        ...
        if self.checkField("PATTERN_VALID_TIPO_PREL", TIPO_PREL): 
        ...
        if self.checkField("PATTERN_VALID_UNIT_MIS_PREL", UNIT_MIS_PREL): 
        ...
        if self.checkField("PATTERN_VALID_PRELIEVO_GIORN", PRELIEVO_GIORN): 
    ...
    def checkField(self, pattern, field):
        return not bool(re.match(self.conf["CGR"][pattern], field))
    ...
    def save_csv_amm(self, x):
        #NUM_RIGA;COD_TIPO_FILE;PIVA_UTENTE;VERIFICA_AMM;COD_CAUSALE;MOTIVAZIONE
        txt = ";".join([ str(x[7]), str(x[8]), str(x[9]), str(x[10]), ";", ";"]) + "\n"
