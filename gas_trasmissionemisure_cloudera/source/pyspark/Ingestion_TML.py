...
class Ingestion:
    ...
    def start(self):
        ...
        sc, sqlCtx = self.set_spark_context(self.appName +  " " + self.codFlusso + ( " Anno:" + self.anno if self.anno is not None else "") + (" Mese:" + self.mese if self.mese is not None else "") + (" Giorno:" + self.giorno if self.giorno is not None else ""), self.master)
	...
	#print("dataParsed Valid count:", dataParsed_valid_cnt) 
        #print("dataParsed not Valid count:", dataParsed_notValid_cnt) 

	#return

        # Elaborazione/Ingestione nella tabella cmg_file_p

	#if (dataParsed_valid_cnt > 0):
	self.elaborateFilesThead(dataValidate, 11, sc, sqlCtx)

        if (dataParsed_notValid_cnt > 0):
	...
	# Dimensione del file
        ...
        tipo_servizio_udf = udf(
            lambda d: (os.path.basename(d).split("_")[3] if len(os.path.basename(d).split("_")[3]) <= 3 else os.path.basename(d).split("_")[3][0:3]) if len(os.path.basename(d).split("_")) >= 4 else ""
        )
	...
	dfElaborateFiles = dfElaborateFiles.withColumn("t_nome_file",  lit(col('filename')))
        ...
        dfElaborateFiles = dfElaborateFiles.withColumn("t_piva_distributore", lit(""))
        dfElaborateFiles = dfElaborateFiles.withColumn("t_piva_udd", lit(""))
	...
	#dfElaborateFiles.show(truncate = False)
        #constants.PATHHDFS_ELEBORATEFILES= "/user/silvia/au/misure_gas_au/cmg_gas/prt_cmg_file_test"
	...
	anno_caricamento_udf = udf(
        ...
        # Mese file nel cloud
        mese_caricamento_udf = udf(
            lambda d: d.split("/")[8][0:2]
	)

        # Giorno file nel cloud
        giorno_caricamento_udf = udf(
            lambda d: d.split("/")[8][2:4]
	)
