...
class FlussoFUI:
    ...
    def isValidFile(self, filename):
        pattern = "^\\w{11,16}_\\w{11,16}_\\d{6}_(F|f)(U|u)(I|i)(\\d{4}|.\\d{4})_\\d*(_\\d{1,}|).\\w{3}$"
    ...
    def getFileXSD(self, codice_servizio, codice_flusso):
        file_xsd = ""
        # Flusso FUI
        if codice_servizio == constants.CODSERVIZIO_FUI and codice_flusso == "0350":
            file_xsd = constants.FILE_XSD_FUI_0350
        elif codice_servizio == constants.CODSERVIZIO_FUI and codice_flusso =="0351":
            file_xsd = constants.FILE_XSD_FUI_0351
    ...
    def write(self, rdd, pool_id, sc, sqlCtx):
        ...
        dfFUI = sqlCtx.createDataFrame(rdd, schema=constants.schemaFlussoFUI)
        ...
        dfFUI = dfFUI.withColumn("t_name_file",  filename_udf(col('local_file')))
        dfFUI = dfFUI.withColumn("n_id_file", n_id_file_udf(col('local_file')))
        dfFUI = dfFUI.withColumn("annomese_riferimento", annomese_rif_udf(col('local_file')))
        dfFUI = dfFUI.withColumn("annomese", annomese_udf(col('data_deco_switch')))
        dfFUI = dfFUI.withColumn("local_file", expr("regexp_replace(local_file, '/isilonshare_gas', '')"))

        dataElaborazione = str(datetime.datetime.now().isoformat())
        dfFUI = dfFUI.withColumn("d_caricamento",  lit(dataElaborazione))
        
        print ("Partition List: ", constants.PARTITIONLIST_FUI)
        print ("Write: ", constants.PATHDFS_FUI)
        dfFUI.write.partitionBy(constants.PARTITIONLIST_FUI).parquet(constants.PATHDFS_FUI, 'append')
        sqlCtx.sql(constants.CMD_REFRESH_FUI)
