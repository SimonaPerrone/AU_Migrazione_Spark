...
class FlussoTAL:
    ...
    def isValidFile(self, filename):
        pattern = "^\\w{11,16}_\\w{11,16}_\\d{6}_(T|t)(A|a)(L|l)(\\d{4}|.\\d{4})_\\d*(_\\d{1,}|).\\w{3}$"
    ...
    def makeError(self, dataElaborazione, anno, mese, local_file):
        ...
        result.append(
            (None, None, dataElaborazione, None, None, DatiPdr_cod_pdr,
             DatiPdr_matr_mis, DatiPdr_matr_conv, DatiPdr_data_com_autolet_cf,
             DatiPdr_let_tot_prel, DatiPdr_let_tot_conv, DatiPdr_esito_val, DatiPdr_note, anno, mese, local_file)
        )
        return result
    ...
    def getFileXSD(self, codice_servizio, codice_flusso):
        file_xsd = ""
        # Flusso TAL
        if codice_servizio == constants.CODSERVIZIO_TAL and codice_flusso == "0050":
            file_xsd = constants.FILE_XSD_TAL_0050
        elif codice_servizio == constants.CODSERVIZIO_TAL and codice_flusso == "0150":
            file_xsd = constants.FILE_XSD_TAL_0150
        return file_xsd

    def write(self, rdd, pool_id, sc, sqlCtx):
        print("Thread TAL")
        sc.setLocalProperty("spark.scheduler.pool", str(pool_id))
        dfTAL = sqlCtx.createDataFrame( rdd, schema=constants.schemaFlussoTAL_0050)
        ...
        dfTAL = dfTAL.withColumn("t_name_file",  filename_udf(col('local_file')))
        dfTAL = dfTAL.withColumn("n_id_file", n_id_file_udf(col('local_file')))
        dfTAL = dfTAL.withColumn("annomese_riferimento", annomese_rif_udf(col('local_file')))
        dfTAL = dfTAL.withColumn("annomese", annomese_udf(col('data_com_autolet_cf')))

        dataElaborazione = str(datetime.datetime.now().isoformat())
        dfTAL = dfTAL.withColumn("d_caricamento",  lit(dataElaborazione))
        dfTAL = dfTAL.withColumn("local_file", expr("regexp_replace(local_file, '/isilonshare_gas', '')"))

        #dfTAL = dfTAL.withColumn("data_com_autolet_cf", expr( "from_unixtime(unix_timestamp(data_com_autolet_cf, 'dd/MM/yyyy'))").cast("timestamp"))
        
        print("Partition List: ", constants.PARTITIONLIST_TAL)
        print("Write: ", constants.PATHDFS_TAL)
        dfTAL.write.partitionBy(constants.PARTITIONLIST_TAL).parquet(constants.PATHDFS_TAL, 'append')
        sqlCtx.sql(constants.CMD_REFRESH_TAL)
    ...
    def getItems(self, root, dataElaborazione, anno, mese, local_file):
        ...
        if DatiPdR is None:
            print("Warn", "flusso TAL non valido")
            return self.makeError(dataElaborazione, anno, mese, local_file)
        ...
        for item in DatiPdR:
            ...
            DatiPdr_note = None if item.find("note") is None else item.find("note").text

            result.append(
                (codice_servizio, cod_flusso, dataElaborazione, pivaUtente, piva_distr, DatiPdr_cod_pdr,
                 DatiPdr_matr_mis, DatiPdr_matr_conv, DatiPdr_data_com_autolet_cf,
                 DatiPdr_let_tot_prel, DatiPdr_let_tot_conv, DatiPdr_esito_val, DatiPdr_note, anno, mese, local_file)
            )
