...
class Command:
	...
	if not self.directoryName:
        ...
        if self.zipCmd == True:
	    # Unzip file
            unzip = Unzip(file = self.directoryName, mode = self.mode, giorno = self.giornoZIP, mese = self.meseZIP, anno = self.annoZIP, codiceFlusso = self.codiceFlusso, distr = self.distr)
            unzip.start()
	    ...
	    logging.info("Modalita ingestion dati")
            #ingestion = Ingestion(directoryName = self.directoryName, mode = self.mode, codFlusso = self.codiceFlusso, file=self.file)
	    ingestion = Ingestion(\
		directoryName = self.directoryName, \
		mode = self.mode, \
		codFlusso = self.codiceFlusso, \
		file=self.file, \
		anno=self.annoZIP, \
                mese=self.meseZIP, \
