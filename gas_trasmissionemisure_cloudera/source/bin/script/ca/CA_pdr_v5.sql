...
drop table au_test.PDR_ELAB;
create table au_test.PDR_ELAB stored as PARQUET  
LOCATION '/user/hive/warehouse/settle_gas.db/PDR_ELAB' 
as
SELECT explode(split("${PDR}", ';')) AS PDR
;
...
as
    ...
    (
        ...
        where 
            ...
            rcugas_massivo_p.t_codice_pdr in (select PDR from au_test.PDR_ELAB)
...
as
    ...
    where PDR in (select PDR from au_test.PDR_ELAB)
...
as 
    ...
    from (
        ...
        join (
            ...
            tmp.cod_pdr = fornituredaver.t_codice_pdr          
            where ( T_PROCESSO  = 'VARIAZIONE') and T_TIPO_OP  = 'InserisciFORNITURA'
            and fornituredaver.t_codice_pdr in (select PDR from au_test.PDR_ELAB)
...
as 
    ...
    where PDRNOCONTFORN.t_codice_pdr is null  
         ...
         and PDRNOCONTFORN.t_codice_pdr in (select PDR from au_test.PDR_ELAB)
...
--            (     -- Non si ricalcola la classe di prelievo
  ...
  --          )                        
    ...
    where TAB_RCUGAS_SETTLE_GAS_DEDOTTI_PDR.pdr in (select PDR from au_test.PDR_ELAB)
...
as
    ...
    ON TAB_SAG1_CONT_FORN.cod_pdr = TAB_RCUGAS_SETTLE_GAS_PDR_V2.pdr
    where TAB_SAG1_CONT_FORN.cod_pdr in (select PDR from au_test.PDR_ELAB)
...
as 
    ...
    where t.data_mis1 is not null  and t.data_mis2 is not null
        and t.cod_pdr in (select PDR from au_test.PDR_ELAB)
...
INSERT OVERWRITE TABLE au.TAB_SAG1_SETTLE_GAS_PDR_Bucketed 
    ...
    join au.TAB_SAG1_CONT_FORN
    on TAB_SAG1_CONT_FORN.cod_pdr = TAB_RCUGAS_SETTLE_GAS_PDR_V2.PDR
    where TAB_RCUGAS_SETTLE_GAS_PDR_V2.pdr in (select PDR from au_test.PDR_ELAB)  
...
WHERE TAB_PROFILI_GIORN_STD_PERC_FORMAT.data >= TAB_PDR_SEGMENTO_DELTA_MIS_MM.DATA_DZ
    ...
    and TAB_PDR_SEGMENTO_DELTA_MIS_MM.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    where TDenominatore.Denominatore <> 0 and DELTA_MIS is not NULL
        and TDenominatore.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    where TAB_PDR_SEGMENTO_CONSUMO_MM.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    where TAB_CONSUMO_ANNUO_MM.pdr in (select PDR from au_test.PDR_ELAB)  
...
--            (     -- Non si ricalcola la classe di prelievo
  ...
  --          )            
    ...
    where TAB_CONSUMO_ANNUO_MM_PDR.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    where TAB_SAG1_SETTLE_CODPROFSTDNEW_INTERM.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    and TAB_SAG1_SETTLE_CODPROFSTDNEW_X.pdr in (select PDR from au_test.PDR_ELAB)  
;
...
INSERT OVERWRITE TABLE au.TAB_SAG1_SETTLE_GAS_PDR_Bucketed_RICALCOLATO 
    ...
    and pdr in (select PDR from au_test.PDR_ELAB)  
...
WHERE TAB_PROFILI_GIORN_STD_PERC_FORMAT.data >= TAB_SAG1_SETTLE_COERZ_NO.data_mis1
    ...
    and TAB_SAG1_SETTLE_COERZ_NO.pdr in (select PDR from au_test.PDR_ELAB)  
...
where TAB_SAG1_SETTLE_COERZ_NO.pdr in (select PDR from au_test.PDR_ELAB)  
...
as 
    ...
    where TAB_CONSUMO_ANNUO_GG.pdr in (select PDR from au_test.PDR_ELAB)  
;
...
--            (   --Non si ricalcola la classe di prelievo
  ...
  --          )
    ...
    where TAB_CONSUMO_ANNUO_GG_PDR.pdr in (select PDR from au_test.PDR_ELAB)  
...
*/
CREATE TABLE IF NOT EXISTS au.TAB_DATI_SETTLE_SAG_RES_ORACLE (
	  id_sag_ann int, 
...
partitioned by (data_elaborazione String) 
...
INSERT TABLE au.TAB_DATI_SETTLE_SAG_RES_ORACLE PARTITION(data_elaborazione)
select 
    row_number() over () as ID_SAG_ANN,
    '${YEAR_VAR}' as anno_competenza, 
    ...
    from_unixtime(unix_timestamp(),'${CURRENT_DATE}') as data_elaborazione
...
drop TABLE au.TAB_DATI_SETTLE_SAG_RES_ORACLE_LAST;
