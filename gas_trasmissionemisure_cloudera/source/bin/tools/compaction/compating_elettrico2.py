...
def get_size_partition(path):
    print("get size: {}".format(path))
    res = os.popen("hadoop fs -du -s {}".format(path)).read().split()[0]
...
def main(argv):
    ...
    #file_partitions="/tmp/au_flusso_misure_noaggr_partitions.dat"
    with open(filename_partitions) as file_in:
        ...
        for line_str in file_in:
            ...
            file = "{}/{}/{}".format(path_hdfs, table_name, line)
            path_hdfs_dest = "{}/{}".format(path_hdfs, table_name_dest)

            size_partition = get_size_partition(file) 
            if size_partition > 0:
                ...
                print("size partions:{}".format(convert_size(size_partition)))
