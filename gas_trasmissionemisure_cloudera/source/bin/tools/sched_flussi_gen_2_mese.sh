...
echo "Avvio procedura di decompressione " $>> ${LOG_FILE}
if [ "$TEST" = true ]; then
  echo "./flusso-misure.sh -DGAS ${FLUSSO}-T --anno ${ANNO} --mese ${MESE} --giorni ${GIORNO}"  &>> ${LOG_FILE}
  echo "./flusso-misure.sh -DGAS ${FLUSSO}-T --anno ${ANNO} --mese ${MESE} --giorni ${GIORNO}"
  ./flusso-misure.sh -DGAS ${FLUSSO}-T --anno ${ANNO} --mese ${MESE} --giorno ${GIORNO} &>> /tmp/decompressione.log
else
  ./flusso-misure.sh -DGAS ${FLUSSO} --anno ${ANNO} --mese ${MESE} --giorni ${GIORNO} &>> /tmp/decompressione.log
fi
...
if [ "$TEST" = true ]; then
  ...
  echo "${PATH_TOOLS}/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO} -v"
  echo "${PATH_TOOLS}/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO} -v" >>  ${LOG_FILE}
  ${PATH_TOOLS}/ingestion_data_flussi2.sh -e /tmp/query.sql -f ${FLUSSO} -v &>> ${LOG_FILE}
fi
