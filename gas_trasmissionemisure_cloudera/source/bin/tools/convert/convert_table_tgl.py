...
def main(argv):
    ...
    database = argv[0]
    table_name = argv[1]
    table_name_dest = argv[2] 
    path_hdfs_dest = argv[3]
    filename_partitions = argv[4]

    print("database: {}\n"
          "table_name: {}\n"
          "table_name_dest: {}\n"
          "path_hdfs_dest: {}\n"
          "filename_partitions: {}".format(database, table_name, table_name_dest, path_hdfs_dest, filename_partitions))

    app_name = "Converti table: {}".format(table_name)
    mode = "yarn-client"
    #mode = "local"
    ...
    with open(filename_partitions) as file_in:
        ...
        for line_str in file_in:
            ...
            print("size directory:{}".format(size))
            print("Read parquet: {}".format(file_p))
            df1 = sqlCtx.read.parquet(file_p)

            print("path hdfs dest:{}".format(path_hdfs_dest))
            mode_write='append'

            key="mese_comp"
            value=str(file_p.split("=")[1])
            #print("mese_comp={}".format(value))
            df1 = df1.withColumn(key, lit(value))
            df1 = df1.withColumn("piva_distr", lit(str(col('piva_distr'))))
            df1 = df1.withColumn("piva_utente", lit(str(col('piva_utente'))))
            #print(df1.schema)

            partitions=["mese_comp"]
            N = get_repartition_factor(sc,size)
            print("repartition: {}".format(N))
            #df1.coalesce(1).write.partitionBy(partitions).parquet(path_hdfs_dest,mode_write)
