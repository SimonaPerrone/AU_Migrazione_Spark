...
import it.eng.au.pubblicazione_cce.dao.cce.{CceCalcoloCaDao, CceCalcoloDao, CceEsitoDao, CceEsitoExportDao, CceRichiestaFiltroDao}
...
import it.eng.au.pubblicazione_cce.flow.consumi.{PubblicazioneConsumiCAFlow, PubblicazioneConsumiFlow}
...
class PubblicazioneCAFlow(val processo: String, val dataRichieste: LocalDate) extends Serializable {
  ...
  val pubblicazioneConsumiCAFlow: PubblicazioneConsumiCAFlow =
    new PubblicazioneConsumiCAFlow(dataRichieste = dataRichieste)
  ...
  def run(): Unit = {
    ...
    val esitoMisure = pubblicazioneConsumiCAFlow.run()
      .persist(StorageLevel.MEMORY_AND_DISK_SER_2)
