package it.eng.au.sgsFlussoStoricoGas.utility.log

import com.typesafe.config.{Config, ConfigFactory}
import it.eng.au.sgsFlussoStoricoGas.utility.constants.FieldConstants.TIMESTAMP_FORMAT
...
object LogUtility {
  ...
  def printInitialLog(sysDate: Option[DateTime] = None): Unit = {
    ...
    if (sysDate.isDefined) log.warn(s"$logName Sys date: ${sysDate.get.toString(TIMESTAMP_FORMAT)}")
