session="PC-misure-gas-33m"
DATA_OGGI=$(date "+%Y%m%d%H%M%S")
LOGFILE=${deploy.path.logs}/${DATA_OGGI}-MISURE_GAS_33M-log.txt
...
${deploy.path.local}/portale-consumi.jar -p ${deploy.path.hdfs}/params.properties -f MISURE_GAS -i 33M 2>&1 | tee -a ${LOGFILE}
