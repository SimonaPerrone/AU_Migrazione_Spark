session="PC-forniture-gas-full"
DATA_OGGI=$(date "+%Y%m%d%H%M%S")
LOGFILE=${deploy.path.logs}/${DATA_OGGI}-FORNITURE_GAS_FULL-log.txt
...
${deploy.path.local}/portale-consumi.jar -p ${deploy.path.hdfs}/params.properties -f FORNITURE_GAS -i FULL 2>&1 | tee -a ${LOGFILE}
