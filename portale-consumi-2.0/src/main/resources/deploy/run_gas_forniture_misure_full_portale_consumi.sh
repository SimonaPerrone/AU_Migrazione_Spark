# lancia prima processo forniture e poi processo misure
# i lanci sono standard e quindi non fanno forzature su delta o completo

set -e

session="PC-forniture-misure-gas-full"
DATA_OGGI=$(date "+%Y%m%d%H%M%S")
LOGFILE=${deploy.path.logs}/${DATA_OGGI}-FORNITURE_MISURE_GAS_FULL-log.txt
...
${deploy.path.local}/portale-consumi.jar -p ${deploy.path.hdfs}/params.properties -f FORNITURE_GAS 2>&1 | tee -a ${LOGFILE}
...
--conf spark.sql.shuffle.partitions=$SHUFFLE_PARTITION \
--conf spark.executor.memoryOverhead=$EXECUTOR_MEMORYOVERHEAD \
${deploy.path.local}/portale-consumi.jar -p ${deploy.path.hdfs}/params.properties -f MISURE_GAS -i FULL 2>&1 | tee -a ${LOGFILE}
