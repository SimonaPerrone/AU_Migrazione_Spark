...
class RdbIncoerentiSbgTest extends EnvironmentSparkTest {
  ...
  def testGetAggregato(): Unit = {
    ...
    val dailyConsumptionDF = Environment.sparkContext.parallelize(
      List(
        ...
        (java.sql.Date.valueOf("2022-07-16"), 0.1, 10, true, "G", "202207", "000PDR", "000DISTR", "10238291008", "000UDD", "000UDB", "Y", "000REMI", 0.1, "000IDCL", "CODPROF_0", "U", "T", "SBG")
      )
    ).toDF(
    ...
    val outDF = RdbIncoerentiSbg.getAggregato(dailyConsumptionDF)

    outDF.show(truncate = false)

    Assert.assertEquals(1, outDF.filter(col("prelievo_giorn_12") === lit(100000)).count)

    val outFields = RdbIncoerentiSbg.csvFields
    val x = 1
    ...
    Assert.assertEquals(outDF.filter(col("piva_it") === "10238291008").count, 0)
  }
