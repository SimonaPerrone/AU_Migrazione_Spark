...
import it.eng.au.aggregatoreConsumiCommon.schema.{EsclusiOutputSchema, InfoOutputSchema, ValidatedFlowsAggSchema}
...
trait EsclusiTrait extends RunnableAggregatorTrait {
  val operationName = "INCOERENTI_EXC"
  val annoMese: String = EsclusiOutputSchema.annoMese.toString
  ...
  val pdrDettaglioEsclusi: PdrDettaglioEsclusi
  val elencoFlussiDettaglioEsclusi: ElencoFlussiDettaglioEsclusi
  ...
  override def run(df: DataFrame): Unit = {
    val dfAggregatoElencoFlussi = elencoFlussiDettaglioEsclusi.getAggregato(df, getAndPrepareValidateFlow())
    val dfAggregatoPdr = pdrDettaglioEsclusi.getAggregato(df)
    ...
    val fieldsElencoFlussi = elencoFlussiDettaglioEsclusi.csvFields
    val fieldsPdr = pdrDettaglioEsclusi.csvFields
    ...
    val rddInfoElencoFlussi = writeCsvAnnoMeseElencoFlussi(csvOutputModelElencoFlussi, fieldsElencoFlussi, elencoFlussiDettaglioEsclusi)
    val rddInfoPdr = writeCsvAnnoMesePdr(csvOutputModelPdr, fieldsPdr, pdrDettaglioEsclusi)
    ...
    writeInfoInTable(rddWithInfo.distinct)
  }
  ...
  def writeCsvAnnoMeseElencoFlussi(rddCsvOutput: RDD[(ListMap[String, String], Row)], columnsField: List[String], csvOutputPath: RunnableAggregatorPerfomanceOld): RDD[((String, String), String)] = {
    ...
    rddCsvOutput.groupByKey().map({ case (mapKeys, rows) =>
      ...
      val header = publicationType match {
        case "AGG" => Some(columnsField.map(_.toLowerCase).mkString(CSV_SEPARATOR))
        case "SBG" => Some(columnsField.map(_.toLowerCase).mkString(CSV_SEPARATOR))
      }
  ...
  def writeZipAnnoMese(rddCsvPath: RDD[((String, String), String)]): RDD[(String, String, String, String, Timestamp, Long)] = {
    ...
    val rddInfo = rddCsvPath.flatMap({ case ((pivaHead, annomese), path) =>
      ...
      val exist = if (outputFolderIfExists.exists()) {
        ...
        pathInputFileCsv.getParentFile.listFiles().filter(value => value.getName.substring(value.getName.length - 4).equals(".csv") && value.getName.contains(operationName)).foreach { csvFile =>
  ...
  def getZipOutputName(pivaFolder: String, annomese: String, sessionName: String, today: LocalDateTime, year: String): String = {
    ...
    //es: 0123456789_DETTAGLIO_PDR_(AGG_S1_PRE|SBG)_INCOERENTI_EXC_2022(04)_20220428105421_1.zip
