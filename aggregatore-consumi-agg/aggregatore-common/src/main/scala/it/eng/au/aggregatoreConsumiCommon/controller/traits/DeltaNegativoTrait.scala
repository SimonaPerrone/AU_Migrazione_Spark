...
import it.eng.au.aggregatoreConsumiCommon.schema.{DeltaNegativoDettaglioSchema, DeltaNegativoElencoFlussiSchema, InfoOutputSchema, ValidatedFlowsAggSchema}
...
trait DeltaNegativoTrait extends RunnableAggregatorTrait {
    val operationName = "DN"
    val annoMese: String = DeltaNegativoElencoFlussiSchema.annomese.toString
    ...
    val pdrDettaglioDeltaNegativo: PdrDettaglioDeltaNegativo
    val elencoFlussiDettaglioDeltaNegativo: ElencoFlussiDettaglioDeltaNegativo
    ...
    override def run(df: DataFrame): Unit = {
      val dfAggregatoElencoFlussi = elencoFlussiDettaglioDeltaNegativo.getAggregato(df, getAndPrepareValidateFlow())
      val dfAggregatoPdr = pdrDettaglioDeltaNegativo.getAggregato(df)
      ...
      val fieldsElencoFlussi = elencoFlussiDettaglioDeltaNegativo.csvFields
      val fieldsPdr = pdrDettaglioDeltaNegativo.csvFields
      ...
      val rddInfoElencoFlussi = writeCsvAnnoMeseElencoFlussi(csvOutputModelElencoFlussi, fieldsElencoFlussi, elencoFlussiDettaglioDeltaNegativo)
      val rddInfoPdr = writeCsvAnnoMesePdr(csvOutputModelPdr, fieldsPdr, pdrDettaglioDeltaNegativo)
