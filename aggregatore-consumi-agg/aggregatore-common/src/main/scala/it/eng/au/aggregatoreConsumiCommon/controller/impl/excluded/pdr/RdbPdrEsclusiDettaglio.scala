...
object RdbPdrEsclusiDettaglio extends PdrDettaglioEsclusi {
  ...
  override val aggregatoColumns: ListMap[String, String] = ListMap(
    ...
    DailyConsumptionAggSchema.pivaRdb.toString -> EsclusiOutputSchema.piva_rdb
  )

  override val csvFields: List[String] = (List(dataValColName) ::: aggregatoColumns.values.toList).diff(List(EsclusiOutputSchema.piva_rdb.toString))
  ...
  override def getCsvOutputPath(baseName: String, piva1: String, piva2: String, annomese: String, sessionName: String, date: LocalDateTime, counterCsv: String): String = {
    ...
    s"/${baseName}_$piva1/$year/$month/${piva1}_AGG_${operationName}_${annomese}_${timestamp}_${counterCsv}.csv"
  }
}
