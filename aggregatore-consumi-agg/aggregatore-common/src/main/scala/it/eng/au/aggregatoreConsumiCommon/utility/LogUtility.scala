...
object LogUtility {
  ...
  def printInitialLog(applicationName: String, logName: String): Unit = {
    ...
    log.warn(s"$logName: Start time: ${Environment.getDateRun}")
    log.warn(s"$logName: Execution ID: ${Timestamp.valueOf(Environment.getDateRun).getTime.toString}")
  ...
  def printFinalLog(applicationName: String, logName: String): Unit = {
    ...
    val startDate = Environment.getDateRun
