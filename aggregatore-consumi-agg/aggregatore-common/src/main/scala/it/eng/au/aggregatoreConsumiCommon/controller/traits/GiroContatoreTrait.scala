...
import it.eng.au.aggregatoreConsumiCommon.schema.{GiroContatoreDettaglioSchema, GiroContatoreElencoFlussiSchema, InfoOutputSchema, ValidatedFlowsAggSchema}
...
import org.apache.spark.sql.functions.{ceil, col, count, lit, monotonically_increasing_id}
...
trait GiroContatoreTrait extends RunnableAggregatorTrait {
    val operationName = "GIRO"
    val annoMese: String = GiroContatoreElencoFlussiSchema.annomese.toString
    ...
    val pdrDettaglioGiroContatore: PdrDettaglioGiroContatore
    val elencoFlussiDettaglioGiroContatore: ElencoFlussiDettaglioGiroContatore
    ...
    override def run(df: DataFrame): Unit = {
      val dfAggregatoElencoFlussi = elencoFlussiDettaglioGiroContatore.getAggregato(df, getAndPrepareValidateFlow())
      val dfAggregatoPdr = pdrDettaglioGiroContatore.getAggregato(df)
      ...
      val fieldsElencoFlussi = elencoFlussiDettaglioGiroContatore.csvFields
      val fieldsPdr = pdrDettaglioGiroContatore.csvFields
      ...
      val rddInfoElencoFlussi = writeCsvAnnoMeseElencoFlussi(csvOutputModelElencoFlussi, fieldsElencoFlussi, elencoFlussiDettaglioGiroContatore)
      val rddInfoPdr = writeCsvAnnoMesePdr(csvOutputModelPdr, fieldsPdr, pdrDettaglioGiroContatore)
    ...
    def writeCsvAnnoMeseElencoFlussi(rddCsvOutput: RDD[(ListMap[String, String], Row)], columnsField: List[String], csvOutputPath: RunnableAggregatorPerfomanceOld): RDD[((String, String), String)] = {
  ...
  def writeCsvAnnoMesePdr(rddCsvOutput: RDD[(ListMap[String, String], Row)], columnsField: List[String], csvOutputPath: RunnableAggregatorPerfomanceOld): RDD[((String, String), String)] = {
    ...
    rddCsvOutput.groupByKey().map({ case (mapKeys, rows) =>
      ...
      val filteredColumns = columnsField.filterNot(_ == annoMese)
      ...
      val header = publicationType match {
        case "AGG" => Some(filteredColumns.mkString(CSV_SEPARATOR))
        case "SBG" => Some(filteredColumns.map(_.toLowerCase).mkString(CSV_SEPARATOR))
      }
   ...
   */
    ...
    def writeZipAnnoMese(rddCsvPath: RDD[((String, String), String)]): RDD[(String, String, String, String, Timestamp, Long)] = {
      ...
      val rddInfo = rddCsvPath.flatMap({ case ((pivaHead, annomese), path) =>
        ...
        val exist = if (outputFolderIfExists.exists()) {
          ...
          pathInputFileCsv.getParentFile.listFiles().filter(value => value.getName.substring(value.getName.length - 4).equals(".csv") && value.getName.contains(sessionName + "_" + operationName +  "_" + annomese + "_")).foreach { csvFile =>
    ...
    def getZipOutputName(pivaFolder: String, annomese: String, sessionName: String, today: LocalDateTime, year: String): String = {
      ...
      val zipName = s"/${pivaFolder}_DETTAGLIO_PDR_${sessionName}_${operationName}_${annomese}_${timestamp}_1.zip"
      zipName
    }
