...
import it.eng.au.aggregatoreConsumiCommon.controller.impl.incoerentiDettaglio.pdr.ItPdrIncoerentiDettaglio
...
class ItIncoerentiDettaglioTest extends EnvironmentSparkTest {
  ...
  def testGetAggregato(): Unit = {
    ...
    val anomalousPdrs = ItIncoerentiDettaglio.getAnomalousPdrs(dailyConsumptionDF)
    val outDF = ItPdrIncoerentiDettaglio.getAggregatoFromPdrs(anomalousPdrs)

    outDF.show(truncate = false)

    //Assert.assertEquals(1, outDF.filter(col("prelievo_non_sterilizzato_giorn_12") === lit(100000.0)).count)

    val outFields = ItPdrIncoerentiDettaglio.csvFields
