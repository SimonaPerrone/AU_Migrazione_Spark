...
class UDDIncoerentiTest extends EnvironmentSparkTest {
...
//  }
  ...
  def testGetAggregato(): Unit = {
    ...
    val dailyConsumptionDF = Environment.sparkContext.parallelize(
      List(
        (java.sql.Date.valueOf("2020-12-12"), 100000.1, 0, true, "G", "202012", "000PDR", "000DISTR", "000IT", "000UDD", "000UDB", "Y", "000REMI", 0.1, "000IDCL", "CODPROF_0", "U", "T", "S_AGG_1"),
        (java.sql.Date.valueOf("2020-12-13"), 0.5, 0, true, "N", "202012", "000PDR", "000DISTR", "000IT", "000UDD", "000UDB", "Y", "000REMI", 0.1, "000IDCL", "CODPROF_0", "U", "T", "S_AGG_1"),
        (java.sql.Date.valueOf("2020-12-14"), 2.0, 0, true, "G", "202012", "000PDR", "000DISTR", "000IT", "000UDD", "000UDB", "Y", "000REMI", 0.1, "000IDCL", "CODPROF_0", "U", "T", "S_AGG_1"),
        (java.sql.Date.valueOf("2020-12-15"), 0.1, 10, true, "G", "202012", "000PDR", "000DISTR", "000IT", "000UDD", "000UDB", "Y", "000REMI", 0.1, "000IDCL", "CODPROF_0", "U", "T", "S_AGG_1")
      )
    ).toDF(
    ...
    val outDF = UddIncoerenti.getAggregato(dailyConsumptionDF)

    outDF.show(truncate = false)

    Assert.assertEquals(1, outDF.filter(col("prelievo_giorn_12") === lit(100000)).count)

    UddIncoerenti.run(dailyConsumptionDF)

    val outFields = UddIncoerenti.csvFields
