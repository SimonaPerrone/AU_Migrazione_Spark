...
class UdbPdrDettaglioUnicoTest extends EnvironmentSparkTest {
  def testAggregato(): Unit = {
    ...
    val aggDF = UdbPdrDettaglioUnico.getAggregato(dailyConsumptionDF).persist()
    aggDF.show()
    ...
    val csvFields = UdbPdrDettaglioUnico.getCsvFields(aggDF)

    Assert.assertEquals(csvColsNameSet.size, csvColsNameSet.intersect(csvFields.toSet).size)
    println(csvFields.toSet.diff(csvColsNameSet))
