...
import it.eng.au.aggregatoreConsumiCommon.controller.impl.incoerentiDettaglio.pdr.UdbPdrIncoerentiDettaglio
...
class UdbIncoerentiDettaglioTest extends EnvironmentSparkTest {
  ...
  def testGetAggregato(): Unit = {
    ...
    val anomalousPdrs = UdbIncoerentiDettaglio.getAnomalousPdrs(dailyConsumptionDF)
    val outDF = UdbPdrIncoerentiDettaglio.getAggregatoFromPdrs(anomalousPdrs)

    outDF.show(truncate = false)

    //Assert.assertEquals(1, outDF.filter(col("prelievo_non_sterilizzato_giorn_12") === lit(100000.0)).count)

    val outFields = UdbPdrIncoerentiDettaglio.csvFields
