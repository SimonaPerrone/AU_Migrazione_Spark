...
import it.eng.au.aggregatoreConsumiCommon.controller.impl.incoerentiDettaglio.pdr.UddPdrIncoerentiDettaglio
...
class UddIncoerentiDettaglioTest extends EnvironmentSparkTest {
  ...
  def testGetAggregato(): Unit = {
    ...
    val anomalousPdrs = UddIncoerentiDettaglio.getAnomalousPdrs(dailyConsumptionDF)
    val outDF = UddPdrIncoerentiDettaglio.getAggregatoFromPdrs(anomalousPdrs)

    outDF.show(truncate = false)

    //Assert.assertEquals(1, outDF.filter(col("prelievo_non_sterilizzato_giorn_12") === lit(100000.0)).count)

    val outFields = UddPdrIncoerentiDettaglio.csvFields
