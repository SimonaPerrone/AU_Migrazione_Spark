...
class RdbAggregatorTest extends EnvironmentSparkTest with Check {
  def testGetAggregato(): Unit = {
    ...
    val result = RdbAggregator.getAggregato(df)
      .cache()

    result.show(false)
    var row = result
      .filter(col(DailyConsumptionAggSchema.annoMese) === lit("202105") and col(RdbAggregatorOutputSchema.PIVA_IT) === lit("it2"))
      .collect()
    checkResultRowRDB(row(0), "202105", "01/05/2021", "rdb2", "it2", "udd2", "pivaUdb2", "dtg2", "cod_remi2", "id-reg2", "cod-prof2", "Y", "tipo-client2", "unit2",
      (10, 2))
    row = result
      .filter(col(DailyConsumptionAggSchema.annoMese) === lit("202104") and col(RdbAggregatorOutputSchema.PIVA_IT) === lit("it2"))
      .collect()
    checkResultRowRDB(row(0), "202104", "01/04/2021", "rdb2", "it2", "udd2", "pivaUdb2", "dtg2", "cod_remi2", "id-reg2", "cod-prof2", "Y", "tipo-client2", "unit2",
      (10, 1))

    row = result
      .filter(col(DailyConsumptionAggSchema.annoMese) === lit("202105") and col(RdbAggregatorOutputSchema.PIVA_IT) === lit("it1"))
      .collect()
