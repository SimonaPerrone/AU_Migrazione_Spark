...
DROP TABLE rcugas.rcugas_massivo_ca_p;
DROP TABLE rcugas.rcugas_tech_ca_p;

CREATE TABLE IF NOT EXISTS rcugas.rcugas_massivo_ca_p STORED AS PARQUET 
LOCATION '/user/hive/warehouse/rcugas.db/rcugas_massivo_ca_p'
AS
...
FROM (
      SELECT b.n_id_pdr, b.t_codice_pdr, b.capacita_trasporto, b.mese_val_cap_trasp, b.t_cod_tipo_pdr, b.t_disalimentabilita,
             ...
             AND date_format(nvl(b.data_fine_for_case,'2999-12-31 00:00:00.0'),'yyyy-MM-dd') > from_unixtime(unix_timestamp('2020-07-01','yyyy-MM-dd'),'yyyy-MM-dd')
      ...
      FROM(
           ...
           AND date_format(nvl(a.d_data_inizio_for,'1492-12-31 00:00:00.0'),'yyyy-MM-dd')
               <= from_unixtime(unix_timestamp('2020-07-01','yyyy-MM-dd'),'yyyy-MM-dd')
      	 AND a.n_id_pdr IN (SELECT cc.n_id_pdr
      	                    FROM rcugas.rcugas_massivo_p AS cc
      						WHERE from_unixtime(unix_timestamp('2020-07-01','yyyy-MM-dd'),'yyyy-MM-dd')
...
CREATE TABLE IF NOT EXISTS rcugas.rcugas_tech_ca_p STORED AS PARQUET
LOCATION '/user/hive/warehouse/rcugas.db/rcugas_tech_ca_p'
AS
...
FROM (SELECT a.t_codice_pdr, a.n_id_pdr
      FROM rcugas.rcugas_massivo_ca_p as a
      ...
      (SELECT mis.n_id_pdr,
       ...
       FROM rcugas.rcugas_pdr_var_misuratore_p as mis
       LEFT OUTER JOIN (SELECT pp.n_id_pdr,
                               pp.n_id_var_misuratore,
                               pp.t_matricola_convertitore,
                               pp.n_num_cifre_convertitore
                        FROM rcugas.rcugas_pdr_var_convertitore_p as pp) as conve
       ON concat(mis.n_id_var_misuratore,mis.n_id_pdr) = concat(conve.n_id_var_misuratore,conve.n_id_pdr)) as bb
