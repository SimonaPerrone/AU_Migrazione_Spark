...
import it.sferanet.au.dal.rettifica.{A02RTable, A40RTable}
import it.sferanet.au.model.rettifica.{A02R, A40R}
...
class A40RTableTest extends EnvironmentSparkTest with Checker {
  ...
  def chekNew(obj: A40R): Unit = {
    Assert.assertTrue(obj.service == "A40R")
    Assert.assertTrue(obj.pdr == "03100000074105")
    Assert.assertTrue(dateFormat.format(obj.date.get) == "02/09/2011")
    Assert.assertTrue(obj.measure.get == 0.0)
    Assert.assertTrue(obj.converted.isEmpty)
    Assert.assertTrue(obj.serialNumberMis.get == "6852531")
    Assert.assertTrue(obj.serialNumberConv.isEmpty)
    Assert.assertTrue(obj.local_file.get == "/mnt/isilonshare_gas/TMG_03542180876/DISTRIBUTORE/TMG_03542180876_03973800877/2021/0209/03542180876_03973800877_201109_A40R_20210208152309_1_R.zip")
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = A40RTable("./src/test/resources/integrazione-ca/misure/a40r/new_route").get()
    val obj: A40R = measures.take(1)(0)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val result1 = new A40RTable(null).filterValidRows(rdd1)
    Assert.assertEquals(1, result1.count())

    val rdd2 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A40R(motivation = Some(5),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result2 = new A40RTable(null).filterValidRows(rdd2)
    Assert.assertEquals(1, result2.count())

    val rdd3 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A40R(motivation = Some(0),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result3 = new A40RTable(null).filterValidRows(rdd3)
    Assert.assertEquals(0, result3.count())

    val rdd4 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A40R(motivation = Some(6),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result4 = new A40RTable(null).filterValidRows(rdd4)
