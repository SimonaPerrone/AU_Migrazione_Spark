...
import it.sferanet.au.dal.prestazionale.S40Table
...
class S40TableTest extends EnvironmentSparkTest with Checker {
  //Environment.resetProperty()


  def checkNewRouteObj(obj: S40): Unit = {
    Assert.assertTrue(obj.isNewRoute) //se nuovo flusso (std)
    Assert.assertTrue(obj.service == "S40") //flusso
    ...
    Assert.assertTrue(obj.pdr == "00882600028944") //cod_pdr
    Assert.assertTrue(obj.local_file.get == "/mnt/isilonshare_gas/TMG_00489490011/DISTRIBUTORE/TMG_00489490011_06655971007/2021/0310/00489490011_06655971007_202005_S40_20210310180009_1_M.zip") //local_file
    Assert.assertTrue(obj.serialNumberConv.isEmpty) //matr_conv matricola convertitore
    Assert.assertTrue(obj.serialNumberMis.get == "MIT0032011590386") //matr_mis matricola misuratore
    Assert.assertTrue(obj.date.get.compareTo(Constants.STANDARD_FORMAT_DATE.parse("20/05/2020")) == 0) //data_pres
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = S40Table("./src/test/resources/integrazione-ca/misure/s40").get()
    val obj: S40 = measures.take(1)(0)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val result1 = new S40Table(null).filterValidRows(rdd1)
    ...
    val result2 = new S40Table(null).filterValidRows(rdd2)
    ...
    val result3 = new S40Table(null).filterValidRows(rdd3)
