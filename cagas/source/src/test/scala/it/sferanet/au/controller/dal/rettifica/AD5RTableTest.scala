...
import it.sferanet.au.dal.rettifica.{AD4RTable, AD5RTable}
import it.sferanet.au.model.rettifica.{AD4R, AD5R}
...
class AD5RTableTest extends EnvironmentSparkTest with Checker {
  ...
  def chekNew(obj: AD5R): Unit = {
    Assert.assertTrue(obj.service == "AD5R")
    Assert.assertTrue(obj.pdr == "00881906667948")
    Assert.assertTrue(dateFormat.format(obj.date.get) == "28/10/2019")
    Assert.assertTrue(obj.measure.get == 235171.0)
    Assert.assertTrue(obj.converted.get == 171222.0)
    Assert.assertTrue(obj.serialNumberMis.get == "0035059889")
    Assert.assertTrue(obj.serialNumberConv.get == "11L18821")
    Assert.assertTrue(obj.local_file.get == "/mnt/isilonshare_gas/TMG_00489490011/DISTRIBUTORE/TMG_00489490011_06655971007/2021/0304/00489490011_06655971007_202102_AD5R_20210304111501_1_R.zip")
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = AD5RTable("./src/test/resources/integrazione-ca/misure/ad5r/new_route").get()
    val obj: AD5R = measures.take(1)(0)
    chekNew(obj)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val result1 = new AD5RTable(null).filterValidRows(rdd1)
    Assert.assertEquals(1, result1.count())

    val rdd2 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      AD5R(motivation = Some(5),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result2 = new AD5RTable(null).filterValidRows(rdd2)
    Assert.assertEquals(1, result2.count())

    val rdd3 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      AD5R(motivation = Some(0),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result3 = new AD5RTable(null).filterValidRows(rdd3)
    Assert.assertEquals(0, result3.count())

    val rdd4 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      AD5R(motivation = Some(6),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result4 = new AD5RTable(null).filterValidRows(rdd4)
