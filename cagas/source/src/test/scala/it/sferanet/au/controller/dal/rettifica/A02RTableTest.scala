...
import it.sferanet.au.dal.rettifica.{A01RTable, A02RTable}
import it.sferanet.au.model.rettifica.{A01R, A02R}
...
class A02RTableTest extends EnvironmentSparkTest with Checker {
  ...
  def chekNew(obj: A02R): Unit = {
    Assert.assertTrue(obj.service == "A02R")
    Assert.assertTrue(obj.pdr == "01613550008444")
    Assert.assertTrue(dateFormat.format(obj.date.get) == "14/11/2016")
    Assert.assertTrue(obj.measure.get == 2.0)
    Assert.assertTrue(obj.converted.isEmpty)
    Assert.assertTrue(obj.serialNumberMis.get == "SMGR034016684180")
    Assert.assertTrue(obj.serialNumberConv.isEmpty)
    Assert.assertTrue(obj.local_file.get == "/mnt/isilonshare_gas/TMG_06724610966/DISTRIBUTORE/TMG_06724610966_06655971007/2021/0303/06724610966_06655971007_201611_A02R_20210303122701_00_R.zip")
    ...
    Assert.assertTrue(obj.motivation.get == 5)
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = A02RTable("./src/test/resources/integrazione-ca/misure/a02r/new_route").get()
    val obj: A02R = measures.take(1)(0)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val result1 = new A02RTable(null).filterValidRows(rdd1)
    Assert.assertEquals(1, result1.count())

    val rdd2 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A02R(motivation = Some(5),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result2 = new A02RTable(null).filterValidRows(rdd2)
    Assert.assertEquals(1, result2.count())

    val rdd3 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A02R(motivation = Some(0),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result3 = new A02RTable(null).filterValidRows(rdd3)
    Assert.assertEquals(0, result3.count())

    val rdd4 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      A02R(motivation = Some(6),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result4 = new A02RTable(null).filterValidRows(rdd4)
