...
import it.sferanet.au.dal.rettifica.{V01RTable, V02RTable}
import it.sferanet.au.model.rettifica.{V01R, V02R}
...
class V02RTableTest extends EnvironmentSparkTest with Checker {
  ...
  def chekNew(obj: V02R): Unit = {
    Assert.assertTrue(obj.service == "V02R")
    Assert.assertTrue(obj.pdr == "02800000068420")
    Assert.assertTrue(dateFormat.format(obj.date.get) == "02/02/2021")
    Assert.assertTrue(obj.measure.get == 1374.0)
    Assert.assertTrue(obj.converted.isEmpty)
    Assert.assertTrue(obj.serialNumberMis.get == "27920257")
    Assert.assertTrue(obj.serialNumberConv.isEmpty)
    Assert.assertTrue(obj.local_file.get == "/mnt/isilonshare_gas/TMG_06724610966/DISTRIBUTORE/TMG_06724610966_06289781004/2021/0217/06724610966_06289781004_202102_V02R_20210217131828_00_R.zip")
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = V02RTable("./src/test/resources/integrazione-ca/misure/v02r").get()
    val obj: V02R = measures.take(1)(0)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val result1 = new V02RTable(null).filterValidRows(rdd1)
    Assert.assertEquals(1, result1.count())

    val rdd2 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      V02R(motivation = Some(5),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result2 = new V02RTable(null).filterValidRows(rdd2)
    Assert.assertEquals(1, result2.count())

    val rdd3 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      V02R(motivation = Some(0),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result3 = new V02RTable(null).filterValidRows(rdd3)
    Assert.assertEquals(0, result3.count())

    val rdd4 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      V02R(motivation = Some(6),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, raccolta = null)
    )
    val result4 = new V02RTable(null).filterValidRows(rdd4)
