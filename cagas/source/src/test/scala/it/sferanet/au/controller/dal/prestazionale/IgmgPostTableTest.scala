...
import it.sferanet.au.dal.prestazionale.IgmgPostTable
...
class IgmgPostTableTest extends EnvironmentSparkTest with Checker {
  //Environment.resetProperty()


  def checkNewRouteObj(obj: IgmgPost): Unit = {
    Assert.assertTrue(obj.isNewRoute) //se nuovo flusso (std)
    Assert.assertTrue(obj.service == "IGMGPOST") //flusso
  ...
  def testAppOnlyNewRoute(): Unit = {
    ...
    val measures = IgmgPostTable("./src/test/resources/integrazione-ca/misure/igmgPost").get()
    val obj: IgmgPost = measures.take(1)(0)
  ...
  def testFilterValidRows(): Unit = {
    ...
    val rdd1 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      IgmgPost(readType = Some('S'), cau_int_cor = Some(1), cau_int_mis = None,
    ...
    val result1 = new IgmgPostTable(null).filterValidRows(rdd1)
    Assert.assertEquals(1, result1.count())

    val rdd2 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      IgmgPost(readType = Some('E'), cau_int_cor = None, cau_int_mis = Some(1),
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, coefCorr = null)
    )
    val result2 = new IgmgPostTable(null).filterValidRows(rdd2)
    Assert.assertEquals(1, result2.count())

    val rdd3 = spark.sparkContext.parallelize(Seq(1)).map(r =>
      IgmgPost(readType = Some('E'), cau_int_cor = None, cau_int_mis = None,
        ammissibilita = None, service = null, pdr = null, date = null, pivaDistr = null, pivaUtente = null,
        measure = null, converted = null, serialNumberMis = null, serialNumberConv = null, local_file = null,
        d_caricamento = null, isNewRoute = false, coefCorr = null)
    )
    val result3 = new IgmgPostTable(null).filterValidRows(rdd3)
