...
import it.sferanet.au.dal.rettifica.AD2RTable._
import it.sferanet.au.model.rettifica.AD2R
...
class AD2RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[AD2R]): RDD[AD2R] = {
    rdd
...
object AD2RTable {
  def apply(inputPath: String): AD2RTable = new AD2RTable(inputPath)
