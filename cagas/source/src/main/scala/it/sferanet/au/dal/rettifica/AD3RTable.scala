...
import it.sferanet.au.dal.rettifica.AD3RTable._
...
class AD3RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[AD3R]): RDD[AD3R] = {
    rdd
...
object AD3RTable {
  def apply(inputPath: String): AD3RTable = new AD3RTable(inputPath)
