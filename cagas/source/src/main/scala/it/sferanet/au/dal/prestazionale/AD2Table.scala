...
import it.sferanet.au.dal.prestazionale.AD2Table._
...
class AD2Table(inputPath: String) extends Serializable {
      ...
      .flatMap(r => {
        if (Flow.getIsNewRouteVersion(r, newRouteField)) {
          ...
          List(AD2(
            ...
            getString(f.pdr, r),
            getOptionString(raccolta, r), //cod_pdr
  ...
  def filterValidRows(rdd: RDD[AD2]): RDD[AD2] = {
    rdd
...
object AD2Table {
  def apply(inputPath: String): AD2Table = new AD2Table(inputPath)
