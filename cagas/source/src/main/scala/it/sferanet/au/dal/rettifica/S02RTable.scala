...
import it.sferanet.au.dal.rettifica.S02RTable._
import it.sferanet.au.model.rettifica.S02R
...
class S02RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[S02R]): RDD[S02R] = {
    rdd
...
object S02RTable {
  def apply(inputPath: String): S02RTable = new S02RTable(inputPath)
