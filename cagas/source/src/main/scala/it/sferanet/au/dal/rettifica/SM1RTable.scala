...
import it.sferanet.au.dal.rettifica.SM1RTable._
...
class SM1RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[SM1R]): RDD[SM1R] = {
    rdd
...
object SM1RTable {
  def apply(inputPath: String): SM1RTable = new SM1RTable(inputPath)
