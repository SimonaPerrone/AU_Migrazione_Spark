...
import it.sferanet.au.dal.autolettura.TasTable._
...
class TasTable(inputPath: String) extends Serializable {
  ...
  def get(): RDD[Tas] = {
    Environment.getSqlContext.read
      ...
      .map(r => {
        ...
        val f = if (isNewRouteVersion) flowFieldsNewRoute else TalTable.flowFieldsOldRoute
        Tas(
...
object TasTable {
  def apply(inputPath: String): TasTable = new TasTable(inputPath)
