...
import it.sferanet.au.dal.rettifica.A01RTable._
...
class A01RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[A01R]): RDD[A01R] = {
    rdd
...
object A01RTable {
  def apply(inputPath: String): A01RTable = new A01RTable(inputPath)
