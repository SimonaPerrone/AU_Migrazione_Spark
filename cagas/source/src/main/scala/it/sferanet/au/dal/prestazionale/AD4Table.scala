...
import it.sferanet.au.dal.prestazionale.AD4Table._
import it.sferanet.au.model.prestazionale.AD4
...
class AD4Table(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[AD4]): RDD[AD4] = {
    rdd
...
object AD4Table {
  def apply(inputPath: String): AD4Table = new AD4Table(inputPath)
