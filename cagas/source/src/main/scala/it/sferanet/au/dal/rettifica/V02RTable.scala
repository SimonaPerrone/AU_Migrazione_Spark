...
import it.sferanet.au.dal.rettifica.V02RTable._
import it.sferanet.au.model.rettifica.V02R
...
class V02RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[V02R]): RDD[V02R] = {
    rdd
...
object V02RTable {
  def apply(inputPath: String): V02RTable = new V02RTable(inputPath)
