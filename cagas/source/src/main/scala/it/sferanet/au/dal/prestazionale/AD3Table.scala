...
import it.sferanet.au.dal.prestazionale.AD3Table._
...
class AD3Table(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[AD3]): RDD[AD3] = {
    rdd
...
object AD3Table {
  def apply(inputPath: String): AD3Table = new AD3Table(inputPath)
