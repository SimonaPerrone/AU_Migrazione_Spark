...
import it.sferanet.au.dal.rettifica.AD5RTable._
import it.sferanet.au.model.rettifica.AD5R
...
class AD5RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[AD5R]): RDD[AD5R] = {
    rdd
...
object AD5RTable {
  def apply(inputPath: String): AD5RTable = new AD5RTable(inputPath)
