...
import it.sferanet.au.dal.rettifica.A01RTable.{flowFields, motivazione, raccolta}
import it.sferanet.au.dal.rettifica.A02RTable._
import it.sferanet.au.model.rettifica.A02R
...
class A02RTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[A02R]): RDD[A02R] = {
    rdd
...
object A02RTable {
  def apply(inputPath: String): A02RTable = new A02RTable(inputPath)
