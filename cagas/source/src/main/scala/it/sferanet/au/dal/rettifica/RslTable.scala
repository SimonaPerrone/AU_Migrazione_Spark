...
import it.sferanet.au.dal.rettifica.RslTable._
...
class RslTable(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[Rsl]): RDD[Rsl] = {
    rdd
...
object RslTable {
  ...
  def apply(inputPath: String): RslTable = new RslTable(inputPath)
