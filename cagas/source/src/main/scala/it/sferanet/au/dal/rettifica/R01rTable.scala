...
import it.sferanet.au.dal.rettifica.R01rTable._
import it.sferanet.au.model.rettifica.R01r
...
class R01rTable(inputPath: String) extends Serializable {
      ...
      .flatMap(r => {
        if (Flow.getIsNewRouteVersion(r, newRouteField)) {
          ...
          List(R01r(
            ...
            getOptionInt(R01rTable.motRettLett, r),
  ...
  def filterValidRows(rdd: RDD[R01r]): RDD[R01r] = {
    rdd
...
object R01rTable {
  ...
  def apply(inputPath: String): R01rTable = new R01rTable(inputPath)
