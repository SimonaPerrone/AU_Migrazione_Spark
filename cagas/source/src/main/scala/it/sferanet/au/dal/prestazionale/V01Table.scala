...
import it.sferanet.au.dal.prestazionale.V01Table._
...
class V01Table(inputPath: String) extends Serializable {
  ...
  def filterValidRows(rdd: RDD[V01]): RDD[V01] = {
    rdd
...
object V01Table {
  ...
  def apply(inputPath: String): V01Table = new V01Table(inputPath)
