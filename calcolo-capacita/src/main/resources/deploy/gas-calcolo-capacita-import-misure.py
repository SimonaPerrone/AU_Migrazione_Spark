...
if __name__ == "__main__":
    ...
    annomese_sbg_param = ((str(list(filter(lambda x: 'ANNOMESE_SBG' in x, script_args_list))[0])).split('=')[1]).strip()
    annomese_sbg=annomese_sbg_param.split('/')[0]+annomese_sbg_param.split('/')[1]
    ...
    #GESTISCO IL MESE -1
    datCalc=annomese_sbg+"01"
    d = datetime.strptime(datCalc, "%Y%m%d")
    dPrev = d - dateutil.relativedelta.relativedelta(months=1)
    annomese_sbg=dPrev.strftime("%Y%m")

    #CONTROLLO SE DEVE GESTIRE LA CHIAMATA AUTOMATICA, CIOE SE DEVE RECUPERARE LA DATA ATTUALE E SOTTRARRE -1 MESE
    autamtic_param = (
        (str(list(filter(lambda x: 'AUTOMATIC' in x, script_args_list))[0])).split('=')[1]).strip()
    automatic = autamtic_param
    print(automatic)
    if automatic=="true":
        now = datetime.now().strftime("%Y%m%d")
        nowD = datetime.now().strptime(now, "%Y%m%d")
        a = nowD - dateutil.relativedelta.relativedelta(months=1)
        annomese_sbg=a.strftime("%Y%m")
        print(annomese_sbg)
