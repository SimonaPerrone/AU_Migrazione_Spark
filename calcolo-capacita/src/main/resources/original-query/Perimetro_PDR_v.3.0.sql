DROP VIEW CLG.CLG_PERIMETRO_PDR_V;

/* Formatted on 14/10/2021 15:40:09 (QP5 v5.227.12220.39754) */
CREATE OR REPLACE FORCE VIEW CLG.CLG_PERIMETRO_PDR_V
(
   DATA_CALC,
   N_ID_PDR,
   T_CODICE_PDR,
...
AS
   WITH INPUT_DATA
        AS (SELECT TRUNC (TO_DATE ('2021/08/30', 'yyyy/mm/dd')) DATA_CALC,
                   CASE
                      WHEN TO_NUMBER (
                              TO_CHAR (
                                 TRUNC (TO_DATE ('2021/08/30', 'yyyy/mm/dd')),
                                 'MM')) BETWEEN 10
                                            AND 12
                      THEN
                         TO_CHAR (
   ...
   SELECT D.DATA_CALC,
          ...
          TRATT.T_TRATTAMENTO_SETTLEMENT                                 --TMP
                                        ,
          PROF.T_COD_CAT_USO                                             --TMP
     FROM RCUGAS.RCUGAS_PDR PDR
          ...
          JOIN
          RCUGAS.RCUGAS_VAR_PREL_ANNUO PREL
